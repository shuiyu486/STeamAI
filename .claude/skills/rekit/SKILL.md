...
| `/rekit plan-subagents` | 内部只读计划器：根据 pack manifest 的 `subagentRoutes` 生成子 agent 分片计划；默认经 Go façade 写 `.rekit/reviews/...` 审查产物，并在 `packet.json` / `summary.md` 输出每个 shard 的 read-only reviewer `shardHandoffs[]`、dispatch prompt、expected output、`ledgerWritebackTemplates[]` verification / decision note 命令模板、required fields / allowed values、post-review merge guidance 和 main-agent merge boundary；不启动 agent，不替 reviewer 写 ledger，不写 authority/confirmed，不修改 managed/project source files。日常不要主动推荐给用户。 |
