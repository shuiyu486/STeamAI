...
如果用户没有显式给 `Target`，在 case 模式下使用当前目录向上找到的最近 attached case root；在 kit 模式下 `doctor/status` 作用于 kit 本身。`status` 只读检测迁移，不静默修复；`repair` 写入前必须得到用户确认。
...
13. `plan-subagents` 不是日常用户入口。planning mode 只写 review artifacts 且不自动 spawn agent；reviewer intake 必须由主 Agent显式提供 `PacketPath`、`ReviewerResultPath`、`Lane`、`Actor`，并先用 WhatIf 审查，再用 Apply 让 runtime 写 verification/main decision。不要要求用户手动调用，也不要把该路径误述为 reviewer 自行写 ledger 或 runtime 自动 dispatch。
