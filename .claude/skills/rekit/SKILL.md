...
| `/rekit validate` | `pwsh <templateRoot>/rekit/rekit.ps1 validate [-Target <case>]` |

如果用户没有显式给 `Target`，在 case 模式下使用当前工作目录；在 kit 模式下 `validate/status` 作用于 kit 本身。
...
4. 若 promote 命中绝对路径、样本名、trace/dump/artifact 路径或明显地址快照，先阻止或生成候选报告，不要静默写回模板。
5. 所有写操作后都运行对应 validate；失败时如实报告错误与下一步。
