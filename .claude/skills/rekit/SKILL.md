...
13. Go façade 默认关闭；只有维护者显式设置 `REKIT_GO_ENABLE=1` 时，`status`、`packs`、kit/case `doctor/validate`、`sync/promote` review-only、`gate -WhatIf`、已初始化 case 的 `overview -Format json`、`note -List -Format json`，以及 `start`/`handoff`/`continue` 的 `-WhatIf -Format json` 非写入预览才可委托 Go。`REKIT_GO_DISABLE=1` 强制回退 PowerShell；overview 文本/缺 board 初始化、文本工作线预览、note append 和所有写入路径继续走 PowerShell fallback 或手动 Go CLI。
