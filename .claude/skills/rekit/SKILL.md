...
13. Go façade 默认关闭；只有维护者显式设置 `REKIT_GO_ENABLE=1` 时，`status`、`packs`、kit/case `doctor/validate`、`sync/promote` review-only、`gate -WhatIf`，以及 `start`/`handoff`/`continue` 的 `-WhatIf -Format json` 非写入预览才可委托 Go。`REKIT_GO_DISABLE=1` 强制回退 PowerShell；文本工作线预览和所有写入路径继续走 PowerShell fallback 或手动 Go CLI。
