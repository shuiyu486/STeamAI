...
| `/rekit continue <name>` | 明确接手某条功能支线，只整理该支线的 workspace/outbox、路由 request、刷新接续提示；`-WhatIf -Format json` 预览收集、路由、authority append 计划和结构化 `missionBrief`；显式 `-Apply` 的 JSON envelope、run `status.json` 与 digest 记录 apply 后 `missionBrief`，让 lane executor 直接看到 pending gates 与非阻塞 authorized gates；若该 lane 存在 effective open intervention，则 fail-closed，先要求 `/rekit reconcile <name> -InterventionId <eventId> -Apply`。 |
| `/rekit reconcile <name>` | 显式把 Human-in-the-Lane 干预 reconcile 到 durable lane state；`-WhatIf -Format json` 预览 resolution event、lane event、executor takeover 与 resume/checkpoint/board 刷新；显式 `-Apply` 只写 case-local interventions ledger、lane events、lane.json、RESUME/checkpoint 和 board，不执行 heavy-tool、不写 authority/confirmed。 |
