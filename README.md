...
生成的 `packet.json` / `summary.md` 会标出 route 选择原因、每个 shard 的初始 `planned` 状态、每个 shard 的 `shardHandoffs[]` read-only reviewer dispatch prompt、spawn/merge 责任、verdict 写回入口和被 runtime 阻止的动作（例如 runtime 不自动 spawn、子 agent 不写文件）。这些字段只帮助主会话调度和审计 bounded dispatch，不代表 runtime 会启动 reviewer。
