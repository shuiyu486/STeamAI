...
一句话：**用户主要指挥主 Agent / Mission Commander；主 Agent 调度 durable member lanes、可替换会话执行体和短命 tactical subagents；lane-local `RESUME.md` / typed checkpoint 持久化 Mission Control brief、executor action snapshot、gate snapshot 与下一步动作；`/rekit`、Go CLI/backend 是背后的 canonical deterministic runtime/API，`rekit.ps1` 仅作为迁移期 legacy façade，默认路径继续向 PowerShell-free / Go-native / 跨平台收敛。**
