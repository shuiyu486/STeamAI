...
| `/rekit run-current-loop` | case-local bounded review-first loop | 主 Agent/harness在同一初始 route/lane 上连续推进最多 `-MaxSteps 1..20` 个 deterministic current steps。先运行 `-WhatIf -Format json`，再以返回的 `expectedCurrentLoopPlanSha256` 和相同 `MaxSteps` 显式 Apply；每步都刷新 durable status、重建 exact nested plan并留下 receipt。external reviewer stop 还会返回 `continuationRequest`：按本段已执行步数扣减后的 `remainingMaxSteps`、fresh WhatIf command和结构化 observation alternatives；主Agent/harness只需添加所选真实observation，再做新的hash-bound preview。各segment receipts不自动累计；没有前一typed output时只能启动fresh loop，不能声称恢复旧预算。managed packet把`reviewerResultDropPath`标为canonical input destination，result save必须从与drop/input/source paths分离的现有case-local `ReviewerResultInputSourcePath`进入；无managed input-save capability的direct packet则把drop path标为direct result destination，外部harness写入唯一ReviewerResult后fresh preview进入strict intake。route/lane漂移、guidance/blocker、新出现的Human-in-the-Lane reconcile、no-progress、step limit或nested error都会停止；已成功步骤不回滚。runtime不跨lane、不调用Agent tool、不管理session、不执行heavy-tool、不写authority/confirmed；必须显式`-Target`且只支持JSON。 |
