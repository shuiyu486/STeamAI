...
| `/rekit release-run` | 只读本机验证 | 顺序执行 `release-check` gateProfile 已解析的 local release minimum steps，汇总每步 exit code / duration / output tail；不写 repo/case state，不执行 heavy tool，不写 authority/confirmed。 |
