...
| 命令 | 方向 | 什么时候用 |
|---|---|---|
| `/rekit status` | 只读 | 看当前 case 绑定状态、case-local thin shim / canonical skill readiness 与 Mission Commander 接手第一屏；若目录被移动或 shim drift，只提示，不修复；case 模式会投影 `pendingGateHandoffs[]` 的 review / WhatIf / request-decision boundary 与 `authorizedGateHandoffs[]` 的 execution report contract boundary；`-Format json` 输出机器可读 status envelope 与 `caseShim` readiness。 |
