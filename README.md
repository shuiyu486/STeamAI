...
一句话：**用户主要指挥主 Agent / Mission Commander；主 Agent 调度 durable member lanes、可替换会话执行体和短命 tactical subagents；`status`、`overview`、project/lane `handoff`、`continue` artifacts、lane-local `RESUME.md` 与 typed checkpoint 现在共享 Mission Commander action queue，并把 current action 投影为 `inspect-current → apply-or-run-current / preview-current → refresh-state → follow-up-after-refresh` 的只读 run-loop；authorized-gate adapter live validation 也共享 `currentRunLoopStepId` / `runLoop`、dispatch command、owner/provenance 和 tooling lineage，让主 Agent / replacement executor 先看 current action、当前 validation 步骤、boundary 与刷新步骤，而不是在多份 JSON/Markdown 间手工拼接；`overview` JSON/text 与 project handoff 仍提供逐 lane typed executor action index 和 blocker-aware 下一步，`start` / `gate` / `reconcile` envelope/text、`handoff` JSON/Markdown、`continue` envelope/run artifacts 与 lane-local `RESUME.md` / typed checkpoint 复用 Mission Control brief、基于 typed facts 的 executor action blocker counts、gate snapshot 与下一步动作；`/rekit`、Go CLI/backend 是背后的 canonical deterministic runtime/API，`rekit.ps1` 仅作为 retained compatibility façade，不承载业务 runtime，也没有 PowerShell 业务 fallback；默认路径继续向 PowerShell-free / Go-native / 跨平台收敛。**
