...
| `/rekit handoff` | case-local 状态 | 生成项目级接手索引 `.rekit/handovers/latest.md`；索引包含 Mission Control brief，汇总 ready/blocked lanes、pending gates、open decisions、interventions、next agent actions 与 escalations；维护自动化可用 `-WhatIf -Format json` 消费写入预览，显式 `-Apply` 输出 Go JSON envelope；不代表某个会话。 |
