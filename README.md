...
```text
/rekit          # 无子命令时默认只读 status，先确认 kit/case 绑定，并先看 compact Mission Commander first-screen strip / focus routing / queue current action 接手；kit-mode 会先投影 latest-batch project current action 与 project runbook，再回到 case/reviewer/pack-memory first-screen strip，空 case mission 会先投影 onboarding current action
/rekit doctor
```
