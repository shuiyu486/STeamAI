...
lane executor / tool adapter 执行前可以由主 Agent 调用 `gate -ExecutionReportContract -GateEventId <id> -Format json` 读取只读 contract。该 contract 投影已授权 gate 的 action、budget、output paths、allowed statuses、stop conditions、record/notify、report path rule、summary/escalation size、boundary/escalation requirements、validation failure stages/codes 与 denied actions，供 adapter 在不重扫 ledger 的情况下做 live validation，并在执行前知道 `gate -ValidateExecutionReport` 可能返回的 stable failure taxonomy；该读取不写 ledger、不执行工具、不写 authority/confirmed。
