...
lane executor / tool adapter 完成 `authorized-gate` 覆盖的实际动作并写出 sidecar 后，主 Agent 可以先调用 `gate -ValidateExecutionReport -GateEventId <id> -ExecutionReportPath <path> -Format json` 做只读 strict validation preflight。该路径复用正式 intake 的 path/schema/action/status/gateEventId/budget/ref/boundary/escalation 校验，输出 `isMutation=false` / `applied=false` / `valid=true` envelope，不写 observations ledger、不执行工具、不写 authority/confirmed。
