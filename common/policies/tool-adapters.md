...
lane executor / tool adapter 执行前可以从 Mission brief / overview / handoff / continue artifacts 的 authorized-gate line 直接取得 `eventId` 与可复制 `reportContract=/rekit gate -ExecutionReportContract -GateEventId <id> -Format json` handoff，也可以由主 Agent 在 case-local 或 authorized output workspace cwd 中调用同一 command 读取只读 contract，省略 `-Target` 时 Go runtime 会向上解析 attached case root。该 contract 投影已授权 gate 的 action、budget、output paths、allowed statuses、stop conditions、record/notify、report path rule、summary/escalation size、boundary/escalation requirements、validation failure stages/codes、`validationRepairHints[]` 与 denied actions，并提供 `liveValidation` handoff：包含 `authorizedWorkspaces[]`、`reportFileName`、`caseRelativeReportPath`、可直接填充的 bounded sidecar template、workspace-relative `validateCommand` / `recordCommand` 与 `validateArgs` / `recordArgs`、case-relative `caseRelativeValidateCommand` / `caseRelativeRecordCommand` 与 `caseRelativeValidateArgs` / `caseRelativeRecordArgs`、从 pack `tooling/catalog.yml` 结合本次 `heavyToolGates` action 投影出的 `adapterCandidates[]`、默认 `selectedAdapter`、replay behavior（重复 `RecordArgs` / `CaseRelativeRecordArgs` 返回 `duplicate eventId` 且不追加 observations）、`<executor-id>` replacement guidance 与 no-write/no-heavy-tool notes；contract 同时给出 `defaultReportPath` 作为首选 case-relative sidecar 位置。adapter live validation 还会投影 `dispatchCommand`、`currentRunLoopStepId` / `runLoop`（`inspect-contract → record-dispatch → run-external-adapter → draft-or-write-report → validate-report → record-receipt → record-observation → review-recorded-evidence`）以及 current executor/generation、harness/session、tooling catalog SHA、artifact count 等 owner/provenance 字段；status、overview、handoff 与 validation 会复用同一投影。adapter 应从 `adapterCandidates[]` 中选择具体工具，把 sidecar `adapterId` 设为该 candidate id，使后续 read-only validation 与 observation evidence 能保留具体 pack tooling provenance；若没有候选，仍必须显式填写 bounded adapter id 并遵守同一授权边界。adapter 可在不重扫 ledger 或手工拼接命令的情况下做 live validation，并在执行前知道当前接手步骤以及 `gate -ValidateExecutionReport` 可能返回的 stable failure taxonomy 与 repair actions；该读取不写 ledger、不执行工具、不写 authority/confirmed。

durable lane 且本次 action 选中 tooling catalog candidate 时，外部 adapter 开始前必须先消费 report contract / status / handoff 返回的 `gate -RecordAdapterExecutionDispatch` preview，并执行其 exact expected-binding-hash Apply，写入 immutable pre-execution dispatch。dispatch 绑定 authorized gate、current lane owner/generation、selected candidate/catalog、harness/session、authorized budget 与 canonical report path；report、completion receipt 与 observation 必须保留同一 dispatch ID/path/SHA lineage。一个 authorized gate 只对应一个 attempt；report-first backfill、same-gate retry及 owner/generation/session/catalog/gate/report path drift均 fail-closed，retry必须使用 distinct authorized gate。若 dispatch current 但 report 尚缺失，应等待 external harness；只有 harness outcome 已知为 `failed` 或 `aborted` 时，才可用 runtime 返回的 canonical report path执行 `gate -DraftExecutionReport -ExecutionStatus failed|aborted` preview→expected-hash Apply。runtime 不自动把超时、失联或未启动推断为失败；takeover 后新 owner 不能采用旧 dispatch。existing exact report replay保持幂等。
...
```json
{
  ...
  "dispatch": {
    "dispatchId": "<immutable-dispatch-id>",
    "path": ".rekit/lanes/<lane>/adapter-executions/<gateEventId>/dispatch.json",
    "sha256": "<dispatch-file-sha256>"
  },
