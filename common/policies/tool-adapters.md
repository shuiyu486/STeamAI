...
lane executor / tool adapter 执行前可以从 Mission brief / overview / handoff / continue artifacts 的 authorized-gate line 直接取得 `eventId` 与可复制 `reportContract=/rekit gate -ExecutionReportContract -GateEventId <id> -Format json` handoff，也可以由主 Agent 在 case-local 或 authorized output workspace cwd 中调用同一 command 读取只读 contract，省略 `-Target` 时 Go runtime 会向上解析 attached case root。该 contract 投影已授权 gate 的 action、budget、output paths、allowed statuses、stop conditions、record/notify、report path rule、summary/escalation size、boundary/escalation requirements、validation failure stages/codes 与 denied actions，并新增 `liveValidation` handoff：包含 `authorizedWorkspaces[]`、`reportFileName`、`caseRelativeReportPath`、可直接填充的 bounded sidecar template、workspace-relative `validateCommand` / `recordCommand` 与 `validateArgs` / `recordArgs`、case-relative `caseRelativeValidateCommand` / `caseRelativeRecordCommand` 与 `caseRelativeValidateArgs` / `caseRelativeRecordArgs`、replay behavior 与 no-write/no-heavy-tool notes；contract 同时给出 `defaultReportPath` 作为首选 case-relative sidecar 位置。adapter 可在不重扫 ledger 或手工拼接命令的情况下做 live validation，并在执行前知道 `gate -ValidateExecutionReport` 可能返回的 stable failure taxonomy；该读取不写 ledger、不执行工具、不写 authority/confirmed。
