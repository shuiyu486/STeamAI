...
- 已固化：`common/policies/agent-team.md`（角色 + packet + 状态流）、`common/policies/subagents.md`（L1/L2/L3 + output contract）、manifest `subagentRoutes`、Go-owned Mission Control/workstream runtime、vmp-re pack、`_template` pack。
- 已落地：Go-owned ledger runtime（9 种 kind、batchId、overview/handoff/note-List、run digest）、sync/promote review-first、Go review-only plan/artifacts、Go gate authorization preflight 与 pending-gate / authorized-gate request ledger decision 写入、authorized execution observation evidence / adapter execution report strict intake 写入与 read-only validation `valid=true/false` envelope/failure taxonomy、durable lane reconcile / executor takeover、start/continue/handoff/overview autonomy profile summary、Go façade 对 read-only/case lifecycle/sync/promote/ledger/gate/start/handoff/continue/reconcile preview/apply safe subset 的默认接管，`_template` pack 的 Go package workstream、gate/dispatch 与 reviewer/decision 闭环测试，`generic-binary-re` 的 pack-neutral non-feature lane E2E 测试、`web-security` 的非 RE-only pack-neutral route/network gate E2E 测试，安全领域 pack 的真实临时 case Agent Team dry-run smoke，以及 pack-memory promote/reconsume package E2E。
...
- **Go runtime current owner**：R4-R6 初期曾以 PowerShell runtime 为准；Batch 240 后 legacy modules 已删除，当前 25 个 public commands 全部 Go-owned/no-fallback。新增逻辑只进入 Go packages；`REKIT_GO_DISABLE=1` 不再回落到 PowerShell 业务实现。
