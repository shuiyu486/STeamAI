...
- 当前状态：公共 `/rekit` 默认路径已由全部 32 个 Go-owned/no-fallback public commands 支撑，legacy `rekit/lib/*.ps1` 已删除，`rekit.ps1` 仅 retained compatibility façade；CLI registry 为每个 public command 绑定唯一 callback owner，当前声明的 shared capability sinks 也已纳入 admission。`release-run` 提供只读本机 release minimum driver，`next-batch` 提供 kit review-first planning receipt。Ledger runtime 支持 9 种 kind、`batchId`、Mission brief / executor action、显式 `reconcile`、typed autonomy preflight 与 pending/authorized gate visibility；`plan-subagents` planning mode 已输出 shard handoff、reviewer result/intake/decision/writeback contracts且不自动 spawn reviewer；Batch 353 已由主 Agent在本机真实 spawn 一个 read-only reviewer，并通过显式 Go-native reviewer intake 完成 strict WhatIf/Apply、verification-before-decision 写回与 post-validation。`binary-re` 与 `web-security` 的已声明 production adapter slices、execution evidence 与 installed real-Claude 验收已闭合；当前缺口是专属 owner 内聚、DTO/reducer/session data-flow 统一、未来 provider integration、更多 pack 的真实 session 产品场景与跨平台 product-path E2E；current baseline 见 `docs/go-first-convergence-plan.md` 顶部与 `docs/batch-plan.md` 的 current/active/next 区。
...
- 已固化：`common/policies/agent-team.md`（角色 + packet + 状态流）、`common/policies/subagents.md`（L1/L2/L3 + output contract）、manifest `subagentRoutes`、Go-owned Mission Control/workstream runtime、`binary-re` 与 `web-security` mature production pack、`_template` pack。
- 已落地：Go-owned ledger runtime（9 种 kind、batchId、overview/handoff/note-List、run digest）、sync/promote review-first、Go review-only plan/artifacts、Go gate authorization preflight 与 pending-gate / authorized-gate request ledger decision 写入、authorized execution observation evidence / adapter execution report strict intake 写入与 read-only validation `valid=true/false` envelope/failure taxonomy、durable lane reconcile / executor takeover、start/continue/handoff/overview autonomy profile summary、Go façade 对 read-only/case lifecycle/sync/promote/ledger/gate/start/handoff/continue/reconcile preview/apply safe subset 的默认接管，`_template` pack 的 Go package workstream、gate/dispatch 与 reviewer/decision 闭环测试，历史 `generic-binary-re` 能力已收敛到 `binary-re`，`binary-re` 与 `web-security` 的 production vertical-slice adapter/evidence/member/Reviewer 验收已闭合，安全领域 pack 的真实临时 case Agent Team dry-run smoke，以及 pack-memory promote/reconsume package E2E。
- 未落地：Mission Commander/member session manager、runtime-managed bounded reviewer auto-spawn/session orchestration、candidate → verified → confirmed 的机器强制 gate、真实 heavy-tool adapter execution/isolation、adapter-specific live validation beyond bounded report intake、真实多 pack/跨 case pack-memory UX、retained PowerShell façade 最终处置，以及 Linux/macOS/Windows product-path E2E；单 reviewer 显式 intake/writeback 本机 E2E 已在 Batch 353 完成。
...
- **不要把 dry-run 当成 runtime 实现**：R0-R2 只压测契约；新增 deterministic runtime 逻辑进入 `internal/rekit/**`，不要重新引入已删除的 `rekit/lib/*.ps1`。
...
- **Go runtime current owner**：R4-R6 初期曾以 PowerShell runtime 为准；Batch 240 后 legacy modules 已删除，当前全部 32 个 public commands 均为 Go-owned/no-fallback，并各有唯一 callback owner。新增逻辑只进入 Go packages；`REKIT_GO_DISABLE=1` 不再回落到 PowerShell 业务实现。
...
以下 R0–G5 细节保留当时的 PowerShell → Go 迁移记录；其中 `rekit/lib/*.ps1` 路径只表示历史实施位置，不是当前实现建议。当前事实以本文顶部、`docs/go-first-convergence-plan.md` current baseline 和 `docs/release-readiness.md` 为准。
...
| 子 agent 契约 | `common/policies/subagents.md` + manifest `subagentRoutes` | L1/L2/L3 升级、bounded parallelism、output contract；当前 `binary-re` 与 `web-security` 各自声明领域 routes |
| 工作线 runtime | `internal/rekit/workstream`、`mission`、`overview`、`note`、`gate`、`autonomy` | manifest 驱动 durable lanes、Mission brief、executor action、handoff/reconcile 与 typed authorization preflight；legacy `rekit/lib/B3.*.ps1` 已删除 |
| pack 领域层 | `packs/binary-re/**`、`packs/web-security/**`、`packs/_template/**` | `binary-re` 与 `web-security` 为 mature production 参考，`_template` 保持作者骨架；各 pack 通过自身 manifest/budget 校验 |
| sync/promote review-first | `internal/rekit/sync`、`promote`、`review` | review packet + bounded diff + sanitized preview；写入需确认 |
| Go runtime backend | `internal/rekit/**`、`cmd/rekit/main.go` | G1/G2.5 只读 status/doctor/manifest/case doctor + G2 sync/promote review/apply、G2.2/G2.3 gate preview/request 与 Stage 5 read/ledger/workstream layer 已逐步接入 façade；Batch 114 起 `gate -WhatIf` 默认委托 Go，Batch 115 起 `gate -Apply` pending-gate request 写入默认委托 Go，Batch 116 起 `note` append / `note -WhatIf` facts JSONL 写入/预览默认委托 Go，Batch 117 起 `start`/`handoff` JSON preview 与显式 apply 默认委托 Go，Batch 118 起 `continue -WhatIf -Format json` 非写入 preview 默认委托 Go，Batch 119 起 overview 文本/JSON 与缺 board 初始化默认委托 Go，Batch 120 起 `note -List` 文本/JSON 只读查询默认委托 Go，Batch 121 起 explicit `continue -Apply` case-local facts/routing/run digest/resume/board 写入默认委托 Go，Batch 122 起 `_template` pack package E2E 覆盖 start/note/continue/handoff 闭环，Batch 123 起 `_template` pack package E2E 覆盖 plan-subagents/gate/overview/handoff 可见性链路，Batch 124 起 `_template` pack package E2E 覆盖 candidate/verification/decision/note-list/overview/handoff 可见性链路，Batch 125 起 `generic-binary-re` package E2E 覆盖 non-feature lane、pack-specific route、candidate/gate/verification/decision/overview/handoff 的 pack-neutral 可见性链路，Batch 126 起 `web-security` package E2E 覆盖非 RE-only route、network gate、candidate/gate/verification/decision/overview/handoff 的 pack-neutral 可见性链路，Batch 127 起 `web-security-agent-team-dryrun-smoke.ps1` 覆盖公共 façade 真实临时 case dry-run，authority/confirmed 仍需人工确认 |
...
| heavy-tool gate runtime | Go `gate -WhatIf/-Apply` 可预览或记录 pending-gate / authorized-gate request ledger decision；durable lane autonomy profile 完全覆盖时可记录 `authorized-gate`，并在 Mission brief / overview / handoff / continue-facing artifacts 中直接展示 requested budget、authorized output paths、stop conditions、gate eventId 与可复制 report contract command；`gate -ExecutionReportContract -GateEventId ... -Format json` 可在动作前只读投影 adapter execution report contract（含 `defaultReportPath`、`liveValidation.authorizedWorkspaces[]` / `reportFileName` / `caseRelativeReportPath`、workspace-relative 与 case-relative validate/record handoff，以及 validation taxonomy）；`gate -ValidateExecutionReport -GateEventId ... -ExecutionReportPath ... -Format json` 可在记录前只读 strict validation bounded adapter sidecar；`gate -Apply -GateEventId ... -ExecutionStatus ... -ExecutionReportPath ...` 可在动作后写回 observation execution evidence（actual budget / output refs / evidence refs / boundary hits / escalation）并 strict intake bounded adapter execution report；sidecar 声明 boundary/escalated 或预算越界时必须自带 boundary/escalation marker；不执行工具、不写 authority/confirmed | lane executor / tool adapter 在 full-trace/debug/inject/patch/dump 前消费 gate decision、autonomy profile 与 report contract，并把真实工具调用、隔离、停止条件执行和 adapter-specific live validation 留在 adapter/lane 层 |
