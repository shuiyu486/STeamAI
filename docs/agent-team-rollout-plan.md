...
- **Go runtime current owner**：R4-R6 初期曾以 PowerShell runtime 为准；Batch 240 后 legacy modules 已删除，当前 26 个 public commands 全部 Go-owned/no-fallback。新增逻辑只进入 Go packages；`REKIT_GO_DISABLE=1` 不再回落到 PowerShell 业务实现。
