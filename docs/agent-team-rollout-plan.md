...
- 已落地：Go-owned ledger runtime（9 种 kind、batchId、overview/handoff/note-List、run digest）、sync/promote review-first、Go review-only plan/artifacts、Go gate authorization preflight 与 pending-gate / authorized-gate request ledger decision 写入、authorized execution observation evidence 写入、durable lane reconcile / executor takeover、start/continue/handoff/overview autonomy profile summary、Go façade 对 read-only/case lifecycle/sync/promote/ledger/gate/start/handoff/continue/reconcile preview/apply safe subset 的默认接管，`_template` pack 的 Go package workstream、gate/dispatch 与 reviewer/decision 闭环测试，`generic-binary-re` 的 pack-neutral non-feature lane E2E 测试、`web-security` 的非 RE-only pack-neutral route/network gate E2E 测试，安全领域 pack 的真实临时 case Agent Team dry-run smoke，以及 pack-memory promote/reconsume package E2E。
- 未落地：Mission Commander/member session manager、runtime-managed bounded reviewer auto-spawn/session orchestration、candidate → verified → confirmed 的机器强制 gate、真实 heavy-tool adapter execution/isolation、真实多 pack/跨 case pack-memory UX、retained PowerShell façade 最终处置，以及 Linux/macOS/Windows product-path E2E；单 reviewer 显式 intake/writeback 本机 E2E 已在 Batch 353 完成。
