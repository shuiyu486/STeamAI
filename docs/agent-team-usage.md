...
/rekit promote -PacketPath <candidate-review-packet> -CandidateDecisionPath <decisions.json> -Apply -Format json # 主 Agent复核同一 WhatIf 后显式执行 managed-doc merge 或 candidate cleanup；返回 candidate/target backups、durable decision receipt 与 recovery envelope；tooling-only reject/superseded receipt无需 reconsume verification并直接闭环
...
/rekit plan-subagents -PacketPath <canonical-case-review-packet> -CollectReviewerResult -ShardId <shard> -Lane <lane> -Actor <actor> -WhatIf -Format json # 仅canonical `.rekit/reviews/<单层review>/packet.json`提供此命令；主Agent保存唯一JSON candidate后strict预览绑定，不创建canonical result
...
/rekit plan-subagents -PacketPath <canonical-case-review-packet> -RecoverReviewerResult -ShardId <shard> -Lane <lane> -Actor <actor> -Reason <reason> -WhatIf -Format json # candidate正确但canonical result含不同regular bytes时，复核exact candidate/result hashes与quarantine路径；不覆盖、不写facts
/rekit plan-subagents -PacketPath <canonical-case-review-packet> -RecoverReviewerResult -ShardId <shard> -Lane <lane> -Actor <actor> -Reason <reason> -ExpectedCandidateSha256 <hash> -ExpectedReviewerResultSha256 <hash> -Apply -Format json # 显式隔离冲突bytes并写strict intent/receipt；中断后同一路径可确定性finalize，恢复后仍需collection WhatIf→Apply
