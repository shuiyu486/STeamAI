...
# campaign continuation：成功Apply将本段按严格递增sequence写入immutable `.rekit/runs/current-loop-segments/<sequence>.json`，并用前一个exact artifact SHA形成完整chain；新会话只消费`status.missionControlRunbook.currentLoopSegment`或explicit`handoff.currentLoopSegment`中`state=ready`的continuation.whatIfCommand（已按本segment appliedSteps扣减remainingMaxSteps）。reader必须验证无gap/fork chain、canonical exact bytes、case/pack、重算的outer plan/request-receipt lineage和refreshed current request；wall-clock回退不改变latest，tamper、chain gap/break、symlink、unknown entry、stale request、terminal、status-unavailable均不回退旧artifact、不暴露continuation。fresh preview仍重验expectedRoute/expectedLane，不能复用旧segment hash或累计旧receipts。external reviewer另从observationContract添加一个alternative的真实flags；managed packet把reviewer JSON先写到与drop/input/source paths分离的现有case-local source，无managed input-save capability时按typed direct-write alternative写到direct destination。checkpoint/continuation不授权自动跨route/lane或自动reconcile
