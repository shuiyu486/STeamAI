...
/rekit plan-subagents -PacketPath <packet> -CollectReviewerResult -ShardId <shard> -Lane <lane> -Actor <actor> -WhatIf -Format json # 主Agent把Agent tool返回的唯一JSON保存到packet派生candidate path后，strict预览packet/route/shard/items/evidence/output绑定；不创建canonical result
/rekit plan-subagents -PacketPath <packet> -CollectReviewerResult -ShardId <shard> -Lane <lane> -Actor <actor> -Apply -Format json # 复核同一WhatIf后，以exact bytes/no-overwrite方式发布到packet派生canonical result；不同内容拒绝覆盖、相同内容幂等
