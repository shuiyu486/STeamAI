...
# campaign continuation：只消费 continuationRequest.whatIfCommand（已按本segment appliedSteps扣减remainingMaxSteps），fresh preview必须重验 expectedRoute/expectedLane，不能复用旧segment hash或累计旧receipts。external reviewer另从observationContract添加一个alternative的真实flags；managed packet把reviewer JSON先写到与drop/input/source paths分离的现有case-local source，无managed input-save capability时按typed direct-write alternative写到direct destination。只有前一typed output才能证明预算接续，continuation不授权自动跨route/lane或自动reconcile
