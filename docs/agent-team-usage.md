...
# campaign resume：成功Apply将本段按sequence写入immutable hash-chain checkpoint；新会话只执行strict `state=ready`的`resumeDriverRequest.command`，旧`continuation.whatIfCommand`不绑定artifact，仅保留历史兼容/诊断。resume preview派生remaining budget并重验route/lane/current request/nested hash，返回exact applyCommand；Apply先写durable one-shot claim再执行，successor以resumeSourceSha256绑定前驱。tamper/gap/stale/terminal/status-unavailable/consumed均不恢复预算。external reviewer把一个observation alternative的真实flags加入resume preview；managed result仍从独立case-local source进入。checkpoint不授权自动跨界、heavy action或authority/confirmed
