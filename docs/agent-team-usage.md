...
/rekit status -Format text    # 显式 status text；显示 packSource、kit/case status、manifest、shim readiness、pending-gate review/WhatIf/request-decision handoff、authorized-gate execution report contract handoff、open decision source/list + note WhatIf/hash-bound recordCommand handoff、open intervention reconcile WhatIf/Apply handoff、execution evidence review summary、latest-batch validation/remote handoff 与 bounded repair/status next-step handoff
...
/rekit overview -Format text # 只读显示 case Mission Commander action queue、lane/executor/evidence review summary/evidence/section handoff
...
/rekit promote -PacketPath <candidate-review-packet> -CandidateDecisionPath <decisions.json> -WhatIf -Format json # 严格绑定 packet/candidate/target/evidence hashes，预览 reviewed accept/reject/superseded actions，不写入；支持 managed accept 与 tooling reject/superseded 混合 decision，以及无 managed index 的 tooling-only cleanup
/rekit promote -PacketPath <candidate-review-packet> -CandidateDecisionPath <decisions.json> -Apply -Format json # 主 Agent复核同一 WhatIf 后显式执行 managed-doc merge 或 candidate cleanup；accepted receipt返回source-case-local verification workspace与provisioning命令，tooling-only reject/superseded无需reconsume并直接闭环
...
/rekit promote -PacketPath <candidate-review-packet> -CandidateDecisionPath <decisions.json> -VerifyCandidateDecision -FreshCaseRoot <workspace/fresh> -AttachedCaseRoot <workspace/attached> -WhatIf -Format json # provisioning后执行accepted candidate的pack/fresh/attached doctor与cleanup/hash binding只读验证；三个case必须彼此不同
/rekit promote -PacketPath <candidate-review-packet> -CandidateDecisionPath <decisions.json> -VerifyCandidateDecision -FreshCaseRoot <fresh-case> -AttachedCaseRoot <attached-case> -Apply -Format json # 复核同一 WhatIf 后只写 repo-local verification proof；幂等 replay，返回canonical verification workspace retirement preview command
...
/rekit promote -PacketPath <candidate-review-packet> -CandidateDecisionPath <decisions.json> -RetireCandidateVerificationWorkspace -ExpectedRetirementSha256 <hash> -Apply -Format json # 复核WhatIf hash后写repo-local durable retirement intent，删除exact provisioned trees并写receipt；crash resume与exact replay幂等
...
/rekit plan-subagents -PacketPath <canonical-case-review-packet> -StageReviewerResult -ShardId <shard> -ReviewerResultSourcePath <case-local-json> -Lane <lane> -Actor <actor> -WhatIf -Format json # 主Agent先把read-only reviewer唯一JSON保存到任意symlink-free case-local regular source；strict验证packet/route/shard/items/evidence并返回source SHA-256，不写candidate
/rekit plan-subagents -PacketPath <canonical-case-review-packet> -StageReviewerResult -ShardId <shard> -ReviewerResultSourcePath <case-local-json> -Lane <lane> -Actor <actor> -ExpectedSourceSha256 <hash> -Apply -Format json # 锁内重读并no-overwrite发布到packet-derived candidate；source drift/different candidate fail-closed，exact replay幂等
...
- 继续完善 `plan-subagents` 的 tactical reviewer dispatch、多 shard intake 与跨会话接手收敛；当前已支持 planning review artifacts、planning compact orchestration summary，以及单个 contract-compliant reviewer result 的显式 WhatIf/Apply verification + main-decision writeback，后续重点是 bounded orchestration，而不是重复实现单 reviewer intake 或恢复手动 note 落账。
