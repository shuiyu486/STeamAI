...
- `status` 与 project/lane `handoff` 会额外输出 `dailyMissionControlRunbook`：同源汇总当前 `currentDriverRequest`、status refresh command、handoff preview/apply command，以及 `inspect-status → consume-current-driver-request → refresh-after-driver → preview-handoff → write-handoff-for-takeover` 的日常主 Agent/harness 接手节奏；该 runbook 仍是只读 handoff，不 spawn session、不执行命令、不写 authority/confirmed。
