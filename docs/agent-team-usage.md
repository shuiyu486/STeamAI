...
/rekit                       # 无子命令时默认只读 status；attached case 中可省略 -Pack 并使用 metadata templatePack；kit-mode 会先显示 latest-batch project current action、focus routing explanation 与 project runbook；pack-memory focus 会先显示 evidence shortlist，再回到 compact Mission Commander first-screen strip / queue current action 接手，空 mission 会先投影 onboarding current action
/rekit status -Format text    # 显式 status text；显示 packSource、kit/case status、manifest、shim readiness、Mission Commander queue/current action、first-screen focus routing、pack-memory focus evidence shortlist、latest-batch project current action/project runbook、pending-gate review/WhatIf/request-decision handoff、authorized-gate execution report contract handoff、open decision source/list + note WhatIf/hash-bound recordCommand handoff、open intervention reconcile WhatIf/Apply handoff、execution evidence review summary、latest-batch validation/remote handoff 与 bounded repair/status next-step handoff；active reviewer/evidence/adapter review blocker 会优先于普通 handoff/continue follow-up
...
/rekit release-run -Format text   # 执行本机 release minimum 并汇总每步 exit code / duration / output tail；不写 repo/case state，不替代远程 CI green
...
/rekit plan-subagents -PacketPath <canonical-case-review-packet> -CaptureReviewerResultSource -ShardId <shard> -ReviewerResultInputPath <case-local-reviewer-json-input> -Lane <lane> -Actor <actor> -WhatIf -Format json # 主Agent把read-only reviewer唯一JSON先放在symlink-free case-local input；strict验证packet/route/shard/items/evidence并返回input SHA-256，不写packet-derived source
/rekit plan-subagents -PacketPath <canonical-case-review-packet> -CaptureReviewerResultSource -ShardId <shard> -ReviewerResultInputPath <case-local-reviewer-json-input> -Lane <lane> -Actor <actor> -ExpectedReviewerResultInputSha256 <hash> -Apply -Format json # 锁内重读input与packet，no-overwrite发布exact bytes到packet-derived reviewerStagingCommands.sourcePath；input drift/different source fail-closed，exact replay幂等
