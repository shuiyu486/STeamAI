...
/rekit release-run -Format text   # 执行 Windows 本机 release minimum，汇总每步 exit code / duration / output tail，并输出只读 release handoff（git clean/sync、一次 implementation push cadence、remote async/non-blocking boundary）；不写 repo/case state，不联网读取或等待远程 CI，不替代远程 CI green
...
`run-current-loop`在同一固定route/lane内连续执行deterministic步骤；遇到durable member handoff或reviewer handoff时停止并发布strict checkpoint，不跨边界自动管理session。刷新status后消费`missionControlRunbook.currentLoopOperator.externalMemberHandoff`或`externalReviewerHandoff`：每个非direct-write alternative同时返回绑定exact checkpoint与member/reviewer attempt的`observationEnvelopeTemplate`和统一`observationPathCommand`。外部harness可把一次accepted/returned/failed observation写入canonical `.rekit/external-session-observations/inbox/*.json`；fresh status通过anchored no-follow/reparse-safe枚举，只在恰好一个strict候选匹配latest ready checkpoint时把`observationInbox.selectedDriverRequest`提升为当前可执行WhatIf。多个匹配、任一invalid entry或namespace异常都会fail-closed；陈旧checkpoint候选仅计数、不被选择。也可显式用任意case-local、symlink-free、≤64 KiB strict JSON的`-CurrentLoopObservationPath`做WhatIf；preview返回exact envelope SHA及只携带path、expected observation SHA、checkpoint SHA和current-loop hash的Apply命令，文件bytes、checkpoint、attempt、state或capability漂移均fail-closed，envelope不得与legacy observation flags或`-Actor`混用。成功Apply发布successor checkpoint并返回`observationReceipt`；fresh status从successor的source/path/SHA lineage恢复`observationInbox.latestReceipt`，原文件不被删除，replay由source checkpoint one-shot claim阻断。member accepted后contract只保留returned/failed；returned前必须写strict manifest和全部declared bounded outputs，进入intake-ready后旧member handoff会从status移除。显式handoff Markdown、quickstart与replacement executor takeover消费同一typed package；Go runtime不spawn、poll或stop外部session，不执行heavy tool、不写authority/confirmed。
...
- 已完成lane或terminal mission需要纠正时，只使用`/rekit reopen <name>`的WhatIf→expected-hash Apply；terminal feature会与已失效的main aggregate completion作为显式compound targets共同复开。历史completion intent/receipt与events不删除，final operation commit前handoff和ordinary mutation拒绝半完成状态；复开清空旧executor并递增generation，不恢复旧session或current-loop budget。
