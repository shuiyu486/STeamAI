...
- `note -List` 与 note append 去重 / lane guard、gate lane guard 的 facts JSONL、fact-file mapping、board 读取和 known-lane 诊断应复用同一 Go case snapshot helper，避免 note、overview、handoff、start/gate 对 ledger 文件名或 lane guard 维护并行实现。
