...
/rekit plan-subagents -PacketPath <canonical-case-review-packet> -RecoverReviewerResult -ShardId <shard> -Lane <lane> -Actor <actor> -Reason <reason> -WhatIf -Format json # candidate正确但canonical result含不同regular bytes或被empty regular file占据时，复核exact hash或typed object fingerprint与quarantine路径；symlink/directory/其它non-regular object仅typed识别并fail-closed，不覆盖、不写facts
/rekit plan-subagents -PacketPath <canonical-case-review-packet> -RecoverReviewerResult -ShardId <shard> -Lane <lane> -Actor <actor> -Reason <reason> -ExpectedCandidateSha256 <hash> -ExpectedReviewerResultSha256 <hash> -Apply -Format json # 显式隔离冲突bytes或Windows empty-file本体并写strict intent/receipt；empty-file路径以source handle attributes、durable intent namespace guard和destination parent handle执行no-replace move；symlink/directory/其它non-regular与非Windows平台不提升runnable recovery，中断后可确定性finalize，恢复后仍需collection WhatIf→Apply
