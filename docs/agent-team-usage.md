...
- `note -List` 与 note append 去重 / lane guard、gate lane guard、workstream board snapshot、shared facts 写入/读取路径、handoff/continue facts snapshot、gate duplicate eventId 与 workstream lane/workspace JSONL 读取的 facts JSONL、fact-file mapping、board 读取、open-lane 过滤和 known-lane 诊断应复用同一 Go case snapshot helper，避免 note、overview、handoff、start/continue/gate 对 ledger 文件名、board JSON、JSONL/facts reader 或 lane guard 维护并行实现。
