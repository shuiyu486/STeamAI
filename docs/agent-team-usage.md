...
/rekit                       # 无子命令时默认只读 status；attached case 中可省略 -Pack 并使用 metadata templatePack；kit-mode 会先显示 latest-batch project current action、focus routing explanation 与 project runbook，再回到 compact Mission Commander first-screen strip / queue current action 接手，空 mission 会先投影 onboarding current action
/rekit status -Format text    # 显式 status text；显示 packSource、kit/case status、manifest、shim readiness、Mission Commander queue/current action、first-screen focus routing、latest-batch project current action/project runbook、pending-gate review/WhatIf/request-decision handoff、authorized-gate execution report contract handoff、open decision source/list + note WhatIf/hash-bound recordCommand handoff、open intervention reconcile WhatIf/Apply handoff、execution evidence review summary、latest-batch validation/remote handoff 与 bounded repair/status next-step handoff；active reviewer/evidence/adapter review blocker 会优先于普通 handoff/continue follow-up
