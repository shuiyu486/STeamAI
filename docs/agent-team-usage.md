...
/rekit plan-subagents -PacketPath <canonical-case-review-packet> -CaptureReviewerResultSource -ShardId <shard> -ReviewerResultInputPath <reviewerStagingCommands.sourceCaptureInput> -Lane <lane> -Actor <actor> -WhatIf -Format json # 主Agent把read-only reviewer唯一JSON先保存到packet/status/handoff投影的deterministic input path（canonical `.rekit/reviews/<review-root>/results/inputs/<shard>.reviewer-input.json`）；strict验证packet/route/shard/items/evidence并返回input SHA-256，不写packet-derived source
