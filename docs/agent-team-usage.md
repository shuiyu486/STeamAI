...
`run-current-loop`在同一固定route/lane内连续执行deterministic步骤；遇到durable member handoff或reviewer handoff时停止并发布strict checkpoint，不跨边界自动管理session。刷新status后消费`missionControlRunbook.currentLoopOperator.externalMemberHandoff`或`externalReviewerHandoff`：每个非direct-write alternative同时返回绑定exact checkpoint与member/reviewer attempt的`observationEnvelopeTemplate`和统一`observationPathCommand`。外部harness把一次accepted/returned/failed observation写入case-local、symlink-free、≤64 KiB strict JSON，再用`-CurrentLoopObservationPath`做WhatIf；preview返回exact envelope SHA及只携带path、expected observation SHA、checkpoint SHA和current-loop hash的Apply命令，文件bytes、checkpoint、attempt、state或capability漂移均fail-closed，envelope不得与legacy observation flags或`-Actor`混用。member accepted后contract只保留returned/failed；returned前必须写strict manifest和全部declared bounded outputs，进入intake-ready后旧member handoff会从status移除。显式handoff Markdown与replacement executor takeover消费同一package；Go runtime不spawn、poll或stop外部session，不执行heavy tool、不写authority/confirmed。
