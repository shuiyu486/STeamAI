...
状态：已完成 adapter report drift repair continuation closure 的 runtime/test/doc 工作树实现、focused adapter product-path regression 与完整本机 release-run；implementation commit/push 与 push-triggered remote release-gate inspection 待最终执行。Batch 653-655 已证明 authorized output workspace 可以从 execution report contract scaffold → draft → validate → hash-bound record，Batch 664 已证明 evidence acknowledgement 后 recorded adapter handoff 会变成 provenance-only；本批补齐 canonical adapter report 在 record 前 drift/malformed 或 validation fail-closed 后的 replacement executor first-screen continuation：invalid report 是 unrecorded/non-authoritative，下一步是修复 bounded sidecar、重新运行 read-only validation，成功后再使用 validation/status 返回的 exact `-ExpectedExecutionReportSha256` record。
