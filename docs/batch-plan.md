...
边界：只增强 `plan-subagents -ReviewerResultPath ...` reviewer intake JSON/text 的只读 terminal summary 与 CLI product-path coverage；不自动 spawn/monitor reviewer、不执行 heavy-tool、不写 authority/confirmed，不改变 verification-before-decision writeback 顺序、duplicate retry/idempotency、blocked fail-closed、postValidation full overview/handoff/doctor snapshots、sync/promote review-first、case durable schema、PowerShell façade 或公共 façade removal 门禁。

已完成内容：

- `ReviewerIntakeSummary` 与 `ReviewerPostValidationSummary` 新增 `reviewerWritebackSummary`，复用 `workstream.ReviewerWritebackSummaryFor(...)`，并由 top-level intake summary 继承 postValidation summary 中的 compact provenance。
- CLI reviewer intake text 在 `reviewer intake summary post-validation reviewer writeback summary...` 与 `reviewer intake post-validation summary reviewer writeback summary...` 行直接输出 verification/decision counts、latest reviewer result、packet/route/shard/session、owner binding、risk/conflict/route output/evidence refs 与 boundary。
- `TestRunPlanSubagentsReviewerIntakeWhatIfApplyE2E` 覆盖 Apply JSON top-level summary、postValidation summary 与 already-complete text 的 compact reviewer writeback summary，锁定 latest reviewer result path 与 terminal first-screen visibility。
- 文档同步更新 `README.md`、`docs/release-readiness.md`、`rekit/tests/README.md`、`CHANGELOG.md`，并将 Batch 529 归档到 `docs/batch-history.md`。

验证结果：已通过 `gofmt -w internal/rekit/subagents/intake.go internal/rekit/cli/cli.go internal/rekit/cli/reviewer_intake_test.go`、focused `go test ./internal/rekit/cli -run TestRunPlanSubagentsReviewerIntakeWhatIfApplyE2E -count=1`、combined focused `go test ./internal/rekit/subagents ./internal/rekit/cli -run "TestRunPlanSubagentsReviewerIntakeWhatIfApplyE2E|TestRunPlanSubagentsReviewerOrchestrationE2E|TestRunPlanSubagentsReviewerIntakeCaseLocalProductPathUsesMetadataRuntime" -count=1`、package `go test ./internal/rekit/cli -count=1`、package `go test ./internal/rekit/subagents -count=1` 与 `git diff --check`（仅 LF/CRLF conversion warnings，无 whitespace error）。完整本地 release minimum、implementation commit/push 与远程 release-gate inspection 待执行并记录。

上一批摘要：Batch 529 已完成 authorized-gate start/reconcile takeover compact handoff closure，并归档到 `docs/batch-history.md`。
...
2. **Reviewer orchestration E2E residuals（如仍有缺口）**：Batch 489/499/505/514-516/523-525/530 已覆盖 reviewer intake product path、reviewer dispatch/intake downstream/durable/progress handoff、reviewer writeback identity、reviewer result provenance summary，以及 reviewer intake terminal summary/postValidation summary 的 compact reviewer writeback provenance；后续仅在 multi-reviewer 接续仍要求解析 nested JSON、打开 reviewer result artifact 或手工拼 route output 才能接续时推进，不自动 spawn reviewer、不执行 heavy-tool。
