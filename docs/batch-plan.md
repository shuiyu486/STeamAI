...
状态：已完成 pack-memory downstream proof follow-up closure 的 runtime/test 实现，release-run 尚待执行；candidate decision Apply 后如果 verification/provisioning current action 与 cleanup/reconsume proof 缺失同时存在，`status` / `release-check` 现在会保留 verification current action，同时把缺失的 cleanup/reconsume proof draft 暴露为可复制的 Mission Commander follow-up action，避免 replacement executor 在 verification 队列和 proof summary 之间手工拼接 downstream closure。
