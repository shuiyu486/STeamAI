...
当前路线是 `steamai-self-contained-project-v1`，Batch 828 已完成 Windows 本机产品闭环：默认产品从中央 kit + case thin shim 迁移为一个真实项目目录内自带 `/steamai`、`.steamai`、verified runtime 和 selected pack 的 STeamAI 项目，并同时关闭 compact status、人话 recovery、bounded autonomy 和 legacy migration。当前没有已批准下一批；commit/push 未获授权，release cadence 保持 `implementation-pending`。
...
- 当前完成只表示 Windows 本机产品闭环与 local validation；未授权 commit/push 时 machine receipt 不能变成 post-push receipt，也不解锁下一批。
