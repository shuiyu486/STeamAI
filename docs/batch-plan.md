...
状态：已完成 pack-memory multi-receipt current-action closure 的 runtime/test/doc 实现、focused 本机验证与完整本机 release-run；本批修复同一 pack 存在多个 candidate decision verification receipts 时的 Mission Commander current action 选择，避免较早的 provisioning-required receipt 遮蔽更靠后的 verification/retirement closure。完整本机 `go run ./cmd/rekit -- -Command release-run -Format text` 已通过，返回 `ready=true` / `summary=release run ok`，聚合 `passed=7 failed=0 skipped=0`，覆盖 `release-check`、`status`、`packs`、`doctor`、`go test ./...`、`go vet ./...` 与 `git diff --check`；implementation push 与 push-triggered remote release-gate inspection 将在提交后回填。
