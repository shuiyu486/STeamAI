...
- 将 fallback deny baseline generic 化，pack-specific deny patterns 与 heavy-tool gate action 仅由 manifest 声明；Batch 151 已移除 Go manifest 对 `promoteDenyPatterns` 的隐式 fallback，schema / pack authoring 文档要求 pack 显式声明 deny baseline；Batch 152 已移除 Go manifest 对 `promoteFiles` 的 `managedFiles` 隐式 fallback，要求 pack 显式声明允许回流的 managed 文件子集；Batch 153 已移除 Go manifest 对 `budgets.defaultMarkdown` 的隐式注入，要求 pack 显式声明默认 Markdown/text 预算。
...
- runtime 不含领域业务逻辑；default pack 决策点已在 Batch 150 收口到 Go `defaults.DefaultPack`，Batch 151/152/153 已取消 Go manifest 的 deny baseline、promoteFiles 与 defaultMarkdown fallback，gate action、default risk、默认 stop conditions、promote deny baseline、回流文件范围与默认文档预算来自 pack manifest，而不是 Go runtime 硬编码。
