...
当前进度：Batch 104 已完成低风险 read-only default façade：`status`、`packs`、kit/case `doctor/validate` 默认委托 Go；Batch 113 将已初始化 case 的 `overview -Format json` 与 `note -List -Format json` 只读查询也纳入默认 Go façade；Batch 114 将 `gate -WhatIf` 非写入 heavy-tool gate preview纳入默认 Go façade；Batch 115 将 `gate -Apply` pending-gate request ledger 写入纳入默认 Go façade；Batch 117 将 attached case 的 `start`/`handoff` JSON preview 与显式 `-Apply` 纳入默认 Go façade；Batch 118 将 attached case 的 `continue -WhatIf -Format json` 非写入 preview 纳入默认 Go façade；Batch 119 将 attached case 的 `overview` 文本/JSON 与缺 board 初始化纳入默认 Go façade；Batch 120 将 attached case 的 `note -List` 文本/table/tsv 查询也纳入默认 Go façade；Batch 121 将 explicit `continue -Apply` 的 case-local facts/routing/run digest/resume/board 写入纳入默认 Go façade；Batch 224 将 `status`、`packs`、`doctor` 与 `validate` 的 PowerShell fallback 退休，和 `release-check` 一起进入 no-fallback Go-owned baseline；Batch 225 退休 `plan-subagents` fallback，Batch 226 退休 `gate` fallback，Batch 227 退休 `overview` 与 `note` fallback，Batch 228 退休 `sync` 与 `update` fallback，Batch 230 退休 case lifecycle `attach` / `repair` / `init` / `bootstrap` fallback；`REKIT_GO_DISABLE=1` 不再让这些 Go-owned no-fallback 命令回落到 PowerShell 业务实现；Batch 232 后 `start`、`handoff` 与 `continue` 的文本/default 工作线 flow 也进入 Go-owned no-fallback baseline，fallback retirement candidateCommands 清零；Batch 233 进一步把 `rekit.ps1` legacy runtime module dot-source 移到 Go delegation 与 no-fallback guard 之后，使 default delegation / disabled no-fallback 路径不预加载 retired PowerShell modules；Batch 234 删除已不可达的 legacy dot-source block 与 command switch fallback dispatcher，使公共 façade 只保留 Go delegation、no-fallback guard 和 retired dispatcher error；Batch 305 将 Go-native public surface counts 收敛为共享 summary，减少 release handoff、CLI text 与 tests 对 commands/handlers/symbols/profiles/boundaries/policies/prerequisites count 的重复 plumbing；Batch 306 将 case shim 与 public default docs readiness counts 收敛为共享 summary，减少 release handoff、CLI text 与 tests 对 required/canonical/forbidden/document/boundary counts 的重复 plumbing；Batch 307 将 CI release gate counts 收敛为共享 summary，减少 release handoff、CLI text 与 tests 对 workflow/job/command/forbidden counts 的重复 plumbing；Batch 308 将 release-check 顶层 inventory counts 收敛为共享 summary，减少 release handoff、CLI text 与 tests 对 recommended/required/document/pack/gate-action/boundary/known-gap/warning counts 的重复 plumbing；Batch 309 将 PowerShell deprecation inventory counts 收敛为共享 summary，减少 release handoff、CLI text 与 tests 对 command/module/freeze/fallback/façade/module-removal/module-reference/warning counts 的重复 plumbing；Batch 310 将 release handoff counts 与 pack maturity counts 收敛为共享 summary，减少 release handoff readiness、CLI text 与 tests 对 readFirst/signals/knownGaps/validation/nextActions/warnings/pack-maturity counts 的重复 plumbing；Batch 311 将 Go-native public surface group/policy nested counts 收敛为共享 summary，减少 readiness、releasecheck tests 与 CLI JSON assertions 对 command profile group、boundary group 与 policy violation command counts 的重复 plumbing；Batch 312 将 public façade removal top-level counts 收敛为共享 summary，减少 readiness、handoff details、CLI text 与 tests 对 prerequisites/warnings/removal-plan/deletion-gate/execution-step/impact counts 的重复 plumbing；Batch 313 将 Go-native public surface coverage/warning counts 收敛进共享 summary，减少 readiness、CLI warning guard、releasecheck tests 与 CLI JSON assertions 对 commands/handlers/symbols/profiles/mutation-boundaries/warnings counts 的重复 plumbing；Batch 314 将 PowerShell deprecation fallback/façade/module-removal/module-reference nested counts 收敛进共享 summary，减少子 inventory readiness、public façade removal prerequisites、releasecheck tests 与 CLI JSON assertions 对 warnings 与 coverage counts 的重复 plumbing；Batch 315 将 public façade removal plan/impact row counts 收敛进共享 summary，减少 aggregate counts、releasecheck tests 与 CLI JSON assertions 对 replacement/deletion/execution/boundary/recovery/documentation/impact row counts 的重复 plumbing；Batch 316 将 public façade removal plan/impact validation guards 收敛到 per-row count helpers，减少 replacement/deletion/execution/boundary/recovery/documentation/impact validation path 中 raw `len(...)` guard plumbing；Batch 317 将 public façade removal validation command coverage guards 收敛到 shared missing-command helper，减少 plan/impact required validation command contains plumbing；Batch 318 将 releasecheck package tests 与 CLI JSON assertions 的 public façade removal validation command smoke 收敛到本地 helper，减少重复 validation command count 与 required command contains plumbing；Batch 319 将 public façade removal plan/impact warnings counts 收敛进共享 summary，减少 readiness guards、releasecheck tests 与 CLI JSON assertions 对 warning length 的重复 plumbing；Batch 320 将 public façade removal plan/impact aggregate validation guards 收敛到已有 count summaries，减少 replacement/deletion/execution/boundary/impact aggregate raw `len(...)` plumbing；Batch 321 将 Go-native public surface façade removal prerequisite row/not-ready counts 收敛进共享 summary，减少 readiness helper、public façade removal prerequisite summary、releasecheck tests 与 CLI JSON assertions 对 prerequisite length/readiness 的重复 plumbing；Batch 322 将 Go-native public surface boundary row/command counts 收敛进共享 summary，减少 mutation-boundary readiness、boundary row validation、releasecheck tests 与 CLI JSON assertions 对 boundary row length/count 的重复 plumbing；Batch 323 将 Go-native public surface profile summary counts 收敛进共享 summary，减少 profile summary readiness guards、group/summary consistency guards、boundary summary checks、releasecheck tests 与 CLI JSON assertions 对 `CommandProfileSummary` fields / boundary map 的重复 plumbing；Batch 324 将 Go-native public surface handler/symbol/profile command coverage counts 与 drift 收敛进共享 summary，减少 coverage guards、façade removal prerequisite readiness、releasecheck tests 与 CLI JSON assertions 对 handler/symbol/profile missing/unknown 的重复 plumbing；Batch 325 将 Go-native public surface catalog empty/duplicate counts 与 mutation boundary inventory counts 收敛进共享 summary，减少 catalog guards、mutation-boundary readiness、releasecheck tests 与 CLI JSON assertions 对 raw catalog length/duplicate scan 和 mutation-boundary length/unknown 的重复 plumbing；Batch 326 将 Go-native public surface profile catalog rows、empty/duplicate、unknown-boundary、heavy-tool、authority/confirmed 与 review/apply policy-adjacent counts 收敛进共享 summary，减少 profile catalog guards、façade removal prerequisite readiness、releasecheck tests 与 CLI JSON assertions 对 raw profile scan 的重复 plumbing。Stage 2 的核心完成信号已达成，后续转向 removal-candidate PowerShell module 独立删除/归档批次、public Go-native entrypoint 或 blocked migration 设计，而不是继续按已清空的 `candidateCommands[]` 寻找普通 fallback。
