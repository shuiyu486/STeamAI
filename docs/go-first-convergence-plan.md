...
- 继续扩展更多非 `_template` / 非 `vmp-re` pack 的真实 dry-run 脚本或中型闭环，尤其是选择至少一个非 RE-only pack 暴露领域 pack 差异。
...
- Batch 125 已先选 `generic-binary-re` 作为 pack-neutral Go package E2E 试点；后续可继续选 `web-security` 或另一个非 RE-only pack 补足最小真实使用文档和 dry-run 工作线。
