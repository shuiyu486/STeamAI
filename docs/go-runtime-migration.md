...
| Façade Go start/handoff preview/apply/text | retired legacy façade start/handoff compatibility path | Batch 117 起 attached case 的 start/handoff JSON preview 与显式 apply 默认委托 Go；Batch 231 起这些 Go-owned structured invocation 在 `REKIT_GO_DISABLE=1` 或 Go delegation 不可用时直接 no-fallback；Batch 232 起文本 preview 与 bare/default 工作线 flow 也经 façade 委托 Go text output，direct Go CLI 继续保持默认 JSON contract；Batch 234 起 `rekit.ps1` 不再加载 legacy modules 或保留 command switch fallback dispatcher；不写 authority/confirmed、不执行 heavy-tool。 |
| Façade Go continue preview/apply/text | retired legacy façade continue compatibility path | Batch 118 起 JSON preview 默认委托 Go，输出非写入机器可读预览；Batch 121 起 explicit apply 默认委托 Go，写 case-local facts/routing/run digest/lane resume/checkpoint/board；Batch 232 起文本/default preview 也经 façade 委托 Go text output，`REKIT_GO_DISABLE=1` 或 Go delegation 不可用时直接 no-fallback；Batch 234 起 `rekit.ps1` 不再加载 legacy modules 或保留 command switch fallback dispatcher；不写 authority/confirmed、不执行 heavy-tool。 |
