...
- G2.3 已完成 Go gate request/evidence ledger 写入与 adapter contract projection：`gate -Apply` 默认 append pending-gate / authorized-gate request JSONL，要求 `-Actor`，不执行 heavy-tool；`gate -ExecutionReportContract -GateEventId ... -Format json` 只读投影 authorized-gate 的 adapter execution report contract，供 lane executor/tool adapter 执行前消费 action、budget、output paths、status、stop conditions 与 sidecar 规则；`gate -Apply -GateEventId ... -ExecutionStatus ... -ExecutionReportPath ...` 可在 authorized-gate 后记录 observation execution evidence（actual budget、output refs、evidence refs、boundary hits、escalation）并 strict intake bounded adapter execution report；authorized-gate 会进入 Mission brief、overview、handoff 与 continue digest/status，但不阻塞 lane。
