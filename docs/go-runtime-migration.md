...
- G2.3 已完成 Go gate request/evidence ledger 写入：`gate -Apply` 默认 append pending-gate / authorized-gate request JSONL，要求 `-Actor`，不执行 heavy-tool；`gate -Apply -GateEventId ... -ExecutionStatus ...` 可在 authorized-gate 后记录 observation execution evidence（actual budget、output refs、evidence refs、boundary hits、escalation）；authorized-gate 会进入 Mission brief、overview、handoff 与 continue digest/status，但不阻塞 lane。
