...
- 在 PowerShell façade 默认委托前，继续用手动 Go CLI smoke 验证；写入命令、工作线文本 workflow、内部命令、note append、authority/confirmed 更新和 schema 迁移仍需单独确认；当前 façade 只允许 overview/note list JSON 只读输出与工作线 `-WhatIf -Format json` 非写入预览委托。
