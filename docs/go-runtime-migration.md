...
5. `promote -CreateCandidates`（G3.6 已完成迁移预研与 preflight smoke；仍未实现 Go 写入，见 `docs/promote-candidates-migration.md`）；
...
不委托：`attach`、`repair`、`gate -Apply`、`sync -Apply`、`init/bootstrap -Apply`、`promote -Apply/-CreateCandidates`、工作线命令、ledger `note`。其中 `attach`、`repair`、`sync -Apply`、`init/bootstrap -WhatIf/-Apply` 可手动运行 Go CLI 验证；公共 `/rekit` 入口继续由 PowerShell 处理写入命令。
...
| Promote candidates preflight | `.\rekit\tests\promote-candidates-preflight-smoke.ps1` | 验证 PowerShell `promote -WhatIf -CreateCandidates` baseline、Go promote review artifact/sanitized preview parity、Go 写入 guard 与 façade fallback；不写 pack candidates。 |
...
- G3.6 已完成 `promote -CreateCandidates` 迁移预研与 preflight smoke：见 `docs/promote-candidates-migration.md`；Go backend 仍拒绝该写入 flag，下一步若继续推进，应先实现手动 candidate helper 或补更强 promote parity，不要直接迁移 `promote -Apply` 或 authority/confirmed 写入。
