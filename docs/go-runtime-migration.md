...
| Pack smoke matrix | `.\rekit\tests\pack-smoke-matrix.ps1` / `-Packs web-security,generic-binary-re` / `-Format json` | 显式运行安全领域 skeleton pack smoke 全量或子集，文本模式逐 pack 输出 running/passed/failed、elapsedMs 和原始 smoke 输出；JSON 模式输出 `pack-smoke-matrix` envelope 与 results[]，便于自动化消费；只编排现有自包含 smoke，不改变 runtime 或 pack 行为。 |
