...
- `/rekit` 是公共 ABI，Go backend 已成为全部 20 个 public commands 的 deterministic owner；retained `rekit.ps1` 只做参数兼容、Go delegation、no-fallback guard 与错误透传。Batch 104–232 逐步完成 read-only、case lifecycle、sync/promote、ledger/gate/workstream 和 `plan-subagents` 默认委托与 fallback retirement，Batch 234 删除 legacy dispatcher，Batch 240 删除 `rekit/lib/*.ps1`；`REKIT_GO_DISABLE=1` 现在只触发 no-fallback guard。Batch 122/123/124 已用 `_template` pack Go CLI package E2E 覆盖 workstream、gate/dispatch 与 reviewer/decision 闭环，Batch 125/126 又覆盖 `generic-binary-re` 与 `web-security` pack-neutral flow。
