...
- G3.5 已完成 `init/bootstrap -WhatIf/-Apply` Go CLI 手动路径：见 `docs/init-bootstrap-migration.md`；下一步若继续推进，应优先评估 `promote -CreateCandidates` 预研或补更强 apply parity，不要直接迁移 authority/confirmed 写入。
