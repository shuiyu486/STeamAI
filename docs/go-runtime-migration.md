...
- G3.6/G3.7 已完成 `promote -CreateCandidates` 迁移预研、preflight smoke 与 Go CLI 手动写入路径：见 `docs/promote-candidates-migration.md`；公共 PowerShell façade 仍不委托该写入命令，下一步若继续推进不要直接迁移 `promote -Apply` 或 authority/confirmed 写入。
