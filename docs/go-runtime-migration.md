...
| Façade Go start/handoff preview/apply | structured invocation no-fallback; legacy text flow compatibility path | Batch 117 起 attached case 的 start/handoff JSON preview 与显式 apply 默认委托 Go；Batch 231 起这些 Go-owned structured invocation 在 `REKIT_GO_DISABLE=1` 或 Go delegation 不可用时直接 no-fallback；文本 preview 和无 `-Apply` 的文本工作线 workflow 仍回退 PowerShell；不写 authority/confirmed、不执行 heavy-tool。 |
| Façade Go continue JSON preview/apply | structured invocation no-fallback; legacy text flow compatibility path | Batch 118 起 JSON preview 默认委托 Go，输出非写入机器可读预览；Batch 121 起 explicit apply 默认委托 Go，写 case-local facts/routing/run digest/lane resume/checkpoint/board；Batch 231 起这些 Go-owned structured invocation 在 `REKIT_GO_DISABLE=1` 或 Go delegation 不可用时直接 no-fallback；文本预览和无 `-Apply` 的工作线 flow 仍回退 PowerShell；不写 authority/confirmed、不执行 heavy-tool。 |
...
| Façade smoke | `.\rekit\tests\facade-smoke.ps1` / `-CaseRoot <case> -Pack vmp-re` | 默认创建自包含临时 case，也可显式验证长期 case；覆盖 read-only、case lifecycle、sync、promote review/preview、promote candidate 写入与 promote apply 写入默认 Go 委托、continue JSON preview/apply 默认委托、plan-subagents review artifact 默认委托、已退休 fallback no-fallback 边界、workstream structured disabled no-fallback、overview 文本/JSON、note list 文本/JSON、note append、start/handoff JSON preview/apply 委托，以及剩余文本 fallback；fake `REKIT_GO_EXE` sentinel 证明应委托组合确实经 Go façade。 |
