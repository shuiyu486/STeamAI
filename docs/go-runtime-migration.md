...
当前边界：Batch 106 起 PowerShell façade 默认委托 `/rekit sync` review、`sync -Apply` 实际写入与 `sync -Apply -WhatIf -Format json` 非写入 preview，Batch 228 起 `sync` / `update` PowerShell fallback 已退休，`REKIT_GO_DISABLE=1` 或 Go delegation 不可用时直接 no-fallback 失败；Batch 107 起默认委托 `/rekit promote` review、review artifact 写入、`promote -CreateCandidates -WhatIf -Format json` 与 `promote -Apply -WhatIf -Format json` 非写入 preview；Batch 108 起默认委托 `promote -CreateCandidates` 实际候选写入；Batch 112 起 `promote -Apply` 实际 pack source 写入默认经 façade 委托 Go；其 Go helper 的 backup、validation rows、deny 与 restore failure 边界由 `go test ./internal/rekit/promote` 覆盖；Batch 229 起 `promote` PowerShell fallback 已退休，文本 what-if、`REKIT_GO_DISABLE=1` 或 Go delegation 不可用时直接 no-fallback 失败。
...
每一步都必须有临时 case smoke、backup 检查和 review-first 边界验证。后续所有实施计划必须写入 `docs/batch-plan.md` 或本文件后再实施，避免只留在聊天上下文中；仅在当前用户 goal/session 明确授权具体仓库和分支时，才随代码提交并推送。
...
2. `start`（G4.2 已完成 Go CLI 手动路径：`-WhatIf` 非写入预览，`-Apply` 显式创建/进入 feature lane 并初始化 board/facts/policy/default authority lane；Batch 117 起 attached case 的 JSON preview 与显式 apply 默认经 façade 委托 Go，Batch 232 起文本 preview 与无 `-Apply` 文本 flow 也由 façade 明确请求 Go text output，不再回退 PowerShell；Batch 234 后公共 façade 不再保留 legacy command switch fallback dispatcher）；
3. `handoff`（G4.3 已完成 Go CLI 手动路径：`-WhatIf` 非写入预览，`-Apply` 显式写项目级/工作线级 handoff、展示最近 verification/decision 并刷新 lane resume/checkpoint；Batch 117 起 attached case 的 JSON preview 与显式 apply 默认经 façade 委托 Go，Batch 232 起文本 preview 与无 `-Apply` 文本 flow 也由 façade 明确请求 Go text output，不再回退 PowerShell；Batch 234 后公共 façade 不再保留 legacy command switch fallback dispatcher）；
...
G2.4 增加显式环境变量开关；Batch 104 起，低风险 read-only 命令默认委托 Go；Batch 105 起 case lifecycle 预览/显式 apply 默认委托 Go；Batch 106 起 `/rekit sync` review 与 `sync -Apply` 默认委托 Go；Batch 107 起 `/rekit promote` review 与 JSON what-if preview 默认委托 Go；Batch 108 起 `promote -CreateCandidates` 实际候选写入默认委托 Go；Batch 112 起 `promote -Apply` 实际 pack source 写入默认委托 Go；Batch 113 起 `note -List -Format json` 只读查询默认委托 Go；Batch 114 起 `gate -WhatIf` 非写入 heavy-tool gate preview 默认委托 Go；Batch 115 起 `gate -Apply` pending-gate request ledger 写入默认委托 Go；Batch 116 起 attached case 的 `note` append 与 `note -WhatIf` 默认委托 Go；Batch 117 起 attached case 的 `start`/`handoff` JSON preview 与显式 `-Apply` 默认委托 Go；Batch 118 起 attached case 的 `continue -WhatIf -Format json` 非写入 preview 默认委托 Go；Batch 119 起 attached case 的 `overview` 文本/JSON 与缺 board 初始化默认委托 Go；Batch 120 起 attached case 的 `note -List` 文本/table/tsv 只读查询默认委托 Go；Batch 121 起 attached case 的 explicit `continue -Apply` case-local 写入默认委托 Go；Batch 230 起 case lifecycle PowerShell fallback 退休：
...
- `gate -WhatIf` 非写入 heavy-action authorization preflight 与 `gate -Apply` pending-gate / authorized-gate request ledger decision 写入；authorized-gate 在 Mission brief / overview / handoff / continue-facing artifacts 中作为非阻塞授权决策展示，并直接投影 requested budget、authorized output paths、stop conditions、gate eventId 与可复制 report contract command；
...
| Go init/bootstrap apply | `go run ./cmd/rekit -- -Command init -Target <newCase> -Pack vmp-re -Apply` | 创建 metadata / shim / legacy metadata / managed docs / managed block / template create-if-missing / sync state，返回 `isMutation=true`；Batch 105 起公共 façade 对 `init/bootstrap -WhatIf/-Apply` 默认委托 Go，Batch 230 起 `REKIT_GO_DISABLE=1` 报 no-fallback error。 |
...
| Promote candidates preflight | `.\rekit\tests\promote-candidates-preflight-smoke.ps1` | 验证 promote text what-if no-fallback、Go promote review artifact/sanitized preview parity、Go `-CreateCandidates -WhatIf` 非写入、默认 façade JSON 预览/默认 review artifact/默认 create-candidates 委托与 disabled no-fallback；不写 pack candidates。 |
...
| Promote apply preflight | `.\rekit\tests\promote-apply-preflight-smoke.ps1` | 验证 promote apply text/disabled no-fallback、backup/deny/pack-root cleanup、Go apply what-if、默认 façade JSON 预览委托与 actual apply 默认委托。 |
| Go promote apply | `go test ./internal/rekit/promote` / `.\rekit\tests\promote-apply-smoke.ps1` | Batch 112 起公共 façade 默认委托 Go 写 pack managed docs，验证 what-if 非写入、backup、blocked deny、validation rows、validation failure restore、pack-root containment、tooling 不写入、cleanup 与 Batch 229 起 disabled no-fallback。 |
...
| Go overview initialization / readonly | `.\rekit\tests\overview-readonly-smoke.ps1` | 手动 Go CLI 缺 board 时初始化 case-local board/facts/policy/default authority lane，JSON envelope 标记 `isMutation=true`；后续读取既有 board/facts 输出项目总览，含最近 verification/decision，并保持 snapshot 只读；Batch 119 起 PowerShell façade 默认委托 overview 文本与 JSON，Batch 227 起 `REKIT_GO_DISABLE=1` 报 no-fallback error，不再回退 PowerShell。 |
| Go start apply | `.\rekit\tests\start-apply-smoke.ps1` / `go test ./internal/rekit/cli` | Go CLI 与 retained façade `start -WhatIf -Format json` / `start -Apply` 预览或创建 feature lane，验证 board/facts/policy/lane/workspace scaffold、默认 JSON/text preview/apply Go 委托、`REKIT_GO_DISABLE=1` no-fallback 与 doctor；Batch 117 起 JSON preview/apply 默认委托 Go，Batch 232 起 text/bare flow 也委托 Go；Batch 122 起 package test 覆盖 `_template` 的最小闭环。 |
| Go handoff apply | `.\rekit\tests\handoff-apply-smoke.ps1` | Go CLI 与 retained façade `handoff -WhatIf -Format json` / `handoff -Apply` 预览或写项目级/工作线级 handoff，验证 lane resume/checkpoint、verification/decision ledger 展示、默认 JSON/text preview/apply Go 委托、`REKIT_GO_DISABLE=1` no-fallback 与 doctor；Batch 117 起 JSON preview/apply 默认委托 Go，Batch 232 起 text/bare flow 也委托 Go。 |
...
| Go continue preview/apply | `.\rekit\tests\continue-whatif-smoke.ps1` / `go test ./internal/rekit/cli` | Go CLI `continue -WhatIf` 输出非写入 preview，`continue -Apply` 写 case-local facts/routing/run digest/lane resume/checkpoint/board，并覆盖 JSON/text/default preview 与 explicit apply 的 façade Go 委托、preview no-write、`REKIT_GO_DISABLE=1` no-fallback、duplicate skipped 与 authority guard；Batch 122 起 `_template` pack package test 验证输出可被 handoff 消费；不写 authority/confirmed、不执行 heavy-tool。 |
...
| Façade Go gate dry-run/apply | retired legacy façade gate dry-run/apply compatibility path | Batch 114 起 what-if 默认委托 Go 输出非写入 gate plan；Batch 115 起 apply 默认委托 Go 写 request ledger；Batch 330 起 gate preflight 读取 durable lane autonomy profile，可在完全覆盖时记录 authorized-gate，否则保持 pending/denied fail-closed decision；Batch 226 起 PowerShell fallback 已退休，`REKIT_GO_DISABLE=1` 报 no-fallback error；两者都不执行 heavy-tool、不写 confirmed/authority。 |
| Façade Go attach/repair lifecycle | retired legacy façade attach/repair lifecycle compatibility path | Batch 105 起默认委托 Go；attach/repair 预览非写入，显式 apply 只写 binding metadata、initial state 与 thin shim；Batch 230 起 `REKIT_GO_DISABLE=1` 报 no-fallback error，不再回退 PowerShell。 |
| Façade Go init/bootstrap lifecycle | retired legacy façade init/bootstrap lifecycle compatibility path | Batch 105 起默认委托 Go；预览不创建 target，显式 apply 创建/刷新 case scaffold 与 managed content；Batch 230 起裸命令、`REKIT_GO_DISABLE=1` 或 Go delegation 不可用均报 no-fallback error。 |
| Façade Go promote candidate write / JSON preview | retired legacy façade promote candidate and JSON preview compatibility path | Batch 107 起 JSON 非写入 candidate preview 默认委托 Go；Batch 108 起实际 candidate/index/tooling candidate 写入也默认委托 Go；Batch 229 起文本 create-candidates what-if 与 `REKIT_GO_DISABLE=1` 均报 no-fallback error。 |
| Façade Go promote apply / JSON preview | retired legacy façade promote apply and JSON preview compatibility path | Batch 107 起 JSON 非写入 pack apply preview 默认委托 Go；Batch 112 起实际 pack source 写入也默认委托 Go；Batch 229 起文本 apply what-if 与 `REKIT_GO_DISABLE=1` 均报 no-fallback error。 |
| Façade Go overview text/JSON | retired legacy façade overview text/JSON compatibility path | Batch 119 起 attached case 的 overview 文本/JSON 默认委托 Go；缺 board 时初始化 case-local board/facts/policy/default authority lane，JSON envelope 标记 `isMutation=true`；后续读取保持只读；Batch 227 起 PowerShell fallback 已退休，`REKIT_GO_DISABLE=1` 报 no-fallback error。 |
| Façade Go note list / append | retired legacy façade note list/append compatibility path | Batch 120 起 `note -List` 文本/table/tsv 与 JSON 默认委托 Go 输出只读 ledger events；Batch 116 起 attached case 的 append 与 `-WhatIf` 默认委托 Go 输出 JSON envelope，并只写 facts JSONL 或预览；Batch 227 起 PowerShell fallback 已退休，`REKIT_GO_DISABLE=1` 报 no-fallback error。 |
| Façade Go start/handoff preview/apply/text | retired legacy façade start/handoff compatibility path | Batch 117 起 attached case 的 start/handoff JSON preview 与显式 apply 默认委托 Go；Batch 231 起这些 Go-owned structured invocation 在 `REKIT_GO_DISABLE=1` 或 Go delegation 不可用时直接 no-fallback；Batch 232 起文本 preview 与 bare/default 工作线 flow 也经 façade 委托 Go text output，direct Go CLI 继续保持默认 JSON contract；Batch 234 起 `rekit.ps1` 不再加载 legacy modules 或保留 command switch fallback dispatcher；Batch 235 起 release inventory 输出 `facadeRuntime=true legacyImports=false dispatcher=false`；不写 authority/confirmed、不执行 heavy-tool。 |
| Façade Go continue preview/apply/text | retired legacy façade continue compatibility path | Batch 118 起 JSON preview 默认委托 Go，输出非写入机器可读预览；Batch 121 起 explicit apply 默认委托 Go，写 case-local facts/routing/run digest/lane resume/checkpoint/board；Batch 232 起文本/default preview 也经 façade 委托 Go text output，`REKIT_GO_DISABLE=1` 或 Go delegation 不可用时直接 no-fallback；Batch 234 起 `rekit.ps1` 不再加载 legacy modules 或保留 command switch fallback dispatcher；Batch 235 起 release inventory 输出 `facadeRuntime=true legacyImports=false dispatcher=false`；不写 authority/confirmed、不执行 heavy-tool。 |
...
| Façade smoke | `.\rekit\tests\facade-smoke.ps1` / `-CaseRoot <case> -Pack vmp-re` | 默认创建自包含临时 case，也可显式验证长期 case；覆盖 read-only、case lifecycle、sync、promote review/preview、promote candidate 写入与 promote apply 写入默认 Go 委托、continue JSON preview/apply/text 默认委托、plan-subagents review artifact 默认委托、已退休 fallback no-fallback 边界、overview 文本/JSON、note list 文本/JSON、note append、start/handoff JSON preview/apply/text 委托；fake `REKIT_GO_EXE` sentinel 证明应委托组合确实经 Go façade，isolated sentinel fixture 证明 default/no-fallback 路径不加载 legacy runtime；Batch 235 起同一状态还由 `powerShellDeprecation.facadeRuntime` 机器可读库存审计。 |
...
- `/rekit` 是公共 ABI，Go backend 已成为全部 32 个 public commands 的 deterministic owner，且每个 public command 都有唯一 callback owner；retained `rekit.ps1` 只做参数兼容、Go delegation、no-fallback guard 与错误透传。`release-run` 提供只读本机 release minimum driver，`next-batch` 提供 kit review-first planning receipt。Batch 104–232 逐步完成 read-only、case lifecycle、sync/promote、ledger/gate/workstream 和 `plan-subagents` 默认委托与 fallback retirement，Batch 234 删除 legacy dispatcher，Batch 240 删除 `rekit/lib/*.ps1`；`REKIT_GO_DISABLE=1` 现在只触发 no-fallback guard。Batch 122/123/124 已用 `_template` pack Go CLI package E2E 覆盖 workstream、gate/dispatch 与 reviewer/decision 闭环，Batch 125/126 又覆盖 `generic-binary-re` 与 `web-security` pack-neutral flow。
...
- G2.2 已完成 Go gate dry-run：`gate -WhatIf` 输出非写入 gate decision plan，拒绝无 `-WhatIf` 调用，不执行 heavy-tool；durable autonomy profile 完全覆盖时可预览 `authorized-gate`。
- G2.3 已完成 Go gate request/evidence ledger 写入与 adapter contract projection：`gate -Apply` 默认 append pending-gate / authorized-gate request JSONL，要求 `-Actor`，不执行 heavy-tool；`gate -ExecutionReportContract -GateEventId ... -Format json` 只读投影 authorized-gate 的 adapter execution report contract，供 lane executor/tool adapter 执行前消费 action、budget、output paths、default report path、status、stop conditions、boundary/escalation requirements、validation failure taxonomy、`validationRepairHints[]`、`liveValidation` `authorizedWorkspaces[]` / `reportFileName` / `caseRelativeReportPath`、sidecar template、workspace-relative 与 case-relative validate/record command strings + args、从 pack `tooling/catalog.yml` 投影的 `adapterCandidates[]`、默认 `selectedAdapter` / concrete sidecar `adapterId` guidance、`<executor-id>` replacement guidance、`RecordArgs` / `CaseRelativeRecordArgs` replay behavior 与 sidecar 规则，并可在 case-local / authorized output workspace cwd 中省略 `-Target`；`gate -ValidateExecutionReport -GateEventId ... -ExecutionReportPath ... -Format json` 可在记录前只读 strict validation bounded adapter sidecar，并以 `valid=true/false` envelope、stable failure taxonomy、`repairHints[]` 与 `adapterContext` 暴露 valid/invalid sidecar，report path 可为 case-relative、case-contained absolute 或当前 authorized output workspace 相对路径；`gate -Apply -GateEventId ... -ExecutionStatus ... -ExecutionReportPath ...` 可在 authorized-gate 后记录 observation execution evidence（actual budget、output refs、evidence refs、boundary hits、escalation、adapterContext）并 strict intake bounded adapter execution report，也可在 case-local / authorized output workspace cwd 中省略 `-Target`、消费 workspace-relative sidecar path；重复提交同一 sidecar 返回 `duplicate eventId` 且不重复 append observations；sidecar 声明 boundary/escalated 或预算越界时必须自带 boundary/escalation marker，`boundaryHits` 必须被本次 authorized gate `stopConditions` 覆盖，sidecar / explicit evidence 的 output refs、evidence refs 与 report path 必须落在 authorized output paths 内，failed/boundary/escalated/aborted sidecar 必须包含 bounded summary；authorized-gate 会进入 Mission brief、overview、handoff 与 continue digest/status，并直接携带 requested budget、authorized output paths、stop conditions、gate eventId 与可复制 report contract command，但不阻塞 lane。
- G3.1 已完成 Go attach 路径：`attach -WhatIf` 只预览，`attach -Apply` 写 binding metadata、初始 state 与 case-local thin shim；不写 managed docs/board/facts/lanes，Batch 105 起 retained façade 默认委托 Go，Batch 230 起 no-fallback。
- G3.2 已完成 Go repair 路径：默认/`repair -WhatIf` 只预览，`repair -Apply` 刷新 binding metadata、缺失初始 state 与 case-local thin shim；不写 managed docs/board/facts/lanes 或 authority，Batch 105 起 retained façade 默认委托 Go，Batch 230 起 no-fallback。
...
- G4.2 已完成 `start` Go 路径：`-WhatIf` 非写入预览，`-Apply` 显式初始化 board/facts/policy/default authority lane 并创建或进入 feature lane；Batch 117 起 attached case 的 `start -WhatIf -Format json` 与 `start -Apply` 默认经 façade 委托 Go；Batch 232 起文本 preview 与无 `-Apply` 文本 flow 也由 façade 明确请求 Go text output，不再回退 PowerShell；Batch 235 起 façade runtime dependency 状态由 `powerShellDeprecation.facadeRuntime` 审计。
- G4.3 已完成 `handoff` Go 路径：`-WhatIf` 非写入预览，`-Apply` 显式写项目级/工作线级 handoff 并刷新 lane resume/checkpoint；Batch 117 起 attached case 的 `handoff -WhatIf -Format json` 与 `handoff -Apply` 默认经 façade 委托 Go；Batch 232 起文本 preview 与无 `-Apply` 文本 flow 也由 façade 明确请求 Go text output，不再回退 PowerShell；Batch 235 起 façade runtime dependency 状态由 `powerShellDeprecation.facadeRuntime` 审计。
- G4.4 已完成 `plan-subagents` Go CLI review artifact 路径：按 manifest `subagentRoutes` 生成分片 packet/summary；Batch 58 已补 route/shard/review-loop observability；Batch 123 已用 `_template` pack Go CLI package E2E 验证 plan-subagents packet 可进入 gate request、overview 与 handoff 可见性链路；Batch 125 已用 `generic-binary-re` 验证 pack-specific route 与 non-feature lane handoff；Batch 126 已用 `web-security` 验证非 RE-only route 与 network gate handoff；Batch 190 起 retained PowerShell façade 默认委托 Go，Batch 225 起 no-fallback；runtime 不启动 agent、不写 board/facts/lanes/handoff/authority/confirmed。
- Batch 62-64 已新增只读 `packs` inventory，并将 pack maturity 固化为 manifest 显式字段；`-Format json` 提供机器可读 envelope，Batch 104 起 retained façade 默认委托 Go，Batch 224 起 PowerShell fallback 已退休并保持 no-fallback。
- 内部命令、authority/confirmed 更新和 schema 迁移仍需单独确认；当前 façade 默认委托 read-only、overview 文本/JSON 与缺 board 初始化、note list 文本/table/tsv/JSON 只读查询、note append/what-if facts JSONL 写入/预览、`gate -WhatIf` 非写入 preview、`gate -Apply` pending-gate request 写入、start/handoff JSON preview/显式 apply/文本 flow、`continue -WhatIf -Format json` 非写入 preview、explicit `continue -Apply` case-local 写入、continue 文本/default preview、case lifecycle、sync review/apply、promote review/artifact、promote candidate 写入、promote apply 写入与 promote JSON 非写入预览路径。
...
- 如果 Go-owned 写入路径影响旧 case，停止 apply/委托并记录差异，优先修复或使用 backup/recovery plan；所有 public commands 已 no-fallback，不得恢复 PowerShell 业务 runtime。
