...
| Pack smoke matrix | `.\rekit\tests\pack-smoke-matrix.ps1` / `-Packs web-security,generic-binary-re` | 显式运行安全领域 skeleton pack smoke 全量或子集，逐 pack 输出 running/passed/failed、elapsedMs 和原始 smoke 输出；只编排现有自包含 smoke，不改变 runtime 或 pack 行为。 |
