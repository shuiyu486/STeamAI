...
不委托：实际 heavy-tool 执行、无 `-Apply` 的工作线文本 workflow、authority/confirmed 写入和非 note/gate/continue apply 的其它 ledger 写入命令。文本 `sync -Apply -WhatIf` 已随 Batch 228 进入 no-fallback guard；文本 promote what-if 已随 Batch 229 进入 no-fallback guard。公共 `/rekit` 入口继续由 PowerShell 处理工作线文本命令、仍未退休的 legacy fallback 和未迁移写入路径。
