...
| Façade Go continue preview/apply/text | retired legacy façade continue compatibility path | Batch 118 起 JSON preview 默认委托 Go，输出非写入机器可读预览；Batch 121 起 explicit apply 默认委托 Go，写 case-local facts/routing/run digest/lane resume/checkpoint/board；Batch 232 起文本/default preview 也经 façade 委托 Go text output，`REKIT_GO_DISABLE=1` 或 Go delegation 不可用时直接 no-fallback；不写 authority/confirmed、不执行 heavy-tool。 |
