...
- G2.3 已完成 Go gate request ledger 写入：`gate -Apply` 只 append pending-gate / authorized-gate request JSONL，要求 `-Actor`，不执行 heavy-tool；authorized-gate 会进入 Mission brief、overview、handoff 与 continue digest/status，但不阻塞 lane。
