...
- `subagentRoutes` 的 namespaced route id（例如 `<pack>:bounded-review`，namespace 必须匹配当前 pack id）、taskTypes、trigger、shardBasis、subagentPermissions 和 outputContract；trigger 必须显式非空，说明何时触发该 route；subagentPermissions 只能是 `read-only` 或 `read-only-or-workspace-only`；taskTypes、mainAgentOwns 与 outputContract 是逗号/分号分隔列表，不能包含空项。
...
- `subagentRoutes.reference` 指向 managed/template/local 文件，route id 使用匹配当前 pack id 的 namespaced 形式且唯一，`taskTypes`、`trigger`、`shardBasis`、`targetItemsPerAgent`、`maxParallel`、`subagentPermissions`、`mainAgentOwns`、`outputContract` 齐全，`subagentPermissions` 使用受支持值，且逗号/分号分隔字段不含空项。
