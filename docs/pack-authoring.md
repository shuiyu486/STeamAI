...
- `subagentRoutes` 的 namespaced route id（例如 `<pack>:bounded-review`）、taskTypes、trigger、shardBasis 和 outputContract；trigger 必须显式非空，说明何时触发该 route。
...
- `subagentRoutes.reference` 指向 managed/template/local 文件，route id 使用 namespaced 形式且唯一，`taskTypes`、`trigger`、`shardBasis`、`targetItemsPerAgent`、`maxParallel`、`subagentPermissions`、`mainAgentOwns`、`outputContract` 齐全。
