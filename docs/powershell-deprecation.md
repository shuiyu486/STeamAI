...
| `run-driver-step` bounded current-step runner | Go default | façade delegate + no PowerShell fallback | MVP只消费 focused case-scoped `continue -WhatIf` typed current request；outer `-WhatIf` 返回 exact qualified Apply request 与绑定完整 deterministic preview 的 `expectedDriverStepPlanSha256`，outer `-Apply` 必须 hash-bound，并在 lane mutation lease 内复验 preview SHA 后直接调用 Go continue Apply、刷新 status/receipt；outer/nested 参数均 strict allowlist。不调用 shell、不 spawn/poll session、不执行 reviewer/adapter/heavy-tool、不写 authority/confirmed。PowerShell façade 只验证/透传 runner 参数，不承载 runtime。 |
...
- `commandSurface[]` 来自 `rekit/rekit.ps1` 的 `ValidateSet`，当前基线为 22；每个公开 façade command 都必须同时出现在 `goDefaultCommands[]` 与 `noFallbackCommands[]` 中，确保公共 façade 只透传 Go-owned / no-fallback 命令面。
...
- `facade-command-surface-no-fallback` 要求 22 个 façade commands 与 Go-default / no-fallback command surface 完全一致，不能留下隐式 PowerShell fallback。
...
- `noFallbackCommands[]` 表示已经没有 PowerShell fallback 的 Go-default 命令；当前基线包含 `release-check`、`release-run`、`run-driver-step`、`next-batch`、`status`、`packs`、`doctor`、`validate`、`attach`、`repair`、`init`、`bootstrap`、`sync`、`update`、`promote`、`overview`、`note`、`gate`、`start`、`handoff`、`continue`、`reconcile` 与 `plan-subagents`。
