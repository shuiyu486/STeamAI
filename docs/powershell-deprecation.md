...
|---|---|---|---|
| `release-check` / `release-run` | Go default | façade delegate + no PowerShell fallback | `release-check` 只输出 release gate inventory；`release-run` 顺序执行 local minimum，并在待提交时仅替换 Go-owned Git-local exact validation receipt，不写 tracked repo/case state；PowerShell 不解析或生成 receipt。 |
