...
|---|---|---|---|
| `release-check` / `release-run` | Go default | façade delegate + no PowerShell fallback | `release-check` 只输出 release gate inventory；`release-run` 顺序执行 local minimum，并在待提交时仅替换 Go-owned Git-local strict v2 validation receipt，绑定工作树hash/bytes与Git clean-filter blob OID；PowerShell 不解析或生成 receipt，也不写 tracked repo/case state。 |
