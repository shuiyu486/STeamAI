...
- **PowerShell façade**：`rekit/rekit.ps1` 是 retained public compatibility entry，负责 Go binary 查找、参数兼容、环境变量开关和旧 case 用户体验；对 `plan-subagents` reviewer intake 只透传 `PacketPath` / `ReviewerResultPath` / `Lane` / `Actor` / WhatIf / Apply / Format，拒绝其它 command 携带 `ReviewerResultPath`，并在内部 `Push-Location` 前按 façade 调用者工作目录规范化 packet/result 相对路径。它不实现 reviewer validation、ledger writeback 或其它业务 runtime。
...
|---|---|---|---|
| `release-check` / `release-run` | Go default | façade delegate + no PowerShell fallback | `release-check` 只输出 release gate inventory；`release-run` 顺序执行 local minimum，并在待提交时仅替换 Go-owned Git-local typed v3 validation receipt，显式区分numbered-batch与已completed的exact active-route subject，并绑定工作树hash/bytes与Git clean-filter blob OID；旧v2仅只读兼容numbered-batch，post-commit只额外接受exact LF blob的纯CRLF checkout物化，其它byte drift仍fail-closed；PowerShell 不解析或生成 receipt，也不写 tracked repo/case state。 |
| `next-batch` planning receipt | Go default | façade delegate + no PowerShell fallback | `next-batch -WhatIf` 从 release handoff starter package 预览 `docs/batch-history.md` / `CHANGELOG.md` / `docs/batch-plan.md` 三份 exact writes，先归档更早 active batch并返回 `expectedNextBatchPlanSha256`；`-Apply` 必须带该 hash，可从 exact write prefix 幂等恢复，且只写这三个 kit docs，不触碰 case state、heavy-tool、authority/confirmed、reviewer/adapter/pack-memory/gate/sync/promote mutation、commit/push 或 remote CI。 |
| `run-current-loop` bounded current-loop driver | Go default | façade delegate + no PowerShell fallback | 以一次 hash-bound WhatIf/Apply 在固定 initial route/lane 下最多执行 `MaxSteps` 个 refreshed deterministic current steps，保留逐步 receipts、typed stop reason与final status；route/lane drift、fresh Human-in-the-Lane reconcile和external reviewer stop在有剩余预算时返回统一campaign continuation，携带segment→expected route/lane、扣减后的`remainingMaxSteps`与fresh WhatIf，external reviewer另带observation alternatives。成功Apply另由Go在project mutation lease内按严格递增sequence写immutable current-loop segment checkpoint，并绑定前一个exact artifact SHA；`status`与explicit`handoff`由Go验证完整无gap/fork chain、canonical exact bytes、case/pack、重算的outer plan/request-receipt lineage与refreshed current request，只在`state=ready`时暴露continuation/remaining budget和`resumeDriverRequest`；Go `-ResumeCurrentLoop`从strict checkpoint派生预算、复核expected route/lane/current request，并让fresh preview返回绑定source artifact、新nested plan与observation inputs的exact `applyCommand`；Apply在任何nested mutation前以case-local O_EXCL claim one-shot消费source，claim后status/handoff立即标记`consumed`且不恢复预算，并发、崩溃、nested/publication失败均不能重放同一source。successor checkpoint以`resumeSourceSha256`绑定immediate predecessor，publication lease在创建artifact前拒绝stale source。wall-clock回退不影响latest，tamper/chain break/symlink/unknown entry/stale/claimed/terminal/status-unavailable不回退旧artifact。下一segment从refreshed durable status重新hash-bound，旧plan hash/receipts不跨调用复用或累计；managed reviewer result仍必须从独立case-local source进入。non-executable/guidance、blocker、重复exact step plan、limit或error即停；nested hash/lease/packet/artifact/lock guards保持不变。非事务性，不自动跨界Apply，不调用shell/Agent/session/heavy-tool，不写authority/confirmed。PowerShell façade只验证和透传已有loop/reviewer/member-execution observation参数，以及Go-owned external-session attempt/relay的switch、opaque session identity与expected hashes；不解析attempt/submission、不计算generation/supersession/currentness、campaign continuation、member manifest/output hash、reviewer relay source或envelope；public command surface为32。 |
| `run-current-step` unified current-step router | Go default | façade delegate + no PowerShell fallback | 从 refreshed `missionControlRunbook.scope` 与 typed `currentDriverRequest` 选择 `run-driver-step` 或 `run-reviewer-step`；outer hash 同时绑定 route、current request 与 nested plan SHA，Apply 复用所选 runner 并返回 refreshed request receipt。lane mutation lease 与 reviewer packet/artifact/candidate/intake guards 继续由 nested runner 持有；不合并锁、不调用 Agent tool、不执行 heavy-tool、不写 authority/confirmed。PowerShell façade 只验证和透传统一入口参数，不承载路由业务逻辑。 |
| `run-driver-step` bounded current-step runner | Go default | façade delegate + no PowerShell fallback | 消费 focused case-scoped `start` / `continue` / `reconcile` typed current request；outer `-WhatIf` 返回 exact qualified Apply request 与 `expectedDriverStepPlanSha256`，outer `-Apply` 必须 hash-bound，并在 lane mutation lease 内复验 preview SHA 后直接调用对应 Go handler、刷新 status/receipt；outer/nested 参数均 strict allowlist。不调用 shell、不 spawn/poll session、不执行 reviewer/adapter/heavy-tool、不写 authority/confirmed。PowerShell façade 只验证/透传 runner 参数，不承载 runtime。 |
| `run-reviewer-step` bounded reviewer operator runner | Go default | façade delegate + no PowerShell fallback | 只消费 Mission Commander reviewer operator package 的 typed current request；spawn reviewer 与等待结果保留为外部 harness handoff，dispatch/result-input/completion/source-capture/staging/collection/intake 直接调用 deterministic Go handlers。`-WhatIf` 返回 hash-bound Apply request，`-Apply` 重建当前 request 并刷新 reviewer status/receipt；不由 Go runtime spawn/poll session、不执行 heavy-tool、不写 authority/confirmed。PowerShell façade 只验证和透传外部 observation，不承载 reviewer runtime。 |
| `run-reviewer-wave` packet-level reviewer observation runner | Go default | façade delegate + no PowerShell fallback | 从 current reviewer packet wave 接收 case-local strict observation JSON，一次预览并 hash-bound 应用多个 shard 的 accepted/returned/failed 外部观察；每个 shard 继续复用独立 receipt/result-input guards，单项失败保留此前 immutable writes 并返回 refreshed partial wave。Go runtime 不 spawn/poll/stop session、不调用 Agent、不执行 heavy-tool、不写 authority/confirmed；PowerShell façade 只验证并透传参数。 |
...
| `onboard -WhatIf` / exact-hash `onboard -Apply` | Go default | façade delegate + no PowerShell fallback | 新 case 默认使用 Go-owned review-first mission intent publication；PowerShell 仅校验并透传显式 identity、stamp/hash 和 `WhatIf`/`Apply`，不解析 Goal、不计算 hash、不恢复 publication。 |
| `onboard` immutable mission intent | Go default | façade thin delegate + no PowerShell fallback | 主 Agent把自然语言显式映射为 `Target` / `Pack` / `ProjectName` / opaque bounded `Goal` / `Actor` / `Executor` / `InitialLane`；Go `-WhatIf -Format json` 返回 exact publication stamp/hash 与机器可读 `applyArgs[]`，Apply 只消费 exact identity/stamp/hash，按 intent-first / commit-last 发布并支持 partial recovery / committed replay。PowerShell 只校验并透传参数，不解析自然语言、不计算 intent/hash、不创建 board/lane、不 spawn/poll session、不执行 heavy-tool、不写 authority/confirmed。 |
| `migrate-state` zero-write preview / exact-hash Apply | Go default | façade delegate + no PowerShell fallback | legacy-only attached project 的 `.rekit` → `.steamai` 迁移由 Go owner 完成；PowerShell façade 只校验并透传 `Target` / `Pack` / `Format` / `WhatIf` / `Apply` / `ExpectedMigrationPlanSha256`，不扫描或改写 state root、不计算 plan hash、不生成 runtime bundle 或 receipt。 |
| `sync` / `update` review/apply/JSON preview | Go default | façade delegate + no PowerShell fallback | `update` 作为 sync alias 同属 Go-default façade；PowerShell fallback 已退休，review、JSON preview 与显式 apply 由 Go 维护，继续保持 review-first 与 explicit apply 边界。Completed verified pack-memory selected sync 也由 Go 负责 catalog discovery、single-change WhatIf、exact plan hash、target/state/backup publication与case-local consumption receipt；PowerShell façade只安全校验并逐字透传`SelectPackMemoryChange`和`ExpectedPackMemoryConsumptionPlanSha256`，不扫描case、不解析producer proof、不计算hash、不生成receipt。 |
...
| `gate -WhatIf` / `gate -Apply` gate decision/evidence ledger | Go default | façade delegate + no PowerShell fallback | 只预览或写 pending-gate / authorized-gate request ledger decision；`gate -ExecutionReportContract -GateEventId ... -Format json` 只读投影 authorized-gate 的 adapter execution report contract（含 default report path、validation failure stages/codes、`validationRepairHints[]`、`liveValidation` `authorizedWorkspaces[]` / `reportFileName` / `caseRelativeReportPath`、sidecar template、workspace-relative 与 case-relative validate/record command strings + args、pack tooling `adapterCandidates[]`、默认 `selectedAdapter` / sidecar `adapterId` guidance、`<executor-id>` replacement guidance、`RecordArgs` / `CaseRelativeRecordArgs` replay behavior），可在 case-local / authorized output workspace cwd 中省略 `-Target`；`gate -ValidateExecutionReport -GateEventId ... -ExecutionReportPath ... -Format json` 只读 strict validation bounded adapter report、返回 `valid=true/false` machine-readable envelope（含可用的 `adapterContext`；invalid sidecar 含 failureCode/failureStage/repairHints）且不写 observations，report path 可为 case-relative、case-contained absolute 或当前 authorized output workspace 相对路径；`gate -Apply -GateEventId ... -ExecutionStatus ... -ExecutionReportPath ...` 只记录 authorized execution observation evidence（actual budget、output refs、evidence refs、boundary hits、escalation 与 strict validated bounded adapter execution report provenance 与 concrete tooling candidate adapterContext；sidecar boundary/escalation marker、authorized stopCondition boundaryHits、authorized outputPath evidenceRefs 与 failed/boundary/escalated/aborted summary fail-closed validation），并可在 case-local / authorized output workspace cwd 中省略 `-Target`、消费 workspace-relative sidecar path，重复提交同一 sidecar 返回 `duplicate eventId` 且不重复 append observations；authorized-gate 在 Mission brief、overview、handoff 与 continue digest/status 中可见，且 gate line 直接展示 requested budget、authorized output paths、stop conditions、gate eventId 与可复制 report contract command 但不阻塞 lane；不执行 heavy-tool、不写 authority/confirmed；PowerShell fallback 已退休。 |
| `control -WhatIf` / exact-hash `control -Apply` | Go default | façade delegate + no PowerShell fallback | durable `pause` / `resume` / `stop` 由 Go owner 以 per-lane append-only generation、exact owner 与 reviewed stamp/hash 发布；`pause` 不做 OS suspend，`stop` 的 durable receipt 先于 exact local supervisor owned-containment actuation，进程终止不作为 durable stop 成功判据，opaque Remote Control session 不受本路径管理。PowerShell façade 只校验并逐字透传显式或 case-local resolved `Target` / `Pack` / `Lane` / `Action` / `Actor` / `Reason` / `ControlPublicationStamp` / `ExpectedControlPlanSha256` / `WhatIf` / `Apply` / `Format`，不读取 control head、不计算 generation/hash、不写 receipt、不管理进程。 |
...
| `complete -WhatIf` / exact-hash `complete -Apply` | Go default | façade delegate + no PowerShell fallback | 独立 review-first lane completion owner；要求 actor/reason/case-local evidence refs，拒绝 durable blockers 与 main-before-feature closure，以 intent + final commit marker 可恢复发布 lane/board/resume/checkpoint，并在全 lane committed closed 时返回 typed `mission-complete`；只记录 operational closure，不写或暗示 authority/confirmed、不执行 heavy-tool。PowerShell façade仅验证并透传 selector/evidence/hash参数。 |
| `reopen -WhatIf` / exact-hash `reopen -Apply` | Go default | façade delegate + no PowerShell fallback | 独立 review-first lane completion supersession owner；保留历史 completion artifacts，以append-only lifecycle和operation-level final commit原子化复开terminal feature与已失效的authority aggregate lane，清空旧executor owner并递增generation，拒绝复用旧current-loop预算；不写authority/confirmed、不执行heavy-tool、不自动恢复旧session。PowerShell façade仅验证并透传selector/evidence/hash参数。 |
| `continue -WhatIf` / explicit `continue -Apply` text/JSON | Go default | façade delegate + no PowerShell fallback | Batch 232 起 JSON preview、explicit apply 与文本/default preview 均由 Go-owned 路径接管；`continue -Apply` 仍只写 case-local facts/routing/run digest/lane resume/checkpoint/board，不写 authority/confirmed、不执行 heavy-tool；存在 effective open intervention 时 fail-closed，要求先 reconcile。 |
| `reconcile -WhatIf` / explicit `reconcile -Apply` | Go default | façade delegate + no PowerShell fallback | 显式关闭 lane-local open intervention，把 resolution event 写入 append-only interventions ledger，并刷新 executor generation、lane resume/checkpoint/board；不写 authority/confirmed、不执行 heavy-tool。 |
| `plan-subagents` planning / reviewer intake | Go default | façade delegate + no PowerShell fallback | planning 只写 review packet / summary / combined diff 路径且不自动 spawn reviewer；reviewer intake 由主 Agent 显式调用，严格校验 reviewer result 后按 verification-before-decision 写 case-local facts，并返回 post-validation；不写 authority/confirmed、不执行 heavy-tool。PowerShell fallback 已退休，使用 Go package tests、reviewer-intake E2E 与 `plan-subagents-smoke.ps1` 维护 parity。 |
...
- `commandSurface[]` 来自 `rekit/rekit.ps1` 的 `ValidateSet`，当前基线为 32；每个公开 façade command 都必须同时出现在 `goDefaultCommands[]` 与 `noFallbackCommands[]` 中，确保公共 façade 只透传 Go-owned / no-fallback 命令面。
...
- `commandCatalogPath=internal/rekit/commands/commands.go` 与 `commands[]` 是 Go-owned public command catalog，当前基线为 32；`handlerCommands[]` 来自 Go CLI dispatcher 的 `commands.*` handler case，`symbolCommands{}` 来自 public command symbol catalog；`commandProfiles[]` 与 `mutationBoundaries[]` 记录每个 public command 的 read-only、Git-local validation receipt、case-local append/apply/bootstrap、review writeback、review-first 或 kit review-first 边界；`run-current-loop` 是 case-local review-first bounded current-loop driver，保留 nested runner guard、typed stop/receipts 与 fixed initial route/lane；`release-run` 是 `local-validation-receipt` local runner，只执行 `release-check` gate profile 已解析的本机验证步骤，成功后替换一份不进入tracked工作树、显式区分numbered-batch与exact active-route并绑定工作树SHA-256/bytes与Git clean-filter blob OID的Git-local typed v3 validation receipt；旧v2仅只读兼容numbered-batch，不写 tracked repo/case state；`next-batch` 是 kit review-first planning receipt，只允许 WhatIf 预览和 expected-hash Apply 写 `docs/batch-plan.md` / `CHANGELOG.md`；`complete` 是 case-local review-first lane completion owner；`run-reviewer-wave` 是case-local review-first外部reviewer observation bundle owner；`plan-subagents` 的 mixed mode 以 `case-local-review-writeback` 披露 planning artifact 与显式 reviewer-intake Apply 的最大写入面；`commandProfileSummary` 汇总并校验 `total=32`、`readOnly=5`、`mutating=27`、`writesCase=24`、`writesKit=2`、`reviewFirst=14`、`applyRequired=24`、`heavyTool=0` 与 `authorityConfirmed=0`；`commandProfileGroups` 将这些 counts 反向展开为 read-only、mutating、case write、kit write、review-first、apply-required、heavy-tool、authority/confirmed 和 per-boundary command sets；`commandProfileBoundaries[]` 将 8 个 mutation/read-only boundary 输出为 `boundary`、`count` 与 `commands[]` rows，并与 summary / groups 互相校验；`commandProfilePolicies[]` 将 no heavy-tool、no authority/confirmed、kit write review-first、review-first apply-required 与 known mutation boundary guardrails 输出为 policy rows，并要求 violation count 为 0。release inventory 要求 catalog、dispatcher handlers、symbols 与 command profiles 均覆盖同一 32 个 public commands，并拒绝 public command profile 标记 actual heavy-tool 或 authority/confirmed 写入，避免 public façade command surface、Go-native catalog、CLI handler coverage、mutation boundary catalog、summary counts、command groups、boundary rows 和 policy rows 各自漂移。
...
- `facade-command-surface-no-fallback` 要求 32 个 façade commands 与 Go-default / no-fallback command surface 完全一致，不能留下隐式 PowerShell fallback。
...
- `noFallbackCommands[]` 表示已经没有 PowerShell fallback 的 Go-default 命令；当前基线包含 `release-check`、`release-run`、`run-current-loop`、`run-current-step`、`run-driver-step`、`run-reviewer-step`、`run-reviewer-wave`、`next-batch`、`status`、`packs`、`doctor`、`validate`、`attach`、`repair`、`init`、`bootstrap`、`onboard`、`sync`、`update`、`promote`、`overview`、`note`、`gate`、`start`、`handoff`、`complete`、`reopen`、`continue`、`reconcile` 与 `plan-subagents`。
