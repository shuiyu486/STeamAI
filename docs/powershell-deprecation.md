...
| `run-current-loop` bounded current-loop driver | Go default | façade delegate + no PowerShell fallback | 以一次 hash-bound WhatIf/Apply 在固定 initial route/lane 下最多执行 `MaxSteps` 个 refreshed deterministic current steps，保留逐步 receipts、typed stop reason与final status；external reviewer stop返回按本segment已执行步骤扣减的`continuationRequest.remainingMaxSteps`、fresh WhatIf command和结构化observation alternatives，下一segment重新hash-bound且不伪称跨调用累计receipts。managed reviewer result必须从与canonical drop/input/source paths分离的现有case-local source进入。Human-in-the-Lane reconcile、non-executable/guidance、route/lane drift、重复exact step plan、limit或error即停。后续步骤继续使用nested runner的exact hash/lease/packet/artifact/lock guards；非事务性，不调用shell/Agent/session/heavy-tool，不写authority/confirmed。PowerShell façade只验证和透传已有loop/reviewer observation参数，不承载循环或continuation业务逻辑。 |
