...
|---|---|---|---|
| `release-check` / `release-run` | Go default | façade delegate + no PowerShell fallback | `release-check` 只输出 release gate inventory；`release-run` 顺序执行 local minimum，并在待提交时仅替换 Go-owned Git-local strict v2 validation receipt，绑定工作树hash/bytes与Git clean-filter blob OID；post-commit只额外接受exact LF blob的纯CRLF checkout物化，其它byte drift仍fail-closed；PowerShell 不解析或生成 receipt，也不写 tracked repo/case state。 |
