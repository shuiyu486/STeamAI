...
| `run-current-loop` bounded current-loop driver | Go default | façade delegate + no PowerShell fallback | 以一次 hash-bound WhatIf/Apply 在固定 initial route/lane 下最多执行 `MaxSteps` 个 refreshed deterministic current steps，保留逐步 receipts、typed stop reason、final status 与 fresh resume WhatIf；遇 external reviewer handoff、Human-in-the-Lane reconcile、non-executable/guidance、route/lane drift、重复 exact step plan、limit 或 error 即停。后续步骤继续使用 nested runner 的 exact hash/lease/packet/artifact/lock guards；非事务性，不调用 shell/Agent/session/heavy-tool，不写 authority/confirmed。PowerShell façade 只验证和透传 loop 参数及首步 reviewer observations，不承载循环业务逻辑。 |
