...
| `gate -WhatIf` / `gate -Apply` gate decision/evidence ledger | Go default | façade delegate + no PowerShell fallback | 只预览或写 pending-gate / authorized-gate request ledger decision；`gate -ExecutionReportContract -GateEventId ... -Format json` 只读投影 authorized-gate 的 adapter execution report contract（含 validation failure stages/codes）；`gate -ValidateExecutionReport -GateEventId ... -ExecutionReportPath ... -Format json` 只读 strict validation bounded adapter report、返回 `valid=true/false` machine-readable envelope（invalid sidecar 含 failureCode/failureStage）且不写 observations；`gate -Apply -GateEventId ... -ExecutionStatus ... -ExecutionReportPath ...` 只记录 authorized execution observation evidence（actual budget、output refs、evidence refs、boundary hits、escalation 与 strict validated bounded adapter execution report provenance；sidecar boundary/escalation marker fail-closed validation）；authorized-gate 在 Mission brief、overview、handoff 与 continue digest/status 中可见但不阻塞 lane；不执行 heavy-tool、不写 authority/confirmed；PowerShell fallback 已退休。 |
