...
- `facadeRemovalReady=true` 与 `facadeRemovalPrerequisites[]` 是公共 façade 删除前置库存，不代表本批删除 façade；当前 5 条 prerequisites 分别覆盖 `entrypoint`、`catalog-handler-symbol-profile-coverage`、`mutation-boundary-inventory`、`profile-policy-guards` 与 `unsupported-command-diagnostic`，全部 ready 后才能把公共 façade 删除作为后续独立 removal batch 评估。
