...
| `gate -WhatIf` / `gate -Apply` gate decision/evidence ledger | Go default | façade delegate + no PowerShell fallback | 只预览或写 pending-gate / authorized-gate request ledger decision；`gate -Apply -GateEventId ... -ExecutionStatus ... -ExecutionReportPath ...` 只记录 authorized execution observation evidence（actual budget、output refs、evidence refs、boundary hits、escalation 与 strict validated bounded adapter execution report provenance）；authorized-gate 在 Mission brief、overview、handoff 与 continue digest/status 中可见但不阻塞 lane；不执行 heavy-tool、不写 authority/confirmed；PowerShell fallback 已退休。 |
