...
- **Release inventory**：Go-owned `release-check -Format json` 输出 `powerShellDeprecation`、`ciReleaseGate` 与 `caseShim`，解析本文件中的命令归属、模块状态、freeze gates 和 blocked migrations，并对照 `rekit/rekit.ps1` 默认委托集合、`rekit/lib/*.ps1` 实际模块清单、默认 CI workflow 与 case-local thin shim 模板发现漂移。
...
4. **Release inventory**：`release-check` 的 `powerShellDeprecation` 与 `caseShim` inventory 必须 `ready=true`，并能解析命令归属、模块状态、freeze gates、blocked migrations、默认委托、实际 `.ps1` 模块清单，以及 case-local thin shim 是否保持无 PowerShell / raw CLI 默认入口。
