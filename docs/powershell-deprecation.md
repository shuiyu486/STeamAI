...
- `commandSurface[]` 来自 `rekit/rekit.ps1` 的 `ValidateSet`，当前基线为 24；每个公开 façade command 都必须同时出现在 `goDefaultCommands[]` 与 `noFallbackCommands[]` 中，确保公共 façade 只透传 Go-owned / no-fallback 命令面。
...
- `facade-command-surface-no-fallback` 要求 24 个 façade commands 与 Go-default / no-fallback command surface 完全一致，不能留下隐式 PowerShell fallback。
...
- `noFallbackCommands[]` 表示已经没有 PowerShell fallback 的 Go-default 命令；当前基线包含 `release-check`、`release-run`、`run-driver-step`、`run-reviewer-step`、`next-batch`、`status`、`packs`、`doctor`、`validate`、`attach`、`repair`、`init`、`bootstrap`、`sync`、`update`、`promote`、`overview`、`note`、`gate`、`start`、`handoff`、`continue`、`reconcile` 与 `plan-subagents`。
