...
- `noFallbackCommands[]` 表示已经没有 PowerShell fallback 的 Go-default 命令；当前基线包含 `release-check`、`status`、`packs`、`doctor`、`validate`、`sync`、`update`、`overview`、`note`、`gate` 与 `plan-subagents`。
