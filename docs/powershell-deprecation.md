...
| `run-current-loop` bounded current-loop driver | Go default | façade delegate + no PowerShell fallback | 以一次 hash-bound WhatIf/Apply 在固定 initial route/lane 下最多执行 `MaxSteps` 个 refreshed deterministic current steps，保留逐步 receipts、typed stop reason与final status；route/lane drift、fresh Human-in-the-Lane reconcile和external reviewer stop在有剩余预算时返回统一campaign continuation，携带segment→expected route/lane、扣减后的`remainingMaxSteps`与fresh WhatIf，external reviewer另带observation alternatives。成功Apply另由Go在project mutation lease内按严格递增sequence写immutable current-loop segment checkpoint，并绑定前一个exact artifact SHA；`status`与explicit`handoff`由Go验证完整无gap/fork chain、canonical exact bytes、case/pack、重算的outer plan/request-receipt lineage与refreshed current request，只在`state=ready`时暴露continuation/remaining budget，wall-clock回退不影响latest，tamper/chain break/symlink/unknown entry/stale/terminal/status-unavailable不回退旧artifact。下一segment从refreshed durable status重新hash-bound，旧hash/receipts不跨调用复用或累计；managed reviewer result仍必须从独立case-local source进入。non-executable/guidance、blocker、重复exact step plan、limit或error即停；nested hash/lease/packet/artifact/lock guards保持不变。非事务性，不自动跨界Apply，不调用shell/Agent/session/heavy-tool，不写authority/confirmed。PowerShell façade只验证和透传已有loop/reviewer observation参数，不计算campaign continuation。 |
