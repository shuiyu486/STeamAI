...
| `next-batch` planning receipt | Go default | façade delegate + no PowerShell fallback | `next-batch -WhatIf` 只从 release handoff starter package 预览 `docs/batch-plan.md` / `CHANGELOG.md` planning receipt 并返回 `expectedNextBatchPlanSha256`；`-Apply` 必须带该 hash 且只写这两个 kit docs，不触碰 case state、heavy-tool、authority/confirmed、reviewer/adapter/pack-memory/gate/sync/promote mutation、commit/push 或 remote CI。 |
