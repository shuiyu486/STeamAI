...
| `gate -WhatIf` / `gate -Apply` gate decision ledger | Go default | façade delegate + no PowerShell fallback | 只预览或写 pending-gate / authorized-gate request ledger decision；authorized-gate 在 Mission brief、overview、handoff 与 continue digest/status 中可见但不阻塞 lane；不执行 heavy-tool、不写 authority/confirmed；PowerShell fallback 已退休。 |
