...
当前已经落地的是安全 Agent Team 框架底座、文档契约、`/rekit` 工作线 runtime、review-first sync/promote、首个成熟 pack `vmp-re` 扩展、非 RE pack 骨架 `web-security` 与 `malware-analysis`、tooling candidate、`ida-agent-bridge` 只读 packet contract，以及 evidence ledger runtime（`/rekit note` 手动 append 9 种 kind 事件 + overview/handoff/note-List 读层 + auto 流程 decision 字段对齐 `docs/evidence-ledger.md` 草案）；尚未落地的是 runtime 强制 heavy-tool 执行闭环、真实工具 bridge adapter 实现、自动多 Agent dispatch（R5 判定 runtime 不自动 spawn，由主会话用 Agent 工具完成），也不能宣称已具备自动脱壳/逆向引擎、自动漏洞挖掘器、自动恶意样本分析平台或通用自动渗透平台。
...
- [ ] 扩展 `web-security`、`malware-analysis`、`vuln-research`、`ctf`、`unpack-pe`、`ollvm`、`android-native` 等安全领域 pack（`web-security` 与 `malware-analysis` 已有最小骨架，其它仍待推进）。
...
| pack 作者骨架 | `packs/_template/` | 后续创建新 pack 的最小模板；`web-security` 与 `malware-analysis` 提供非 RE skeleton 参考 |
...
| 多 pack 扩展 | `_template` + `packs/web-security/` + `packs/malware-analysis/` | `web-security` 与 `malware-analysis` 已有 skeleton；继续新增 `vuln-research` / `ctf` / `unpack-pe` / `ollvm` / `android-native` pack |
