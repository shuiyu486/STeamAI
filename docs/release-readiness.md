...
- 仍未退休 candidate commands 的 `REKIT_GO_DISABLE=1` compatibility fallback；`release-check`、`status`、`packs`、`doctor`、`validate`、`attach`、`repair`、`init`、`bootstrap`、`sync`、`update`、`promote`、`overview`、`note`、`gate` 与 `plan-subagents` 已 no-fallback。
