...
- `continue -Apply` 只写 case-local facts/routing/run digest/lane resume/checkpoint/board；lane resume/checkpoint 可包含 pending/authorized gate snapshot，帮助替换 executor 接手，但不执行 heavy-tool；存在 effective open intervention 时 fail-closed，要求先 `reconcile`；不写 authority/confirmed。
