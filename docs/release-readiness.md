...
- Go backend 已是全部 20 个 public commands 的 deterministic owner：`status`、`packs`、`doctor/validate`、case lifecycle、sync/promote、overview、note、gate、start/handoff、continue preview/apply safe subset、reconcile resolution / executor takeover，以及 `plan-subagents` review artifacts / shard handoff prompts / reviewer result contracts / intake checklists / reviewer decision mappings / conflict handling / main-agent writeback sequence / command bindings / reviewer verdict preview/apply writeback templates。
...
- `plan-subagents` review artifact 写入：只生成 packet / summary / combined diff 路径、shard handoff reviewer result contract、main-agent intake checklist、reviewer decision mappings、conflict handling、main-agent writeback sequence / command bindings 与 reviewer verdict preview/apply writeback templates，不自动 spawn reviewer、不替 reviewer 写 ledger、不写 authority/confirmed，PowerShell fallback 已退休。
...
- Mission Commander 尚无统一 session/reviewer orchestrator：durable lane、handoff/checkpoint 与 executor takeover contract 已存在，但仓库 runtime 不启动或管理可替换 Claude Code member session；bounded dispatch 也仍不自动 spawn reviewer，主 Agent需自行完成实际调度、structured result intake、WhatIf、ledger writeback、post-review validation 和 merge decision。
