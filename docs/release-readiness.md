...
- PowerShell `rekit/rekit.ps1` 仍是迁移期公共 legacy façade：负责参数兼容、剩余 compatibility fallback、少量 parity smoke 和 legacy-only 路径；`release-check`、`status`、`packs`、`doctor`、`validate`、`attach`、`repair`、`init`、`bootstrap`、`sync`、`update`、`promote`、`overview`、`note`、`gate`、`start`、`handoff`、`continue` 与 `plan-subagents` 已进入 no-fallback Go-owned 路径，default delegation / disabled no-fallback 先于 legacy runtime module dot-source 执行。当前阶段默认向 PowerShell-free / Go-native / 跨平台路径收敛，后续不应新增 PowerShell runtime logic。
...
- 普通 Go-default command fallback candidates 已清空；`release-check`、`status`、`packs`、`doctor`、`validate`、`attach`、`repair`、`init`、`bootstrap`、`sync`、`update`、`promote`、`overview`、`note`、`gate`、`start`、`handoff`、`continue` 与 `plan-subagents` 均已 no-fallback。
