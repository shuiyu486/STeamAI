...
- 普通 Go-default command fallback candidates 已清空；`release-check`、`status`、`packs`、`doctor`、`validate`、`attach`、`repair`、`init`、`bootstrap`、`sync`、`update`、`promote`、`overview`、`note`、`gate`、`start`、`handoff`、`continue` 与 `plan-subagents` 均已 no-fallback；`powerShellDeprecation.facadeRuntime` 同步锁定公共 façade 不再 dot-source legacy modules 或包含 command switch fallback dispatcher；`powerShellDeprecation.moduleRemoval` 同步锁定 removal-candidate module 清单、façade dependency 与 undocumented module 状态；`powerShellDeprecation.moduleReferences` 同步分类 active test dependency、compatibility fixture、inventory guard、文档/历史引用，并要求 removal blockers 与 unclassified references 为 0。
