...
- `continue -Apply` 只写 case-local facts/routing/run digest/lane resume/checkpoint/board；lane resume/typed checkpoint 可包含 lane-local Mission Control brief、executor action snapshot 与 pending/authorized gate snapshot，帮助替换 executor 看到 blocked/ready、blocker reasons、reconcile/pending-gate/open-decision requirements、resume/handoff command、next action、escalation 与 heavy-action boundary 后接手，但不执行 heavy-tool；存在 effective open intervention 时 fail-closed，要求先 `reconcile`；不写 authority/confirmed。
