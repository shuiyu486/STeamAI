...
- `gate -WhatIf`、`gate -Apply` gate request decision / execution evidence：只预览或写 pending-gate / authorized-gate request ledger decision，或在 `-GateEventId` / `-ExecutionStatus` 模式下写 authorized execution observation evidence（actual budget、output refs、evidence refs、boundary hits、escalation），PowerShell fallback 已退休；JSON/text 输出当前 executor action、WhatIf 写入 request 后的 would executor action、request Apply 后 executor action，或 evidence Apply 后 executor action；authorized-gate 在 Mission Control brief、overview、handoff 与 continue digest/status 中可见但不阻塞 lane；仍不执行 heavy-tool、不写 authority/confirmed。
