...
- 改 Agent Team / workstream / ledger / gate：运行对应临时 case smoke，例如 `agent-team-review-loop-smoke.ps1`、`gate-parity-smoke.ps1`、`web-security-agent-team-dryrun-smoke.ps1`。
...
| `generic-binary-re` | skeleton | main | 已有 pack-neutral package E2E；不执行样本/debug/trace/dump/patch。 |
...
| `vuln-research` | skeleton | main | 不主动扫描、不 fuzz、不 replay exploit。 |
