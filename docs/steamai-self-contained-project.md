...
未接入目录在 init 前没有项目级 `/steamai` skill，不能从目录内凭空启动自包含流程。Windows source-clone-first 入口是在 canonical clone 内执行 `go build -trimpath -o .\steamai-bootstrap.exe .\cmd\rekit`，再运行 `steamai-bootstrap.exe bootstrap -target <existing-project> -goal <original-goal> -pack <mature-pack>`。这个文件仍是 build-info 绑定的 unified `cmd/rekit` image，不是 installer，也不进入 PATH。它复用 daily directory-adoption owner：`initialize-in-place` 只生成 canonical init preview，交互模式完整展示后只有精确输入 `APPLY` 才在同一进程内部携带 `ExpectedInitPlanSHA256` 执行 `confirm-exact-plan`；用户不填写 SHA。其它输入或 EOF 均取消，`-format json` 固定 preview-only。确认前零写入、零 Claude launch；stale hash、existing collision、partial state、state-root conflict、symlink/junction/reparse 或 source/target drift 均 fail-closed。Apply 同一次调用只返回 `ready-to-continue`、manifest-bound typed original-goal continuation 与 `NoAutoResume=true`，不写 onboarding mission、不自动启动 Claude；下一次用户或主 Agent明确继续原目标后，fresh project-local daily 才消费 continuation。project-local executable不能借该 external-only mode接入另一个目录。Apply 发布项目内 skill、`.steamai` 状态根、verified runtime bundle 和 selected pack，并重验 exact target ownership、manifest inventory及 executable identity；此后才进入日常 `/steamai` 流程，且不依赖 bootstrap image、原 clone、PowerShell runtime、全局 plugin或机器 PATH。
