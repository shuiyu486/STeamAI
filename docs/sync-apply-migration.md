...
- 需要判断历史语义差异时，再读“PowerShell 历史语义基线”和“Go 迁移契约”。
- G3.3 时本文是迁移前预研；G3.4 已补 Go `sync -Apply` 手动路径，Batch 79 已补 `sync -Apply -WhatIf` 非写入 JSON preview；Batch 106 起公共 façade 默认委托 `/rekit sync` review 与 `sync -Apply` 实际写入；Batch 228 起 `sync` / `update` PowerShell fallback 已退休，`REKIT_GO_DISABLE=1` 或 Go delegation 不可用时直接失败。
...
结论：G3.4 已实现 Go `sync -Apply` 内部 helper 与临时 case smoke。Batch 79 起 Go backend 支持 `sync -Apply -WhatIf` 非写入 JSON preview；Batch 106 起 `/rekit sync` review 与 `sync -Apply` 实际写入默认经 PowerShell façade 委托 Go；Batch 228 起文本 `sync -Apply -WhatIf` 与 `REKIT_GO_DISABLE=1` 不再回退 PowerShell，façade 对 `sync` / `update` 输出 no-fallback error 并保持 no-write。
...
- [x] Batch 106 将 `/rekit sync` review 与 `sync -Apply` 实际写入纳入默认 Go façade 委托。
- [x] Batch 228 退休 `sync` / `update` PowerShell fallback；文本 apply what-if 与 `REKIT_GO_DISABLE=1` 只触发 no-fallback error，不写 case。
...
- PowerShell façade 默认委托 `/rekit sync` review、`sync -Apply` 实际写入以及 `sync -Apply -WhatIf -Format json` 非写入 preview 到 Go；Batch 228 起文本 `sync -Apply -WhatIf` 与 `REKIT_GO_DISABLE=1` 均不再回退 PowerShell，而是报 no-fallback error 并保持 no-write。
- Go `sync -Apply` 不执行 promote、不写 pack、不写 authority/confirmed、不执行 heavy-tool。
- 临时 case 中默认 façade apply 与直接 Go apply 后的 managed files、template targets、managed block host、metadata/shim、state hash 内容一致；允许 backup 目录时间戳不同。
- apply 后 Go doctor 与 façade doctor 均通过。
- 失败注入时不静默吞错；已备份文件可用于人工恢复。

## 风险与注意事项

- `sync -Apply` 是实质 case 写入命令；Batch 106 接管实际写入 façade 委托前必须依赖既有 Go apply、parity smoke、backup/state/doctor 验证；Batch 228 后 `REKIT_GO_DISABLE=1` 不再是 sync 止损 fallback，而是 no-fallback guard。不得把 preview 成功等同于 apply 安全。
...
Batch 228 后 `sync` / `update` PowerShell fallback 已退休，本节只保留迁移时用于对齐 Go 行为的历史语义基线。

### 入口与 guard

`rekit/lib/Sync.ps1` 的 `Sync-RekitPack` 曾是 canonical 写入实现：
...
- `sync` 默认 review-only。
- `sync -Apply` 显式写入；Batch 106 起公共 PowerShell façade 默认委托 Go；Batch 228 起 `REKIT_GO_DISABLE=1` 或 Go delegation 不可用时不再走 legacy PowerShell fallback。
...
| S18 | façade/Go apply parity | `sync-apply-parity-smoke.ps1` 用两个临时 case 对比默认 façade 与直接 Go apply/force 后的 managed docs、template、managed block、metadata/shim、sync state；排除 backup timestamp/path 差异，仅验证 backup 存在与 containment，并断言 disabled façade no-fallback。 |
...
| S20 | 默认 façade `sync` review / `sync -Apply` / `sync -Apply -WhatIf -Format json` | 默认委托 Go；文本 `sync -Apply -WhatIf` 与 `REKIT_GO_DISABLE=1` 均报 no-fallback error，不再回退 PowerShell。 |
...
3. 分别生成默认 façade sync review 与直接 Go sync review artifacts。
4. 验证两边关键 action 一致、bounded diff 存在、review 不改 case 目标文件。
5. 删除临时 case。

`rekit/tests/sync-apply-parity-smoke.ps1` 已补 S18 apply parity：创建两份 `_template` 临时 case，构造相同 managed/template/block drift，分别执行默认 façade `sync -Apply` 与直接 Go `sync -Apply` 以及 `-Force` apply，归一化比较 managed docs、template target、managed block、metadata/shim 和 `.rekit/state.json`，并验证两侧 backup 存在、Go/façade doctor 均通过；Batch 228 后 fake backend sentinel 证明默认 façade apply 会委托 Go，而 disabled façade 会报 no-fallback error、不再回退 PowerShell。