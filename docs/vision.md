...
- [ ] Phase 3：网络安全多领域 pack 扩展（Web/API 安全、恶意样本分析、漏洞研究、CTF/靶场、通用 PE unpacking、Android native、OLLVM 等；`web-security`、`malware-analysis`、`vuln-research`、`ctf`、`unpack-pe`、`ollvm` 与 `android-native` 已作为首批安全领域 pack 骨架启动）。
- [ ] Phase 4：工具适配与候选工具路由。
