...
- GitHub Release `v1.0.4` workflow 成功，下载后的 `steamai-windows-amd64.exe`、`steamai-release.json` 与 `SHA256SUMS` 完整核验通过；anonymous latest URL 可用，manifest 绑定 exact tag revision 和 exe SHA-256。`v1.0.4` 相对 `v1.0.3` 只收录已验证的文档、测试 fixture 与维护护栏，不改变产品 runtime。
