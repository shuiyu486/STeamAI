...
func TestRunStatusCaseMissionIncludesExecutionEvidenceReview(t *testing.T) {
	...
	if status.CaseMission.Ready || status.CaseMission.ExecutionEvidenceReviewCount != 1 || len(status.CaseMission.ExecutionEvidenceReview) != 1 || !containsSubstring(status.CaseMission.BlockedLanes, "main (pending-gate,intervention,open-decision)") || len(status.CaseMission.LaneExecutorActions) != 1 || !containsSubstring(status.CaseMission.PendingGates, "action=debug") || len(status.CaseMission.PendingGateHandoffs) != 1 || len(status.CaseMission.AuthorizedGates) != 0 || len(status.CaseMission.AuthorizedGateHandoffs) != 0 || len(status.CaseMission.InterventionHandoffs) != 1 || !containsSubstring(status.CaseMission.OpenDecisions, "candidate: handler") || !containsSubstring(status.CaseMission.Interventions, "manual override") || status.CaseMission.FactCounts == nil || status.CaseMission.Sections == nil {
	...
	for _, expected := range []string{
		...
		"status case mission section event：section=openCandidates index=1 eventId= kind=candidate status=open lane=main subject=handler summary=candidate one action= decision=",
