...
func TestRunStatusCaseMissionIncludesExecutionEvidenceReview(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"status case mission queue：total=6 unblocked=3 blocked=3 requiresReview=6 followUp=4 current=/rekit handoff main",
		...
		"status case mission lane action：lane=main label=main status=open workspace=captures/lanes/main executor=session-main generation=3 ready=false blocked=true pendingGates=1 openInterventions=1 openDecisions=3 resume=/rekit continue main -Executor session-main -ExpectedExecutorGeneration 3 handoff=/rekit handoff main commanderState=needs-reconcile commanderPrimary=/rekit reconcile main -InterventionId int-main-1 -WhatIf",
		...
		"outcomes=1 followThrough=ready-for-evidence-review nextActions=6 reviewRequiredActions=6 currentAction=/rekit handoff main",
		"status case mission evidence review summary action queue：total=6 unblocked=3 blocked=3 requiresReview=6 followUp=4 current=/rekit handoff main",
...
func TestRunOverviewJsonEmitsReadOnlyInventory(t *testing.T) {
	...
	if queue.Summary != "total=6 unblocked=3 blocked=3 requiresReview=6 followUp=4 current=/rekit handoff main" || queue.Counts.Total != 6 || queue.Counts.Unblocked != 3 || queue.Counts.Blocked != 3 || queue.Counts.RequiresReview != 6 || queue.Counts.FollowUp != 4 || queue.CurrentAction == nil || queue.CurrentAction.Command != "/rekit handoff main" || queue.CurrentAction.Source != "executionEvidenceReview" || len(queue.UnblockedActions) != 3 || len(queue.BlockedActions) != 3 || len(queue.ReviewRequiredActions) != 6 || len(queue.FollowUpActions) != 4 {
	...
	if commander.State != "needs-reconcile" || commander.PrimaryCommand != "/rekit reconcile main -InterventionId int-main-1 -WhatIf" || !containsSubstring(commander.FollowUpCommands, "/rekit continue main -Executor session-main -ExpectedExecutorGeneration 3 -WhatIf") || !containsSubstring(commander.Boundary, "do not run continue") {
...
func TestRunHandoffApplyWritesProjectAndLane(t *testing.T) {
	...
	for _, expected := range []string{"state=ready-to-continue source=missionCommanderActions blocked=false requiresReview=false command=`/rekit continue main -Executor session-main -ExpectedExecutorGeneration 1`", "state=ready-to-continue source=missionCommanderActions.followUp blocked=false requiresReview=false command=`/rekit handoff main`", "state=needs-reconcile source=missionCommanderActions blocked=false requiresReview=true command=`/rekit reconcile login -InterventionId evt-human-stop -WhatIf`", "state=needs-reconcile source=missionCommanderActions.followUp blocked=true requiresReview=true command=`/rekit reconcile login -InterventionId evt-human-stop -Apply`", "state=needs-reconcile source=missionCommanderActions.followUp blocked=true requiresReview=true command=`/rekit continue login -Executor session-login -ExpectedExecutorGeneration 2 -WhatIf`", "follow-up is available only after resolving current lane blockers", "run as -WhatIf first; do not continue autonomously while lane remains blocked", "commander primary：`/rekit continue main -Executor session-main -ExpectedExecutorGeneration 1`", "commander follow-up：/rekit handoff main", "commander primary：`/rekit reconcile login -InterventionId evt-human-stop -WhatIf`", "commander follow-up：/rekit continue login -Executor session-login -ExpectedExecutorGeneration 2 -WhatIf", "commander boundary：do not run continue for blocked lanes"} {
		if !strings.Contains(string(text), expected) {
	...
	if lane.ExecutorAction.MissionCommanderAction.State != "needs-reconcile" || lane.ExecutorAction.MissionCommanderAction.PrimaryCommand != "/rekit reconcile login -InterventionId evt-human-stop -WhatIf" || !containsSubstring(lane.ExecutorAction.MissionCommanderAction.FollowUpCommands, "/rekit continue login -Executor session-login -ExpectedExecutorGeneration 2 -WhatIf") || !containsSubstring(lane.ExecutorAction.MissionCommanderAction.Boundary, "do not run continue for blocked lanes") {
		t.Fatalf("lane handoff JSON omitted Mission Commander action consumption: %+v", lane.ExecutorAction.MissionCommanderAction)
	}
	if len(lane.MissionCommanderNextActions) != 4 || lane.MissionCommanderNextActions[0].Lane != "feature-login" || lane.MissionCommanderNextActions[0].Label != "login" || lane.MissionCommanderNextActions[0].Command != "/rekit reconcile login -InterventionId evt-human-stop -WhatIf" || lane.MissionCommanderNextActions[0].Source != "missionCommanderActions" || lane.MissionCommanderNextActions[0].Blocked || !lane.MissionCommanderNextActions[0].RequiresReview || !containsSubstring(lane.MissionCommanderNextActions[0].Reasons, "open-decision") || !containsSubstring(lane.MissionCommanderNextActions[0].Boundary, "do not run continue") || !containsMissionCommanderNextAction(lane.MissionCommanderNextActions, "missionCommanderActions.followUp", "/rekit reconcile login -InterventionId evt-human-stop -Apply", true, true) || !containsMissionCommanderNextAction(lane.MissionCommanderNextActions, "missionCommanderActions.followUp", "/rekit continue login -Executor session-login -ExpectedExecutorGeneration 2 -WhatIf", true, true) || !containsMissionCommanderNextAction(lane.MissionCommanderNextActions, "missionCommanderActions.followUp", "/rekit handoff login", true, true) || !containsSubstring(lane.MissionCommanderNextActions[1].Reasons, "after resolving current lane blockers") || !containsSubstring(lane.MissionCommanderNextActions[2].Reasons, "run as -WhatIf first") {
		t.Fatalf("lane handoff JSON missing Mission Commander next action: %+v", lane.MissionCommanderNextActions)
	}
	assertCLIActionQueue(t, lane.MissionCommanderActionQueue, 4, 1, 3, 4, 3, "/rekit reconcile login -InterventionId evt-human-stop -WhatIf")
	...
	for _, expected := range []string{"review queued", "inspect candidate", "- commander primary command: `/rekit reconcile login -InterventionId evt-human-stop -WhatIf`", "commander follow-up commands:", "/rekit reconcile login -InterventionId evt-human-stop -Apply", "/rekit continue login -Executor session-login -ExpectedExecutorGeneration 2 -WhatIf", "commander boundary:", "do not run continue for blocked lanes"} {
...
func TestRunPlanSubagentsReviewerOrchestrationE2E(t *testing.T) {
	...
	var statusWithSourceReady struct {
		CaseMission struct {
			ReviewerDispatchIntakeHandoffs []reviewerDispatchIntakeCLIItem      `json:"reviewerDispatchIntakeHandoffs"`
			ReviewerDispatchIntakeSummary  reviewerDispatchIntakeSummaryCLIItem `json:"reviewerDispatchIntakeSummary"`
		} `json:"caseMission"`
...
func assertReviewerDispatchIntakeSummary(t *testing.T, label string, summary reviewerDispatchIntakeSummaryCLIItem, total, waiting, ready int, latestShard, latestState string) {
	t.Helper()
	if summary.Total != total || summary.WaitingForReviewerResult != waiting || summary.ReadyForPreview != ready || summary.LatestShardID != latestShard || summary.LatestState != latestState || summary.NextAction == "" || !containsSubstring(summary.Boundary, "summary is read-only") {
...
func writeHandoffFixture(t *testing.T, caseRoot string) {
	...
	writeFactFile(t, factsRoot, "interventions.jsonl", []string{`{"kind":"intervention","eventId":"evt-human-stop","lane":"feature-login","subject":"manual override","summary":"needs human","action":"override","target":"batch-handoff","approvedBy":"lead","scope":"metadata","status":"open","batchId":"batch-handoff"}`})
