...
func releaseCheckPublicFacadeRemovalHasDeletionGate(plan releasecheck.PublicFacadeRemovalPlan, name string) bool {
	for _, gate := range plan.DeletionGates {
		if gate.Name == name && gate.Required && gate.BlocksRemoval && strings.TrimSpace(gate.Gate) != "" && len(gate.InputInventory) > 0 && len(gate.BlockedExecutionSteps) == 2 && slices.Contains(gate.BlockedExecutionSteps, "delete-public-facade") && slices.Contains(gate.BlockedExecutionSteps, "rerun-release-gate") && len(gate.ExitCriteria) == 3 && len(gate.FailureSignals) == 3 && len(gate.EscalationTriggers) == 3 && len(gate.EscalationEvidence) == 3 && len(gate.EscalationRecipients) == 3 && len(gate.EscalationHandoffSteps) == 3 && len(gate.EscalationDecisionOptions) == 3 && len(gate.EscalationRetryConditions) == 3 && len(gate.EscalationStopConditions) == 3 && len(gate.VerificationArtifacts) == 3 && len(gate.RemediationActions) == 3 && len(gate.ValidationCommands) == 8 && slices.Contains(gate.ValidationCommands, "go run ./cmd/rekit -- -Command release-check -Format json") {
			return true
		}
	}
	return false
