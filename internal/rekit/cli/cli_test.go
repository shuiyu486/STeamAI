...
func assertReleaseCheckGoNativePublicSurface(t *testing.T, surface struct {
	...
	if surfaceCounts.Boundaries.Rows != 7 || surfaceCounts.Boundaries.Commands != 19 || surfaceCounts.Boundaries.CountedCommands != 19 || surface.CommandProfileBoundaries[0].Boundary != commands.BoundaryCaseLocalAppend || firstBoundaryCounts.Count != 1 || firstBoundaryCounts.Commands != 1 || strings.Join(surface.CommandProfileBoundaries[1].Commands, ",") != "attach,bootstrap,continue,gate,handoff,init,repair,start" || surface.CommandProfileBoundaries[len(surface.CommandProfileBoundaries)-1].Boundary != commands.BoundaryReadOnly || lastBoundaryCounts.Count != 5 || lastBoundaryCounts.Commands != 5 {
