...
func TestRunPlanSubagentsWritesReviewArtifacts(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"plan-subagents reviewer writeback step：shard=shard-01 step=preview-reviewer-intake",
		"plan-subagents reviewer writeback command binding：shard=shard-01 step=preview-reviewer-intake binding=reviewer-intake-preview kind=reviewer-intake command=`/rekit plan-subagents",
