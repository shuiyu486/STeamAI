...
func releaseCheckPublicFacadeRemovalHasDeletionGate(plan releasecheck.PublicFacadeRemovalPlan, name string) bool {
	for _, gate := range plan.DeletionGates {
		if gate.Name == name && gate.Required && gate.BlocksRemoval && strings.TrimSpace(gate.Gate) != "" && len(gate.InputInventory) > 0 && len(gate.ValidationCommands) == 8 && slices.Contains(gate.ValidationCommands, "go run ./cmd/rekit -- -Command release-check -Format json") {
			return true
		}
	}
	return false
