...
func TestRunStatusCaseMissionIncludesExecutionEvidenceReview(t *testing.T) {
	...
	if decisionHandoff.EventID != "dec-main-1" || decisionHandoff.Kind != "decision" || decisionHandoff.Lane != "main" || decisionHandoff.Subject != "decision subject" || decisionHandoff.Decision != "defer" || decisionHandoff.Reason != "needs review" || decisionHandoff.Target != "decision-alpha" || !strings.Contains(decisionHandoff.WhatIfCommand, `-Subject "decision for decision: decision subject"`) || !strings.Contains(decisionHandoff.WhatIfCommand, `-Decision "<accept|reject|defer|supersede>"`) || !strings.Contains(decisionHandoff.WhatIfCommand, `-Reason "needs review"`) || !strings.Contains(decisionHandoff.WhatIfCommand, `-Related "dec-main-1"`) || !containsSubstring(decisionHandoff.Evidence, "decision ledger event dec-main-1") || !containsSubstring(decisionHandoff.Evidence, "target decision-alpha") || !containsSubstring(decisionHandoff.Evidence, "batchId batch-overview") {
	...
	for _, expected := range []string{
		...
		"status case mission open decision handoff：eventId=cand-main-1 kind=candidate lane=main subject=handler summary=candidate one decision= reason= status=open target=candidate-alpha confidence=high review=/rekit handoff main whatIf=/rekit note -Target \"" + caseRoot + "\" -Pack _template -Kind decision -Lane main -Subject \"decision for candidate: handler\"",
		...
		"status case mission open decision handoff：eventId=dec-main-1 kind=decision lane=main subject=decision subject summary= decision=defer reason=needs review status=open target=decision-alpha confidence= review=/rekit handoff main whatIf=/rekit note -Target \"" + caseRoot + "\" -Pack _template -Kind decision -Lane main -Subject \"decision for decision: decision subject\"",
