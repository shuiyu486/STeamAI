...
func TestRunPlanSubagentsReviewerIntakeCaseLocalProductPathUsesMetadataRuntime(t *testing.T) {
	...
	out.Reset()
	if err := Run([]string{"-Command", "status", "-Lane", "main", "-Format", "json"}, &out); err != nil {
		t.Fatal(err)
	}
	...
	for _, expected := range []string{
		...
		"status Mission Commander first screen：focus=case-lane-choice",
		"status Mission Commander lane choice：lane=feature-review label=review state=ready-to-continue",
		"status Mission Commander lane choice：lane=main label=main state=ready-to-continue",
	...
	out.Reset()
	if err := Run([]string{"-Command", "handoff", "-Lane", "main", "-WhatIf", "-Format", "text"}, &out); err != nil {
		t.Fatal(err)
	}
	for _, expected := range []string{
	...
	out.Reset()
	if err := runHashBoundHandoffApply(t, []string{"-Command", "handoff", "-Lane", "main", "-Apply", "-Format", "json"}, &out); err != nil {
		t.Fatal(err)
	}
	...
	out.Reset()
	if err := Run([]string{"-Command", "continue", "-Lane", "main", "-WhatIf", "-Format", "text"}, &out); err != nil {
		t.Fatal(err)
	}
	for _, expected := range []string{
...
func TestRunPlanSubagentsReviewerPromptRepairDriverRequestConsumerLoop(t *testing.T) {
	...
	driverRequestArgs := func(label string, request *missionCommanderDriverRequestSnapshot, replacements *strings.Replacer) []string {
		...
		if replacements != nil {
			...
			requestCopy.Command = replacements.Replace(command)
		}
	...
	dispatchRequest := requireBlockedMissionCommanderDriverRequest(t, repairedStatus.CaseMission.MissionCommanderActionQueue, repairedItem.ReviewerDispatchRecordCommand)
	if repairedStatus.CaseMission.ReviewerDispatchIntakeActionQueue.CurrentDriverRequest == nil || repairedStatus.CaseMission.ReviewerDispatchIntakeActionQueue.CurrentDriverRequest.Guidance != dispatchRequest.Guidance {
...
func TestRunPlanSubagentsReviewerAdoptionRedispatchDriverRequestConsumerLoop(t *testing.T) {
	...
	dispatchRequest := requireBlockedMissionCommanderDriverRequest(t, dispatchStatus.CaseMission.MissionCommanderActionQueue, dispatchItem.ReviewerDispatchRecordCommand)
	if dispatchStatus.CaseMission.ReviewerDispatchIntakeActionQueue.CurrentDriverRequest == nil || dispatchStatus.CaseMission.ReviewerDispatchIntakeActionQueue.CurrentDriverRequest.Guidance != dispatchRequest.Guidance {
...
func TestRunPlanSubagentsReviewerFailedCompletionRedispatchProductPath(t *testing.T) {
	...
	if pkg.CurrentRunLoopStepID != "spawn-reviewer" || pkg.Current.State != "reviewer-session-failed" || pkg.Current.ReviewerSessionOutcome != "failed" || pkg.Current.ReviewerDispatchID != firstDispatch.DispatchID || pkg.Current.DispatchCommand == "" || pkg.RefreshStatusCommand == "" || pkg.RefreshStatusCommand != failedItem.RefreshStatusCommand || pkg.CurrentDriverRequest == nil || pkg.CurrentDriverRequest.ExpectedReceipt.RefreshStatusCommand != failedItem.RefreshStatusCommand || !containsSubstring(pkg.RunbookSteps, "do not source-capture the failed attempt") || !containsSubstring(pkg.RunbookSteps, "dispatch a new reviewer session") {
		t.Fatalf("failed reviewer operator package omitted redispatch context: item=%+v pkg=%+v", failedItem, pkg)
	}
...
func TestRunPlanSubagentsReviewerOperatorPackageExecutableRunLoopProductPath(t *testing.T) {
	...
	driverRequestCommandArgs := func(label string, request *missionCommanderDriverRequestSnapshot) []string {
		...
		requestCopy.Command = runtimeCommand(command)
		args, ok := missionCommanderDriverRequestCommandCLIArgs(t, &requestCopy)
		if !ok {
			t.Fatalf("%s driver request did not expose executable command: %+v", label, request)
		}
	...
	driverRequestsMatch := func(returned, refreshed *missionCommanderDriverRequestSnapshot) bool {
		if returned == nil || refreshed == nil || returned.Kind != refreshed.Kind || returned.RunLoopStepID != refreshed.RunLoopStepID || returned.CommandExecutable != refreshed.CommandExecutable || returned.Blocked != refreshed.Blocked || returned.RequiresReview != refreshed.RequiresReview || returned.ExpectedReceipt.RefreshStatusCommand != refreshed.ExpectedReceipt.RefreshStatusCommand {
			return false
		}
	...
	dispatchRequest := requireBlockedMissionCommanderDriverRequest(t, status.FirstScreenQueue, pkg.Current.ReviewerDispatchRecordCommand)
	if status.ReviewerQueue.CurrentDriverRequest == nil || status.ReviewerQueue.CurrentDriverRequest.Guidance != dispatchRequest.Guidance {
	...
	completionRequest := requireBlockedMissionCommanderDriverRequest(t, status.ReviewerQueue, summary.OperatorPackage.Current.ReviewerCompletionRecordCommand)
	out.Reset()
...
func TestRunPlanSubagentsReadyReviewerResultsCaseLocalProductPath(t *testing.T) {
	...
	out.Reset()
	if err := Run([]string{"-Command", "status", "-Lane", "feature-review", "-Format", "json"}, &out); err != nil {
		t.Fatal(err)
	}
	...
	out.Reset()
	if err := Run([]string{"-Command", "continue", "-Lane", "feature-review", "-WhatIf", "-Format", "json"}, &out); err != nil {
		t.Fatal(err)
	}
...
func TestRunPlanSubagentsReviewerIntakeEmitsPartialRecoveryJSON(t *testing.T) {
	...
	intakeReviewerResult = func(repoRoot, caseRoot, pack string, opt subagents.ReviewerIntakeOptions, _ *lanemutation.Lease) (subagents.ReviewerIntakeResult, error) {
