...
func TestPublicCommandCatalog(t *testing.T) {
	commands := Public()
	if len(commands) != 32 || !slices.IsSorted(commands) {
		t.Fatalf("unexpected public command catalog: %v", commands)
	}
	for _, command := range []string{"attach", "bootstrap", "complete", "continue", "control", "doctor", "gate", "handoff", "init", "migrate-state", "next-batch", "note", "onboard", "overview", "packs", "plan-subagents", "promote", "reconcile", "reopen", "release-check", "release-run", "repair", "run-current-loop", "run-current-step", "run-driver-step", "run-reviewer-step", "run-reviewer-wave", "start", "status", "sync", "update", "validate"} {
...
func TestPublicCommandProfiles(t *testing.T) {
	...
	for _, command := range []string{Complete, Control, Reopen, Onboard, MigrateState, NextBatch, Sync, Update, Promote, RunCurrentLoop, RunCurrentStep, RunDriverStep, RunReviewerStep, RunReviewerWave} {
		profile := profileMap[command]
	...
	if summary.Total != 32 || summary.ReadOnly != 5 || summary.Mutating != 27 || summary.WritesCase != 24 || summary.WritesKit != 2 || summary.ReviewFirst != 14 || summary.ApplyRequired != 24 || summary.HeavyTool != 0 || summary.AuthorityConfirmed != 0 || summary.Boundaries[BoundaryCaseLocalApply] != 9 || summary.Boundaries[BoundaryCaseLocalReviewWriteback] != 1 || summary.Boundaries[BoundaryCaseLocalReviewFirst] != 12 || summary.Boundaries[BoundaryKitReviewFirst] != 2 || summary.Boundaries[BoundaryLocalValidationReceipt] != 1 || summary.Boundaries[BoundaryReadOnly] != 5 {
		t.Fatalf("unexpected public command profile summary: %+v", summary)
	}
	groups := PublicProfileGroupsBaseline()
	if strings.Join(groups.ReadOnly, ",") != "doctor,packs,release-check,status,validate" || strings.Join(groups.WritesKit, ",") != "next-batch,promote" || strings.Join(groups.ReviewFirst, ",") != "complete,control,migrate-state,next-batch,onboard,promote,reopen,run-current-loop,run-current-step,run-driver-step,run-reviewer-step,run-reviewer-wave,sync,update" || len(groups.HeavyTool) != 0 || len(groups.AuthorityConfirmed) != 0 || len(groups.ByBoundary[BoundaryCaseLocalApply]) != 9 || strings.Join(groups.ByBoundary[BoundaryCaseLocalReviewWriteback], ",") != PlanSubagents || strings.Join(groups.ByBoundary[BoundaryCaseLocalReviewFirst], ",") != "complete,control,migrate-state,onboard,reopen,run-current-loop,run-current-step,run-driver-step,run-reviewer-step,run-reviewer-wave,sync,update" || strings.Join(groups.ByBoundary[BoundaryKitReviewFirst], ",") != "next-batch,promote" || strings.Join(groups.ByBoundary[BoundaryLocalValidationReceipt], ",") != ReleaseRun {
