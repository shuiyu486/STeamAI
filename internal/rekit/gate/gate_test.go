...
func TestRecordExecutionWritesObservationForAuthorizedGate(t *testing.T) {
	...
	if len(result.ExecutionEvidenceReview) != 1 || result.ExecutionEvidenceReview[0].Acknowledgement == nil || result.ExecutionEvidenceReview[0].Acknowledgement.AcknowledgementReviewCommand == "" || result.ExecutionEvidenceReview[0].Acknowledgement.AcknowledgementReviewCommand != result.ExecutionEvidenceReview[0].Acknowledgement.AcceptedPreviewCommand {
		t.Fatalf("execution evidence review omitted acknowledgement review command: %+v", result.ExecutionEvidenceReview)
	}
	ackReviewCommand := result.ExecutionEvidenceReview[0].Acknowledgement.AcknowledgementReviewCommand
	commander := result.MissionCommanderAction
	if commander.State != "ready-for-evidence-review" || commander.PrimaryCommand != ackReviewCommand || !strings.Contains(commander.Prompt, authorized.EventID) || !gateContainsSubstring(commander.FollowUpCommands, "/rekit handoff main") || !gateContainsSubstring(commander.FollowUpCommands, "/rekit continue main -WhatIf") || !gateContainsSubstring(commander.Boundary, "bounded observation evidence") || !gateContainsSubstring(commander.Boundary, "did not execute the heavy tool") || !gateContainsSubstring(commander.Boundary, "no authority/confirmed") {
		t.Fatalf("execution evidence omitted Mission Commander acknowledgement handoff: %+v", commander)
	}
...
func TestRecordExecutionDuplicateDoesNotAppend(t *testing.T) {
	...
	wantHandoff := "/rekit handoff main -WhatIf -Format json"
	if len(second.MissionCommanderNextActions) != 2 || second.MissionCommanderNextActions[0].State != "evidence-already-recorded" || second.MissionCommanderNextActions[0].Source != "executionEvidenceReview" || second.MissionCommanderNextActions[0].Command != wantHandoff || second.MissionCommanderNextActions[1].Command != "/rekit overview" || gateNextActionContainsCommand(second.MissionCommanderNextActions, "/rekit continue") || gateNextActionContainsSource(second.MissionCommanderNextActions, "missionCommanderActions") || !gateNextActionBoundaryContains(second.MissionCommanderNextActions, "did not append observation evidence") {
	...
	assertGateActionQueue(t, second.MissionCommanderActionQueue, 2, 2, 0, 2, 1, wantHandoff)
	if second.ExecutionEvidenceReviewSummary.Total != 1 || second.ExecutionEvidenceReviewSummary.DuplicateCount != 1 || second.ExecutionEvidenceReviewSummary.ReadyForReviewCount != 1 || second.ExecutionEvidenceReviewSummary.LatestCommanderState != "evidence-already-recorded" || second.ExecutionEvidenceReviewSummary.NextActionCount != 2 || second.ExecutionEvidenceReviewSummary.ReviewRequiredActionCount != 2 || second.ExecutionEvidenceReviewSummary.CurrentAction != wantHandoff {
...
func TestValidateAdapterExecutionReportReadOnlyPreflight(t *testing.T) {
  ...
  "summary": "Adapter completed bounded debug run"
	...
	if len(validation.MissionCommanderNextActions) != 2 || validation.MissionCommanderNextActions[0].State != "ready-to-record-evidence" || validation.MissionCommanderNextActions[0].Source != "adapterReportValidation.missionCommanderAction" || validation.MissionCommanderNextActions[0].Command != wantRecord || validation.MissionCommanderNextActions[1].Command != "/rekit handoff main -WhatIf -Format json" || !gateNextActionBoundaryContains(validation.MissionCommanderNextActions, "replace <executor-id>") {
