...
func TestDocumentationProgressiveDisclosureInvariants(t *testing.T) {
	...
	routeCardPattern := `(?:(?:RH|DPC|APC)-[0-9]+|P0P3-C[0-9]+|Batch [0-9]+|路线收口)`
...
func TestPowerShellFacadeFreezeInvariants(t *testing.T) {
	...
	for _, required := range []string{
		...
		"if ($Command -notin @('start','handoff','complete','reopen','continue','reconcile','release-check','release-run','next-batch'))",
	...
	for _, required := range []string{
		...
		"| `run-current-loop` bounded current-loop driver | Go default | façade delegate + no PowerShell fallback |",
