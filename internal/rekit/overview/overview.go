...
func Render(repoRoot, caseRoot, pack string) (string, error) {
	...
	if os.IsNotExist(err) {
		return "", fmt.Errorf("overview requires existing .rekit/board.json; run PowerShell /rekit overview once to initialize the case-local board")
	}
	if err != nil {
		return "", err
	}
	...
	fmt.Fprintf(&out, "自动化模式：%s\n", stringValue(board, "automationMode"))
