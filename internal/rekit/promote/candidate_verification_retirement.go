...
func validateCandidateVerificationRetirementProof(authority candidateDecisionAuthority, artifact candidateVerificationRetirementArtifact) error {
	...
	if proof.SchemaVersion != 1 || proof.Kind != "pack-memory-candidate-decision-verification" || proof.Pack != authority.receipt.Pack || !sameCandidateDecisionPath(proof.CaseRoot, authority.instance.CaseRoot) || proof.PacketHash != authority.packetHash || proof.DecisionHash != authority.decisionHash || proof.ReceiptHash != sha256Hex(authority.receiptBytes) || !sameCandidateDecisionPath(proof.ReceiptPath, authority.receiptPath) || !sameCandidateDecisionPath(proof.VerificationProofPath, artifact.VerificationProofPath) || proof.ProvisionIntentSHA256 != artifact.ProvisionIntentSHA256 || proof.ProvisionReceiptSHA256 != artifact.ProvisionReceiptSHA256 || !sameCandidateDecisionPath(proof.ProvisionIntentPath, artifact.ProvisionIntentPath) || !sameCandidateDecisionPath(proof.ProvisionReceiptPath, artifact.ProvisionReceiptPath) || !proof.IsMutation || !proof.Applied || !proof.Ready || proof.PackDoctorRows <= 0 || proof.FreshDoctorRows <= 0 || proof.AttachedDoctorRows <= 0 || !reflect.DeepEqual(proof.VerifiedActions, authority.receipt.Actions) || len(artifact.Roots) != 2 || !sameCandidateDecisionPath(proof.FreshCaseRoot, artifact.Roots[0].CaseRoot) || !sameCandidateDecisionPath(proof.AttachedCaseRoot, artifact.Roots[1].CaseRoot) {
...
func loadCandidateVerificationRetirementEvidence(authority candidateDecisionAuthority, proofPath, intentPath, receiptPath, freshRoot, attachedRoot string) ([]byte, []byte, []byte, candidateVerificationProvisionReceipt, error) {
	...
	if proof.SchemaVersion != 1 || proof.Kind != "pack-memory-candidate-decision-verification" || proof.Pack != authority.receipt.Pack || !sameCandidateDecisionPath(proof.CaseRoot, authority.instance.CaseRoot) || !sameCandidateDecisionPath(proof.FreshCaseRoot, freshRoot) || !sameCandidateDecisionPath(proof.AttachedCaseRoot, attachedRoot) || proof.PacketHash != authority.packetHash || proof.DecisionHash != authority.decisionHash || proof.ReceiptHash != sha256Hex(authority.receiptBytes) || !sameCandidateDecisionPath(proof.ReceiptPath, authority.receiptPath) || !sameCandidateDecisionPath(proof.VerificationProofPath, proofPath) || proof.ProvisionIntentSHA256 != sha256Hex(intentBytes) || proof.ProvisionReceiptSHA256 != sha256Hex(receiptBytes) || !sameCandidateDecisionPath(proof.ProvisionIntentPath, intentPath) || !sameCandidateDecisionPath(proof.ProvisionReceiptPath, receiptPath) || !proof.IsMutation || !proof.Applied || !proof.Ready || proof.PackDoctorRows <= 0 || proof.FreshDoctorRows <= 0 || proof.AttachedDoctorRows <= 0 || !reflect.DeepEqual(proof.VerifiedActions, authority.receipt.Actions) {
