	...
	return []PublicFacadeRemovalExecutionStep{
		{
			...
			EscalationReopenConditions: []string{
				"reopen when replacement entrypoint evidence expires or contradicts Go-native public surface",
				"reopen when unsupported command diagnostics drift back to facade or PowerShell fallback",
				"reopen when product direction changes replacement UX or public facade retention decision",
			},
			EscalationLedgerEvents: []string{
				"escalation.verify-go-native-alternative.opened records replacement readiness drift and escalation trigger",
				"escalation.verify-go-native-alternative.resolved records retry or stop decision with replacement evidence",
				"escalation.verify-go-native-alternative.reopened records stale replacement evidence or diagnostic drift",
			},
			EscalationStateTransitions: []string{
				"verify-go-native-alternative escalation open-to-routed after trigger and evidence packet are recorded",
				"verify-go-native-alternative escalation routed-to-resolved after owner decision selects retry or stop",
				"verify-go-native-alternative escalation resolved-to-reopened when replacement evidence or diagnostic readiness drifts",
			},
			EscalationBoundaryGuards: []string{
				"escalation must not accept PowerShell runtime or public facade fallback as replacement evidence",
				"escalation must preserve canonical /rekit and direct Go CLI alternatives in the evidence packet",
				"escalation must not unblock deletion while unsupported diagnostic readiness is unproven",
			},
			EscalationAuditChecks: []string{
				"audit confirms escalation evidence excludes PowerShell runtime and public facade fallback",
				"audit confirms replacement entrypoint decision and unsupported diagnostic transcript are attached",
				"audit confirms retry or stop disposition is reflected before dependent steps proceed",
			},
		...
		},
		{
			...
			EscalationReopenConditions: []string{
				"reopen when new public facade references or unclassified references appear",
				"reopen when documentationTargets[] drift from the approved reference disposition table",
				"reopen when preserved historical references become user-facing default path guidance",
			},
			EscalationLedgerEvents: []string{
				"escalation.migrate-public-references.opened records unresolved reference disposition and owner routing",
				"escalation.migrate-public-references.resolved records migration, preservation or stop decision",
				"escalation.migrate-public-references.reopened records new or drifted public facade references",
			},
			EscalationStateTransitions: []string{
				"migrate-public-references escalation open-to-routed after unresolved reference evidence is recorded",
				"migrate-public-references escalation routed-to-resolved after migration, preservation or stop disposition is approved",
				"migrate-public-references escalation resolved-to-reopened when new or drifted public facade references appear",
			},
			EscalationBoundaryGuards: []string{
				"escalation must not silently drop user-facing public facade references",
				"escalation must keep documentationTargets[] and migrationTargets[] synchronized",
				"escalation must require removalImpact.unclassifiedReferences to remain zero before retry or closure",
			},
			EscalationAuditChecks: []string{
				"audit confirms every public facade reference has rewritten, preserved or stop disposition",
				"audit confirms documentationTargets[] and migrationTargets[] match the approved disposition table",
				"audit confirms removalImpact.unclassifiedReferences remains zero after escalation closure",
			},
			BoundaryGuards: []string{
		...
		},
		{
			...
			EscalationReopenConditions: []string{
				"reopen when facade-smoke.ps1 returns to default release gate or CI paths",
				"reopen when smokeMigrationTargets[] gain unresolved facade compatibility dependency",
				"reopen when Go-native replacement coverage or explicit retirement record is removed",
			},
			EscalationLedgerEvents: []string{
				"escalation.retire-facade-smoke.opened records facade smoke dependency or coverage gap",
				"escalation.retire-facade-smoke.resolved records replacement coverage, retirement or compatibility decision",
				"escalation.retire-facade-smoke.reopened records facade smoke return to default validation paths",
			},
			EscalationStateTransitions: []string{
				"retire-facade-smoke escalation open-to-routed after facade smoke dependency or coverage gap is recorded",
				"retire-facade-smoke escalation routed-to-resolved after replacement coverage, retirement or compatibility decision is approved",
				"retire-facade-smoke escalation resolved-to-reopened when facade smoke returns to default validation paths",
			},
			EscalationBoundaryGuards: []string{
				"escalation must not return facade-smoke.ps1 to default release gate or CI paths",
				"escalation must require Go-native coverage or explicit compatibility decision before retry",
				"escalation must preserve smokeMigrationTargets[] dispositions before deletion is unblocked",
			},
			EscalationAuditChecks: []string{
				"audit confirms facade-smoke.ps1 is absent from default release gate and CI paths",
				"audit confirms Go-native smoke coverage, retirement approval or compatibility decision is attached",
				"audit confirms smokeMigrationTargets[] dispositions remain visible before deletion",
			},
			BoundaryGuards: []string{
		...
		},
		{
			...
			EscalationReopenConditions: []string{
				"reopen when deletion diff gains unrelated schema, heavy-tool or external-effect changes",
				"reopen when restore-public-facade recovery path or revert owner becomes unavailable",
				"reopen when independent removal batch authorization expires or is contradicted",
			},
			EscalationLedgerEvents: []string{
				"escalation.delete-public-facade.opened records deletion boundary, recovery or authorization blocker",
				"escalation.delete-public-facade.resolved records isolated deletion authorization or retained-facade stop decision",
				"escalation.delete-public-facade.reopened records mixed diff, missing recovery or boundary drift",
			},
			EscalationStateTransitions: []string{
				"delete-public-facade escalation open-to-routed after deletion boundary, recovery or authorization blocker is recorded",
				"delete-public-facade escalation routed-to-resolved after isolated deletion authorization or retained-facade stop decision is approved",
				"delete-public-facade escalation resolved-to-reopened when mixed diff, missing recovery or boundary drift appears",
			},
			EscalationBoundaryGuards: []string{
				"escalation must keep deletion isolated to the independent revertable removal batch",
				"escalation must require restore-public-facade recovery path and revert owner evidence",
				"escalation must not authorize schema migration, heavy-tool action or external effect with deletion",
			},
			EscalationAuditChecks: []string{
				"audit confirms independent removal batch authorization or retained-facade stop decision is recorded",
				"audit confirms deletion diff remains isolated and restore-public-facade recovery evidence is attached",
		...
		},
		{
			...
			EscalationReopenConditions: []string{
				"reopen when release-check, tests, vet or diff fail after closure",
				"reopen when releaseHandoff.signals[] drift from public facade removal readiness",
				"reopen when CI-only or external validation requirements appear after local closure",
			},
			EscalationLedgerEvents: []string{
				"escalation.rerun-release-gate.opened records failing validation or handoff synchronization drift",
				"escalation.rerun-release-gate.resolved records final validation transcript or release-readiness stop decision",
				"escalation.rerun-release-gate.reopened records post-closure validation or handoff drift",
			},
			EscalationStateTransitions: []string{
				"rerun-release-gate escalation open-to-routed after failing validation or handoff synchronization drift is recorded",
				"rerun-release-gate escalation routed-to-resolved after final validation transcript or release-readiness stop decision is approved",
				"rerun-release-gate escalation resolved-to-reopened when post-closure validation or handoff drift appears",
			},
			EscalationBoundaryGuards: []string{
				"escalation must not close while release-check, tests, vet or diff still fail",
				"escalation must keep releaseHandoff.signals[] synchronized with removal readiness",
				"escalation must not treat CI-only or external validation as locally authorized by default",
			},
			EscalationAuditChecks: []string{
				"audit confirms final release-check, test, vet and diff transcripts are green or stop disposition is recorded",
				"audit confirms releaseHandoff.signals[] are synchronized with public facade removal readiness",
				"audit confirms CI-only or external validation disposition is recorded before closure",
			},
			BoundaryGuards: []string{
...
func publicFacadeRemovalExecutionEscalationReopenConditionCount(steps []PublicFacadeRemovalExecutionStep) int {
	count := 0
	for _, step := range steps {
...
func publicFacadeRemovalExecutionEscalationLedgerEventCount(steps []PublicFacadeRemovalExecutionStep) int {
	count := 0
	for _, step := range steps {
...
func publicFacadeRemovalExecutionEscalationStateTransitionCount(steps []PublicFacadeRemovalExecutionStep) int {
	count := 0
	for _, step := range steps {
...
func publicFacadeRemovalExecutionEscalationBoundaryGuardCount(steps []PublicFacadeRemovalExecutionStep) int {
	count := 0
	for _, step := range steps {
...
func publicFacadeRemovalExecutionEscalationAuditCheckCount(steps []PublicFacadeRemovalExecutionStep) int {
	count := 0
	for _, step := range steps {
