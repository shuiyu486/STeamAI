...
func TestReleaseHandoffInventoryFromRepo(t *testing.T) {
	...
	assertHandoffSignalDetail(t, handoff, "PowerShell deprecation", "fallbackRetirement=true noFallback=24 candidates=0 removalModules=0 retiredModules=13")
	assertHandoffSignalDetail(t, handoff, "PowerShell deprecation", "facadeRuntime=true legacyImports=false dispatcher=false")
	assertHandoffSignalDetail(t, handoff, "PowerShell deprecation", "publicFacade=true retained=true facadeCommands=24 noFallback=24")
	...
	assertHandoffSignalDetail(t, handoff, "Go-native public surface", "default=status commands=24 handlers=24 symbols=24 profiles=24 boundaries=7 alternative=go run ./cmd/rekit -- -Command <command>")
	assertHandoffSignalDetail(t, handoff, "Go-native public surface", "profileSummary total=24 readOnly=6 mutating=18 writesCase=16 writesKit=2 reviewFirst=6 applyRequired=16 heavyTool=0 authorityConfirmed=0")
	assertHandoffSignalDetail(t, handoff, "Go-native public surface", "profileGroups readOnly=doctor,packs,release-check,release-run,status,validate reviewFirst=next-batch,promote,run-driver-step,run-reviewer-step,sync,update writesKit=next-batch,promote")
	assertHandoffSignalDetail(t, handoff, "Go-native public surface", "profileBoundaries rows=7 caseLocalApply=attach,bootstrap,continue,gate,handoff,init,reconcile,repair,start caseLocalReviewWriteback=plan-subagents caseLocalReviewFirst=run-driver-step,run-reviewer-step,sync,update kitReviewFirst=next-batch,promote readOnly=doctor,packs,release-check,release-run,status,validate")
