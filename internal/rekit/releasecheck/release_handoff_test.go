...
func TestReleaseHandoffInventoryFromRepo(t *testing.T) {
	...
	if counts.ReadFirst != 1 || counts.Signals != 13 || counts.KnownGaps == 0 || counts.PackMaturity.Total == 0 || counts.Validation == 0 || counts.NextActions == 0 {
	...
	for _, path := range []string{"docs/real-usage-hardening-roadmap.md", "docs/batch-plan.md", "docs/release-readiness.md", "CHANGELOG.md"} {
		for _, doc := range handoff.ReadFirst {
			if doc.Path == path {
				t.Fatalf("release handoff bypasses the progressive-disclosure router with %s: %+v", path, handoff.ReadFirst)
			}
		}
	}
	assertHandoffSignal(t, handoff, "release-check inventory")
	assertHandoffSignal(t, handoff, "CI release gate")
	assertHandoffSignalDetail(t, handoff, "PowerShell deprecation", "fallbackRetirement=true noFallback=32 candidates=0 removalModules=0 retiredModules=13")
	assertHandoffSignalDetail(t, handoff, "PowerShell deprecation", "facadeRuntime=true legacyImports=false dispatcher=false")
	assertHandoffSignalDetail(t, handoff, "PowerShell deprecation", "publicFacade=true retained=true facadeCommands=32 noFallback=32")
	...
	assertHandoffSignalDetailContains(t, handoff, "public facade removal prerequisites", "migrationTargets=77")
	assertHandoffSignalDetailContains(t, handoff, "public facade removal prerequisites", "migrationValidationCommands=616")
...
func TestReleaseHandoffActiveRouteMissingProjectionFieldFailsClosed(t *testing.T) {
	repo := cleanReleaseRepoRoot(t)
	path := filepath.Join(repo, "docs", "batch-plan.md")
	...
	route := result.ReleaseHandoff.ActiveRoute
	if route.Ready || route.ProjectionConsistent || route.CurrentAction == nil || route.CurrentAction.ActionID != "active-route-conflict" || !route.CurrentAction.Blocked || !route.CurrentAction.RequiresReview {
		t.Fatalf("invalid route projection did not fail closed: %+v", route)
	}
	if result.ReleaseHandoff.NextBatchSelectionPackage != nil {
		t.Fatalf("invalid route projection exposed next-batch selection: %+v", result.ReleaseHandoff.NextBatchSelectionPackage)
	}
}
...
func TestReleaseHandoffCompletedRouteWithInvalidProjectionFailsClosed(t *testing.T) {
	...
	route := result.ReleaseHandoff.ActiveRoute
	if route.NextBatchUnlocked || route.CurrentAction == nil || route.CurrentAction.ActionID != "active-route-conflict" || !route.CurrentAction.Blocked || result.ReleaseHandoff.NextBatchSelectionPackage != nil {
		t.Fatalf("invalid completed route did not fail closed: route=%+v package=%+v", route, result.ReleaseHandoff.NextBatchSelectionPackage)
	}
}
...
`, "- Batch 816 fixture note.\n\n")
	...
	if len([]rune(latestBatch.ValidationResult)) > 240 {
		t.Fatalf("public latest validation should remain compact: %q", latestBatch.ValidationResult)
	}
...
func writeReleaseHandoffLatestBatchFixture(t *testing.T, repo, batchSection, changelogEntry string) {
	...
	if batchStart := strings.Index(tail, "### Batch "); batchStart >= 0 {
		if validationStart := strings.Index(tail[batchStart:], "## 验证标准"); validationStart >= 0 {
			tail = tail[:batchStart] + tail[batchStart+validationStart:]
		}
	}
...
`)
	...
	if inventory.MissionCommanderActionQueue.CurrentAction == nil || inventory.MissionCommanderActionQueue.Counts.Total != 2 || inventory.MissionCommanderActionQueue.Counts.RequiresReview != 2 || inventory.MissionCommanderActionQueue.Counts.FollowUp != 1 || len(inventory.MissionCommanderNextActions) != 2 || inventory.MissionCommanderNextActions[0].Label != "fixture" || inventory.MissionCommanderNextActions[0].ActionID != "pack-memory-decision-proof-required" || inventory.MissionCommanderNextActions[0].State != "pack-memory-proof-required" || !strings.Contains(inventory.MissionCommanderNextActions[0].Command, "CreateCandidates -Review") || inventory.MissionCommanderNextActions[1].Source != "packMemoryCandidates.fixture.followUp.proof" || !strings.Contains(inventory.MissionCommanderNextActions[1].Command, "-DraftReviewProof") || !strings.Contains(inventory.MissionCommanderNextActions[1].Command, "-ProofDecision") || !releaseHandoffStringsContain(inventory.MissionCommanderNextActions[0].Reasons, "actionId=pack-memory-decision-proof-required") || !releaseHandoffStringsContain(inventory.MissionCommanderNextActions[0].Boundary, "read-only handoff") {
...
func releaseHandoffRunLoopContains(steps []mission.MissionCommanderRunLoopStep, stepID string) bool {
...
func releaseHandoffRunLoopBoundaryContains(steps []mission.MissionCommanderRunLoopStep, want string) bool {
	for _, step := range steps {
