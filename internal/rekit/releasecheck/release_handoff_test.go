...
func TestReleaseHandoffInventoryFromRepo(t *testing.T) {
	...
	assertHandoffSignalDetail(t, handoff, "PowerShell deprecation", "fallbackRetirement=true noFallback=25 candidates=0 removalModules=0 retiredModules=13")
	assertHandoffSignalDetail(t, handoff, "PowerShell deprecation", "facadeRuntime=true legacyImports=false dispatcher=false")
	assertHandoffSignalDetail(t, handoff, "PowerShell deprecation", "publicFacade=true retained=true facadeCommands=25 noFallback=25")
	...
	assertHandoffSignalDetail(t, handoff, "Go-native public surface", "default=status commands=25 handlers=25 symbols=25 profiles=25 boundaries=7 alternative=go run ./cmd/rekit -- -Command <command>")
	assertHandoffSignalDetail(t, handoff, "Go-native public surface", "profileSummary total=25 readOnly=6 mutating=19 writesCase=17 writesKit=2 reviewFirst=7 applyRequired=17 heavyTool=0 authorityConfirmed=0")
	assertHandoffSignalDetail(t, handoff, "Go-native public surface", "profileGroups readOnly=doctor,packs,release-check,release-run,status,validate reviewFirst=next-batch,promote,run-current-step,run-driver-step,run-reviewer-step,sync,update writesKit=next-batch,promote")
	assertHandoffSignalDetail(t, handoff, "Go-native public surface", "profileBoundaries rows=7 caseLocalApply=attach,bootstrap,continue,gate,handoff,init,reconcile,repair,start caseLocalReviewWriteback=plan-subagents caseLocalReviewFirst=run-current-step,run-driver-step,run-reviewer-step,sync,update kitReviewFirst=next-batch,promote readOnly=doctor,packs,release-check,release-run,status,validate")
