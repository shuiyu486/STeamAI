...
func TestGoNativePublicSurfaceInventoryFromRepo(t *testing.T) {
	...
	if surfaceCounts.SymbolCatalog.Symbols != 30 || surfaceCounts.SymbolCatalog.EmptySymbols != 0 || surfaceCounts.SymbolCatalog.EmptyCommands != 0 || inventory.SymbolCommands["NextBatch"] != "next-batch" || inventory.SymbolCommands["PlanSubagents"] != "plan-subagents" || inventory.SymbolCommands["Reconcile"] != "reconcile" || inventory.SymbolCommands["ReleaseCheck"] != "release-check" || inventory.SymbolCommands["ReleaseRun"] != "release-run" || inventory.SymbolCommands["RunCurrentLoop"] != "run-current-loop" || inventory.SymbolCommands["RunCurrentStep"] != "run-current-step" {
	...
	if strings.Join(inventory.CommandProfileGroups.ReadOnly, ",") != "doctor,packs,release-check,release-run,status,validate" || strings.Join(inventory.CommandProfileGroups.ReviewFirst, ",") != "complete,next-batch,onboard,promote,reopen,run-current-loop,run-current-step,run-driver-step,run-reviewer-step,run-reviewer-wave,sync,update" || strings.Join(inventory.CommandProfileGroups.WritesKit, ",") != "next-batch,promote" || surfaceCounts.Groups.HeavyTool != 0 || surfaceCounts.Groups.AuthorityConfirmed != 0 || surfaceCounts.Groups.CaseLocalApply != 9 || surfaceCounts.Groups.CaseLocalReviewWriteback != 1 || surfaceCounts.Groups.CaseLocalReviewFirst != 10 {
...
func assertFallbackRetirement(t *testing.T, inventory PowerShellDeprecation) {
	...
	if counts.GoDefaultCommands != 30 || counts.NoFallbackCommands != 30 || counts.CandidateCommands != 0 || counts.RemovalCandidateModules != 0 || counts.RetiredModules != 13 {
...
func assertPublicFacade(t *testing.T, inventory PowerShellDeprecation) {
	...
	if counts.CommandSurface != 30 || counts.GoDefaultCommands != 30 || counts.NoFallbackCommands != 30 {
