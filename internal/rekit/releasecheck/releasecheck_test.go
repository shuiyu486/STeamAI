...
func TestGoNativePublicSurfaceInventoryFromRepo(t *testing.T) {
	...
	if surfaceCounts.Catalog.Commands != 25 || surfaceCounts.Catalog.Empty != 0 || surfaceCounts.Catalog.Duplicates != 0 || coverageCounts.Commands != 25 || coverageCounts.HandlerCommands != 25 || coverageCounts.SymbolCommands != 25 || coverageCounts.ProfileCommands != 25 || coverageCounts.CommandProfiles != 25 || coverageCounts.HandlerMissing != 0 || coverageCounts.HandlerUnknown != 0 || coverageCounts.SymbolMissing != 0 || coverageCounts.SymbolUnknown != 0 || coverageCounts.ProfileMissing != 0 || coverageCounts.ProfileUnknown != 0 || surfaceCounts.MutationBoundaryInventory.Rows != 7 || surfaceCounts.MutationBoundaryInventory.Unknown != 0 {
		t.Fatalf("Go-native public surface omitted expected command coverage: %+v", inventory)
	}
	for _, command := range []string{"attach", "bootstrap", "continue", "doctor", "gate", "handoff", "init", "next-batch", "note", "overview", "packs", "plan-subagents", "promote", "reconcile", "release-check", "release-run", "repair", "run-current-step", "run-driver-step", "run-reviewer-step", "start", "status", "sync", "update", "validate"} {
	...
	if surfaceCounts.SymbolCatalog.Symbols != 25 || surfaceCounts.SymbolCatalog.EmptySymbols != 0 || surfaceCounts.SymbolCatalog.EmptyCommands != 0 || inventory.SymbolCommands["NextBatch"] != "next-batch" || inventory.SymbolCommands["PlanSubagents"] != "plan-subagents" || inventory.SymbolCommands["Reconcile"] != "reconcile" || inventory.SymbolCommands["ReleaseCheck"] != "release-check" || inventory.SymbolCommands["ReleaseRun"] != "release-run" || inventory.SymbolCommands["RunCurrentStep"] != "run-current-step" {
	...
	if surfaceCounts.ProfileCatalog.Rows != 25 || surfaceCounts.ProfileCatalog.Empty != 0 || surfaceCounts.ProfileCatalog.Duplicates != 0 || surfaceCounts.ProfileCatalog.UnknownBoundaries != 0 || surfaceCounts.ProfileCatalog.HeavyTool != 0 || surfaceCounts.ProfileCatalog.AuthorityConfirmed != 0 || surfaceCounts.ProfileCatalog.WritesKitNoReview != 0 || surfaceCounts.ProfileCatalog.ReviewNoApply != 0 || profiles["release-check"].MutationBoundary != commands.BoundaryReadOnly || profiles["release-check"].IsMutation || profiles["release-run"].MutationBoundary != commands.BoundaryReadOnly || profiles["release-run"].IsMutation || profiles["next-batch"].MutationBoundary != commands.BoundaryKitReviewFirst || !profiles["next-batch"].WritesKit || !profiles["next-batch"].ReviewFirst || !profiles["next-batch"].ApplyRequired || profiles["reconcile"].MutationBoundary != commands.BoundaryCaseLocalApply || !profiles["reconcile"].WritesCase || !profiles["reconcile"].ApplyRequired || profiles["run-current-step"].MutationBoundary != commands.BoundaryCaseLocalReviewFirst || !profiles["run-current-step"].WritesCase || !profiles["run-current-step"].ReviewFirst || !profiles["run-current-step"].ApplyRequired || !profiles["promote"].WritesKit || !profiles["promote"].ReviewFirst || profiles["sync"].WritesKit || !profiles["sync"].WritesCase || !slices.Contains(inventory.MutationBoundaries, commands.BoundaryKitReviewFirst) {
	...
	if profileSummaryCounts.Total != 25 || surfaceCounts.ProfileTotal != 25 || profileSummaryCounts.ReadOnly != 6 || profileSummaryCounts.Mutating != 19 || profileSummaryCounts.WritesCase != 17 || profileSummaryCounts.WritesKit != 2 || profileSummaryCounts.ReviewFirst != 7 || profileSummaryCounts.ApplyRequired != 17 || profileSummaryCounts.HeavyTool != 0 || profileSummaryCounts.AuthorityConfirmed != 0 || profileSummaryCounts.BoundaryReadOnly != 6 || profileSummaryCounts.BoundaryCaseLocalApply != 9 || profileSummaryCounts.BoundaryCaseLocalWriteback != 1 || profileSummaryCounts.BoundaryCaseLocalReview != 5 || profileSummaryCounts.BoundaryKitReview != 2 {
		t.Fatalf("Go-native public command profile summary drifted: %+v", inventory.CommandProfileSummary)
	}
	if strings.Join(inventory.CommandProfileGroups.ReadOnly, ",") != "doctor,packs,release-check,release-run,status,validate" || strings.Join(inventory.CommandProfileGroups.ReviewFirst, ",") != "next-batch,promote,run-current-step,run-driver-step,run-reviewer-step,sync,update" || strings.Join(inventory.CommandProfileGroups.WritesKit, ",") != "next-batch,promote" || surfaceCounts.Groups.HeavyTool != 0 || surfaceCounts.Groups.AuthorityConfirmed != 0 || surfaceCounts.Groups.CaseLocalApply != 9 || surfaceCounts.Groups.CaseLocalReviewWriteback != 1 || surfaceCounts.Groups.CaseLocalReviewFirst != 5 {
	...
	if surfaceCounts.Boundaries.Rows != 7 || surfaceCounts.Boundaries.Commands != 25 || surfaceCounts.Boundaries.CountedCommands != 25 || surfaceCounts.Boundaries.Unknown != 0 || surfaceCounts.Boundaries.Duplicates != 0 || surfaceCounts.Boundaries.CountMismatches != 0 || surfaceCounts.Boundaries.Unsorted != 0 || surfaceCounts.Boundaries.SummaryMismatches != 0 || surfaceCounts.Boundaries.GroupMismatches != 0 || surfaceCounts.Boundaries.Missing != 0 || surfaceCounts.Boundaries.CoverageMismatches != 0 || inventory.CommandProfileBoundaries[0].Boundary != commands.BoundaryCaseLocalAppend || firstBoundaryCounts.Count != 1 || firstBoundaryCounts.Commands != 1 || strings.Join(inventory.CommandProfileBoundaries[1].Commands, ",") != "attach,bootstrap,continue,gate,handoff,init,reconcile,repair,start" || strings.Join(inventory.CommandProfileBoundaries[4].Commands, ",") != "run-current-step,run-driver-step,run-reviewer-step,sync,update" || strings.Join(inventory.CommandProfileBoundaries[5].Commands, ",") != "next-batch,promote" || inventory.CommandProfileBoundaries[len(inventory.CommandProfileBoundaries)-1].Boundary != commands.BoundaryReadOnly || lastBoundaryCounts.Count != 6 || lastBoundaryCounts.Commands != 6 {
...
func assertFallbackRetirement(t *testing.T, inventory PowerShellDeprecation) {
	...
	if counts.GoDefaultCommands != 25 || counts.NoFallbackCommands != 25 || counts.CandidateCommands != 0 || counts.RemovalCandidateModules != 0 || counts.RetiredModules != 13 {
...
func assertPublicFacade(t *testing.T, inventory PowerShellDeprecation) {
	...
	if counts.CommandSurface != 25 || counts.GoDefaultCommands != 25 || counts.NoFallbackCommands != 25 {
