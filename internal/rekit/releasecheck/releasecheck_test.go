...
func assertPublicFacadeRemoval(t *testing.T, inventory PublicFacadeRemoval) {
	...
	if !inventory.RemovalPlan.Ready || inventory.RemovalPlan.Document != "docs/powershell-deprecation.md" || len(inventory.RemovalPlan.RequiredPhrases) != 9 || len(inventory.RemovalPlan.ExecutionSteps) != 5 || publicFacadeRemovalExecutionValidationCommandCount(inventory.RemovalPlan.ExecutionSteps) != 40 || len(inventory.RemovalPlan.RecoverySteps) != 4 || publicFacadeRemovalRecoveryValidationCommandCount(inventory.RemovalPlan.RecoverySteps) != 32 || len(inventory.RemovalPlan.DocumentationTargets) != 9 || publicFacadeRemovalDocumentationValidationCommandCount(inventory.RemovalPlan.DocumentationTargets) != 72 || !publicFacadeRemovalHasExecutionStep(inventory.RemovalPlan, "delete-public-facade") || !publicFacadeRemovalHasExecutionStep(inventory.RemovalPlan, "rerun-release-gate") || !publicFacadeRemovalHasRecoveryStep(inventory.RemovalPlan, "restore-public-facade") || !publicFacadeRemovalHasDocumentationTarget(inventory.RemovalPlan, "docs/release-readiness.md") || !publicFacadeRemovalHasDocumentationTarget(inventory.RemovalPlan, "CHANGELOG.md") {
