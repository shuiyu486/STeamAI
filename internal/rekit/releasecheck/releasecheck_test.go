...
func TestGoNativePublicSurfaceInventoryFromRepo(t *testing.T) {
	...
	if surfaceCounts.Catalog.Commands != 28 || surfaceCounts.Catalog.Empty != 0 || surfaceCounts.Catalog.Duplicates != 0 || coverageCounts.Commands != 28 || coverageCounts.HandlerCommands != 28 || coverageCounts.SymbolCommands != 28 || coverageCounts.ProfileCommands != 28 || coverageCounts.CommandProfiles != 28 || coverageCounts.HandlerMissing != 0 || coverageCounts.HandlerUnknown != 0 || coverageCounts.SymbolMissing != 0 || coverageCounts.SymbolUnknown != 0 || coverageCounts.ProfileMissing != 0 || coverageCounts.ProfileUnknown != 0 || surfaceCounts.MutationBoundaryInventory.Rows != 7 || surfaceCounts.MutationBoundaryInventory.Unknown != 0 {
	...
	if surfaceCounts.SymbolCatalog.Symbols != 28 || surfaceCounts.SymbolCatalog.EmptySymbols != 0 || surfaceCounts.SymbolCatalog.EmptyCommands != 0 || inventory.SymbolCommands["NextBatch"] != "next-batch" || inventory.SymbolCommands["PlanSubagents"] != "plan-subagents" || inventory.SymbolCommands["Reconcile"] != "reconcile" || inventory.SymbolCommands["ReleaseCheck"] != "release-check" || inventory.SymbolCommands["ReleaseRun"] != "release-run" || inventory.SymbolCommands["RunCurrentLoop"] != "run-current-loop" || inventory.SymbolCommands["RunCurrentStep"] != "run-current-step" {
	...
	if profileSummaryCounts.Total != 28 || surfaceCounts.ProfileTotal != 28 || profileSummaryCounts.ReadOnly != 6 || profileSummaryCounts.Mutating != 22 || profileSummaryCounts.WritesCase != 20 || profileSummaryCounts.WritesKit != 2 || profileSummaryCounts.ReviewFirst != 10 || profileSummaryCounts.ApplyRequired != 20 || profileSummaryCounts.HeavyTool != 0 || profileSummaryCounts.AuthorityConfirmed != 0 || profileSummaryCounts.BoundaryReadOnly != 6 || profileSummaryCounts.BoundaryCaseLocalApply != 9 || profileSummaryCounts.BoundaryCaseLocalWriteback != 1 || profileSummaryCounts.BoundaryCaseLocalReview != 8 || profileSummaryCounts.BoundaryKitReview != 2 {
		t.Fatalf("Go-native public command profile summary drifted: %+v", inventory.CommandProfileSummary)
	}
	if strings.Join(inventory.CommandProfileGroups.ReadOnly, ",") != "doctor,packs,release-check,release-run,status,validate" || strings.Join(inventory.CommandProfileGroups.ReviewFirst, ",") != "complete,next-batch,promote,run-current-loop,run-current-step,run-driver-step,run-reviewer-step,run-reviewer-wave,sync,update" || strings.Join(inventory.CommandProfileGroups.WritesKit, ",") != "next-batch,promote" || surfaceCounts.Groups.HeavyTool != 0 || surfaceCounts.Groups.AuthorityConfirmed != 0 || surfaceCounts.Groups.CaseLocalApply != 9 || surfaceCounts.Groups.CaseLocalReviewWriteback != 1 || surfaceCounts.Groups.CaseLocalReviewFirst != 8 {
	...
	if surfaceCounts.Boundaries.Rows != 7 || surfaceCounts.Boundaries.Commands != 28 || surfaceCounts.Boundaries.CountedCommands != 28 || surfaceCounts.Boundaries.Unknown != 0 || surfaceCounts.Boundaries.Duplicates != 0 || surfaceCounts.Boundaries.CountMismatches != 0 || surfaceCounts.Boundaries.Unsorted != 0 || surfaceCounts.Boundaries.SummaryMismatches != 0 || surfaceCounts.Boundaries.GroupMismatches != 0 || surfaceCounts.Boundaries.Missing != 0 || surfaceCounts.Boundaries.CoverageMismatches != 0 || inventory.CommandProfileBoundaries[0].Boundary != commands.BoundaryCaseLocalAppend || firstBoundaryCounts.Count != 1 || firstBoundaryCounts.Commands != 1 || strings.Join(inventory.CommandProfileBoundaries[1].Commands, ",") != "attach,bootstrap,continue,gate,handoff,init,reconcile,repair,start" || strings.Join(inventory.CommandProfileBoundaries[4].Commands, ",") != "complete,run-current-loop,run-current-step,run-driver-step,run-reviewer-step,run-reviewer-wave,sync,update" || strings.Join(inventory.CommandProfileBoundaries[5].Commands, ",") != "next-batch,promote" || inventory.CommandProfileBoundaries[len(inventory.CommandProfileBoundaries)-1].Boundary != commands.BoundaryReadOnly || lastBoundaryCounts.Count != 6 || lastBoundaryCounts.Commands != 6 {
...
func assertFallbackRetirement(t *testing.T, inventory PowerShellDeprecation) {
	...
	if counts.GoDefaultCommands != 28 || counts.NoFallbackCommands != 28 || counts.CandidateCommands != 0 || counts.RemovalCandidateModules != 0 || counts.RetiredModules != 13 {
...
func assertPublicFacade(t *testing.T, inventory PowerShellDeprecation) {
	...
	if counts.CommandSurface != 28 || counts.GoDefaultCommands != 28 || counts.NoFallbackCommands != 28 {
