...
func TestWritePlanIncludesShardHandoffs(t *testing.T) {
	...
	if result.ReviewerOrchestration.Mode != "dispatch-only-unattached-target" || result.ReviewerOrchestration.ReviewerCount != 2 || result.ReviewerOrchestration.MaxParallel != 5 || len(result.ReviewerOrchestration.Dispatches) != 2 || len(result.ReviewerOrchestration.Lifecycle) != 5 || result.ReviewerOrchestration.Dispatches[0].ShardID != "shard-01" || result.ReviewerOrchestration.BatchPreviewCommand != "" || result.ReviewerOrchestration.BatchApplyCommand != "" || !strings.Contains(result.ReviewerOrchestration.Lifecycle[0].Action, "does not spawn") {
