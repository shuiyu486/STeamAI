...
func TestWritePlanIncludesShardHandoffs(t *testing.T) {
	...
	if managed == nil || managed.Mode != "dispatch-only-unattached-target" || managed.TargetLane != "devirt-main" || managed.PacketPath != result.PacketPath || managed.PromptRoot != result.Observability.PromptRoot || managed.ResultRoot != result.Observability.ResultRoot || managed.ReviewerCount != 2 || managed.MaxParallel != 5 || len(managed.Dispatches) != 2 || len(managed.Runbook) == 0 || !slices.Contains(managed.Boundary, "managed dispatch packet is read-only handoff; runtime does not spawn, stop, monitor, or manage reviewer sessions") {
		t.Fatalf("unexpected managed dispatch packet: %+v", managed)
	}
	managedFirst := managed.Dispatches[0]
	if managedFirst.ShardID != "shard-01" || managedFirst.PromptPath != result.ShardHandoffs[0].DispatchPromptPath || managedFirst.PromptSHA256 != result.ShardHandoffs[0].DispatchPromptSHA256 || managedFirst.AgentToolRequest.PromptPath != result.ShardHandoffs[0].DispatchPromptPath || !managedFirst.AgentToolRequest.ReadOnly || managedFirst.ReviewerResultPath != result.ShardHandoffs[0].ReviewerResultPath || managedFirst.ReviewerResultInputPath != "" || managedFirst.CollectionPreviewCommand != "" || !strings.HasPrefix(managedFirst.IntakePreviewCommand, "n/a:") || !strings.Contains(managedFirst.DispatchCommand, "retain the returned JSON") || !strings.Contains(managedFirst.NextAction, "regenerate a canonical case-local packet") || !strings.Contains(managedFirst.ReviewerResultSkeleton, "packet.packetId") {
		t.Fatalf("unexpected first managed dispatch: %+v", managedFirst)
	}
	...
	if result.ReviewerOrchestration.Summary.Mode != result.ReviewerOrchestration.Mode || result.ReviewerOrchestration.Summary.DispatchCount != 2 || result.ReviewerOrchestration.Summary.IntakeAvailable || result.ReviewerOrchestration.Summary.CollectionAvailable || !result.ReviewerOrchestration.Summary.DispatchOnly || result.ReviewerOrchestration.Summary.FirstDispatch == nil || result.ReviewerOrchestration.Summary.FirstDispatch.ShardID != "shard-01" || !strings.Contains(result.ReviewerOrchestration.Summary.FirstDispatch.DispatchCommand, "dispatch read-only reviewer for shard-01") || result.ReviewerOrchestration.Summary.ManagedDispatchSummary == nil || result.ReviewerOrchestration.Summary.ManagedDispatchSummary.FirstDispatch == nil || !strings.Contains(result.ReviewerOrchestration.Summary.ManagedDispatchSummary.FirstDispatch.NextAction, "regenerate a canonical case-local packet") || !slices.Contains(result.ReviewerOrchestration.Summary.Boundary, "runtime only writes review artifacts and does not spawn, stop, monitor, or manage reviewer sessions") {
	...
	if packet.ReviewerOrchestration.Mode != result.ReviewerOrchestration.Mode || packet.ReviewerOrchestration.PacketPath != result.PacketPath || packet.ReviewerOrchestration.Summary.DispatchCount != result.ReviewerOrchestration.Summary.DispatchCount || packet.ReviewerOrchestration.Summary.FirstDispatch == nil || packet.ReviewerOrchestration.ManagedDispatchPacket == nil || packet.ReviewerOrchestration.ManagedDispatchPacket.Dispatches[0].PromptSHA256 != result.ReviewerOrchestration.ManagedDispatchPacket.Dispatches[0].PromptSHA256 || packet.ReviewerOrchestration.Summary.ManagedDispatchSummary == nil || packet.ReviewerOrchestration.Dispatches[0].ReviewerResultPath != packet.ShardHandoffs[0].ReviewerResultPath || packet.ReviewerOrchestration.MissionCommanderAction == nil || len(packet.ReviewerOrchestration.MissionCommanderNextActions) != len(result.MissionCommanderNextActions) || packet.ReviewerOrchestration.MissionCommanderActionQueue == nil || packet.ReviewerOrchestration.MissionCommanderActionQueue.Summary != result.MissionCommanderActionQueue.Summary {
...
func TestWritePlanBindsAttachedCaseLaneExecutor(t *testing.T) {
	...
	managedFirst := managed.Dispatches[0]
	if managedFirst.ShardID != "shard-01" || managedFirst.PromptPath != result.ShardHandoffs[0].DispatchPromptPath || managedFirst.PromptSHA256 != result.ShardHandoffs[0].DispatchPromptSHA256 || managedFirst.ReviewerResultInputPath != result.ShardHandoffs[0].ReviewerStagingCommands.SourceCaptureInput || managedFirst.ReviewerResultSourcePath != result.ShardHandoffs[0].ReviewerStagingCommands.SourcePath || managedFirst.ReviewerResultCandidatePath != result.ShardHandoffs[0].ReviewerResultCandidatePath || managedFirst.SourceCapturePreviewCommand == "" || managedFirst.SourceCaptureApplyCommand == "" || managedFirst.StagingPreviewCommand == "" || managedFirst.CollectionPreviewCommand == "" || managedFirst.CollectionApplyCommand == "" || !strings.Contains(managedFirst.IntakePreviewCommand, "-WhatIf -Format json") || !strings.Contains(managedFirst.IntakeApplyCommand, "-Apply -Format json") || !strings.Contains(managedFirst.NextAction, "sourceCapturePreviewCommand") {
		t.Fatalf("attached case first managed dispatch omitted canonical command chain: %+v", managedFirst)
	}
