...
func TestQuarantineReviewerResultRejectsRecreatedCanonicalBytes(t *testing.T) {
	...
	if err := quarantineReviewerResult(root, resultPath, quarantinePath, expected); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(resultPath, expected, 0o644); err != nil {
		t.Fatal(err)
	}
	if err := quarantineReviewerResult(root, resultPath, quarantinePath, expected); err == nil || !strings.Contains(err.Error(), "cannot prove") {
