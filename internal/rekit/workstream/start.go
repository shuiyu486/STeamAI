...
type workstreamMutationLease struct {
...
func acquireLaneMutationLock(caseRoot, laneID string) (*workstreamMutationLease, error) {
	...
	return acquireWorkstreamMutationLock(caseRoot, laneID)
}

func acquireProjectMutationLock(caseRoot string) (*workstreamMutationLease, error) {
	return acquireWorkstreamMutationLock(caseRoot, "")
}

func acquireWorkstreamMutationLock(caseRoot, laneID string) (*workstreamMutationLease, error) {
	...
	lease := &workstreamMutationLease{metadataRoot: metadataRoot, metadataPath: metadataPath, metadataInfo: metadataInfo, unlockFile: unlockLaneLeaseFile}
	...
	fail := func(cause error) (*workstreamMutationLease, error) {
	...
	closeLockRoot := func(cause error) (*workstreamMutationLease, error) {
	...
	if err := validateWorkstreamLeaseIdentity(lease); err != nil {
		return fail(err)
	}
	if err := lease.Validate(); err != nil {
		return fail(err)
...
func validateWorkstreamLeaseIdentity(lease *workstreamMutationLease) error {
	if lease.stableCaseFile != nil {
...
func (lease *workstreamMutationLease) Validate() error {
...
func (lease *workstreamMutationLease) Unlock() error {
	if lease == nil || lease.metadataRoot == nil {
		return nil
	}
	var errs []error
