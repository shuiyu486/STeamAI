...
func TestNativeCapabilityContractKeepsVisibleSessionDefault(t *testing.T) {
	...
	for _, required := range []string{
		...
		"相互独立的 HOLD、研究往返、容量/恢复可以拆成短 gate 分次验收",
		"不要求一个外部控制器同步驱动全部窗口",
...
func researchFixtureReplacements() map[string]string {
	return map[string]string{
		...
		"{{REVIEW_ID}}":                          "R-001",
		"{{REVIEWER}}":                           "reviewer",
		"{{FINDING_OWNER}}":                      "static-analysis",
		"{{FINDING_REF}}":                        "../findings/F-001.md",
		"{{FINDING_SHA256}}":                     strings.Repeat("b", 64),
		"{{REVIEW_ROUND}}":                       "1",
		"{{PREVIOUS_REVIEW_ROUND_OR_NONE}}":      "none",
		"{{REVIEWED_EVIDENCE_REFS_WITH_SHA256}}": "- ../evidence/E-001.md — " + strings.Repeat("c", 64),
		...
		"{{SOURCE_REVIEW_REF}}":                  "../../reviews/R-001.md",
		"{{SOURCE_REVIEW_SHA256}}":               strings.Repeat("e", 64),
		"{{PACK_RELATIVE_PATH}}":                 "references/binary-re/general-analysis.md",
		"{{PACK_NAME}}":                          "binary-re",
		"{{PACK_REVISION}}":                      "test-fixture-revision",
		"{{PACK_TREE}}":                          "test-fixture-pack-tree",
		"{{COMMON_TREE}}":                        "test-fixture-common-tree",
		"{{SNAPSHOT_DIGEST}}":                    "sha256:" + strings.Repeat("d", 64),
