...
- 按需入口：`references/binary-re/README.md`。
- 当前任务接手：`references/binary-re/task-handoff.md`。
- 工具链路由：`references/binary-re/toolchain-router.md`。
- 复核专项：`references/binary-re/singleton-handler-review.md`。
- 上下文管理：`references/binary-re/progressive-disclosure.md`。
