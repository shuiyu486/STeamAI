# CTF and lab challenge research route

`ctf` 是已授权 CTF、靶场、课程实验与本地 challenge 的声明式研究 pack。它只提供 challenge triage、solver hypothesis、local reproduction 与 writeup review 的按需 reference、tooling recipe 与安全边界，不执行研究动作。
...
- challenge_ref、artifact_ref、solver_ref 使用 case-local 脱敏引用；finding 必须引用 evidence，review 必须引用 finding/evidence。
