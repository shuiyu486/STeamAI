# Authorized vulnerability research route

`vuln-research` 是已授权漏洞研究、防御性复现、补丁与崩溃分析的声明式研究 pack。它只提供 crash triage、root-cause hypothesis、repro evidence 与 patch analysis 的按需 reference、tooling recipe 与安全边界，不执行研究动作。
...
- target_ref、repro_ref、crash_ref、patch_ref 使用 case-local 脱敏引用；finding 必须引用 evidence，review 必须引用 finding/evidence。
