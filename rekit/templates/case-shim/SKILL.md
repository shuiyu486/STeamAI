...
6. 新会话 first screen 先使用 `/rekit`，确认 `status case shim ready=true`、`installedShimMatchesTemplate=true`，再按 `status case mission` 的 queue/current action 与 durable artifacts 接手；active reviewer/evidence/adapter review blocker 优先于普通 handoff/continue follow-up；project handoff 的 pack-memory 与 next-batch action queues 用于无旧聊天上下文的 durable takeover；已记录 execution evidence 后，终结性的 related verification / decision note 会闭合 review queue，避免 exact `evidence-already-recorded` current action 回流；如果 shim drift，先走 repair preview，不直接修复；默认 `/rekit` 还会先显示 compact Mission Commander first-screen strip，空 mission/缺 board 时先投影 onboarding current action，帮助快速定位 case current action、pack-memory current action 与 next-batch takeover action。
...
- `overview/continue/start/handoff` 是当前推荐的用户日常入口；主 Agent / harness 可按 canonical skill 使用 `run-current-step` 推进一步，或使用 bounded `run-current-loop` 在固定 initial route/lane 内持续推进。优先让 canonical runtime 处理，不要在 shim 中复制逻辑；loop、route、status refresh、nested runner、fallback 与命令执行细节均由 canonical runtime 持有。
