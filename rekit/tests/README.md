...
| `promote-candidates-apply-smoke.ps1` / `go test ./internal/rekit/promote` | 改 Go candidate 写入、façade candidate 写入委托或 promote package helper | 默认 façade candidate 写入、candidate/index/tooling candidate、deny、sanitization、unique candidate path、restore helper、pack-root containment、pack-memory promote → reviewed recipe merge → fresh case reconsume、cleanup。 |
...
| 脚本 | 什么时候跑 | 覆盖重点 |
|---|---|---|
| `plan-subagents-smoke.ps1` / `go test ./internal/rekit/subagents` / `go test ./internal/rekit/cli -run TestRunPlanSubagentsReviewerIntake` | 改 `plan-subagents`、review packet/result contract 或 reviewer intake/writeback | 当前 PowerShell script 覆盖 planning route/sharding/artifacts/no-write/out-of-case/no-fallback；Go tests覆盖 strict reviewer result、packet/route/shard/items/evidence binding、route output contract与顶层 binding、blocked fail-closed、WhatIf no-write、Apply verification-before-decision、deterministic duplicate retry 与 postValidation。PowerShell script 尚未覆盖 intake WhatIf/Apply/duplicate/blocked/postValidation；在补齐前不要把这些行为列为该 script 已覆盖。 |
