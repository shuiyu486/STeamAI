...
| 脚本 | 什么时候跑 | 覆盖重点 |
|---|---|---|
| `facade-smoke.ps1` | 改 `rekit.ps1`、Go façade 委托集合或 JSON preview/read-only 委托 | 只读命令、release-check inventory、已退休 fallback、default delegation / disabled no-fallback 不加载 legacy runtime modules，以及 workstream/lifecycle/sync/promote/note/gate 默认 Go 委托；当前还覆盖 `plan-subagents` reviewer intake preview/apply 的 `PacketPath`、`ReviewerResultPath`、`Lane`、`Actor`、WhatIf/Apply、Format 转发，`ReviewerResultPath` cross-command guard 与 disabled reviewer-intake no-fallback。相对 packet/result path 还会由 capturing fake backend 验证为按 façade 调用者工作目录规范化。 |
...
| `gate-parity-smoke.ps1` / `go test ./internal/rekit/cli -run 'TestRunGo(GateDispatchE2EPlanGateOverviewHandoff|GenericBinaryPackNeutralE2EStartPlanGateOverviewHandoff|WebSecurityPackNeutralE2EStartPlanGateOverviewHandoff|GateApplyAppendsAuthorizedGateRequestVisibility)'` | 改 heavy-tool gate request schema、gate execution evidence schema、gate execution report contract、gate façade 委托、PowerShell 读层、gate/dispatch 闭环或 pack-neutral gate 展示 | 默认 façade gate what-if no-write、默认 façade gate apply request decision 写入 + overview/note/handoff 展示 parity、`REKIT_GO_DISABLE=1` no-fallback 边界；Go package E2E 额外覆盖 `_template` pack pending-gate request 在 Go overview/handoff 中的可见性、`generic-binary-re` non-feature lane pending-gate 的 overview/handoff 可见性、`web-security` network pending-gate 的 overview/handoff 可见性、authorized-gate 在 Mission brief、overview JSON/text、project/lane handoff 与 continue-facing handoff artifacts 中的非阻塞可见性，`gate -ExecutionReportContract -GateEventId ... -Format json` 只读输出 adapter execution report contract（含 validation failure stages/codes），`gate -ValidateExecutionReport -GateEventId ... -ExecutionReportPath ... -Format json` 只读 strict validation bounded adapter report，返回 `valid=true/false` non-mutating envelope（invalid 含 failureCode/failureStage）且不写 observations ledger，以及 `gate -Apply -GateEventId ... -ExecutionStatus ... -ExecutionReportPath ...` 写 observation execution evidence、保留 actual budget/output refs/evidence refs、strict intake bounded adapter report（包含 boundary/escalation marker fail-closed validation）且不写 authority/confirmed；profile evaluation 由 `internal/rekit/gate` 和 `internal/rekit/mission` package tests 锁定。 |
