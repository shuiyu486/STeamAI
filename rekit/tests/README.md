...
| 脚本 | 什么时候跑 | 覆盖重点 |
|---|---|---|
| `facade-smoke.ps1` | 改 `rekit.ps1`、Go façade 委托集合或 JSON preview/read-only 委托 | 只读命令、release-check inventory、已退休 fallback、default delegation / disabled no-fallback 不加载 legacy runtime modules，以及 workstream/lifecycle/sync/promote/note/gate 默认 Go 委托；当前还覆盖 `gate -ExecutionReportContract` / `gate -ValidateExecutionReport` 在 nested authorized output workspace cwd 中省略 `-Target` 的 façade product path 与 workspace-relative sidecar validation，`plan-subagents` reviewer intake preview/apply 的 `PacketPath`、`ReviewerResultPath`、`Lane`、`Actor`、WhatIf/Apply、Format 转发，`ReviewerResultPath` cross-command guard 与 disabled reviewer-intake no-fallback。相对 packet/result path 还会由 capturing fake backend 验证为按 façade 调用者工作目录规范化。 |
