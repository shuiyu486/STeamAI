...
| `agent-team-review-loop-smoke.ps1` / `go test ./internal/rekit/cli -run TestRunGoReviewerDecisionE2ENoteOverviewHandoff` | 改 review loop、verification/decision 展示、note list 委托、note append façade 或 reviewer/decision 闭环 | packet -> note what-if no-write -> verification append -> decision append -> note/list -> overview/handoff 最小闭环，含默认 façade note append/what-if、note list 文本/JSON 委托与 disable fallback；Go package E2E 额外覆盖 `_template` pack candidate → verification → decision → overview/handoff 可见性链路。 |
