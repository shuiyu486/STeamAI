...
`rekit/tests` 里的脚本都是仓库维护验证入口，默认使用临时 case 或只读仓库状态，用于锁定 review-first、planning/no-write、Go façade parity、remaining PowerShell compatibility 和 pack maturity/smoke 边界。Batch 353 的 strict reviewer intake 目前主要由 Go package tests与一次真实本机 reviewer E2E验证；PowerShell smoke 的具体覆盖和缺口按下表如实记录。`catalog.json` 用相同分类记录全部 `*.ps1` smoke/helper 的 `category`、`purpose`、`recommendedFor`、`supportsWorkRoot` 和 `riskBoundary`，供后续自动测试选择器或 CI 读取。

Go-first release gate 优先由 Go-owned `release-check` inventory（完整 release audit）、Go-native lightweight daily `status` / `packs` / `doctor`、focused `TestRunGoSkeletonPackSmokeMatrix`、`go test -count=1 -p=2 -timeout=30m ./...` 与 `go vet ./...` 捕获确定性 invariant；focused matrix 会真实遍历全部 schema-valid skeleton pack，并显式保留 manifest-mature `web-security` production vertical slice smoke，验证 doctor、exact init preview→Apply、case doctor、route packet、promote review、planning no-workstream-write，以及 start 后由 alias 选 lane 的 handoff preview→exact `applyArgs[]` Apply。测试函数名是既有 CI compatibility，不代表 production pack admission 已由 smoke 完成；统一 adapter/fixture/verifier/instruction-consumption 四要素仍需单独 release contract。完整 tests 中`-count=1`禁用 package test-result cache，保证当前 frozen bytes 被 fresh 执行但保留 Go build cache；`-p=2`限制跨 package并发；30分钟是逐 package test binary上限，`release-run`另以45分钟硬上限收口整条命令；`status` 只构建 Mission Control 所需 project handoff，不替代 full release audit。默认远程 CI 见`.github/workflows/release-gate.yml`，在 Linux、Windows、macOS 上先运行 `go vet`，再显式运行 focused Go-native pack smoke 与完整 Go tests；`facade-smoke.ps1`、`catalog-smoke.ps1`、`pack-smoke-matrix-selftest.ps1` 与 PowerShell pack matrix 保留为按需 compatibility / parity 层，不继续扩张成新的 runtime owner。
...
- pack smoke 必须覆盖 Go/PowerShell doctor、Go init、case doctor、`plan-subagents` route packet、promote review managed-doc candidate、planning no-write，以及 handoff preview 返回的 exact `applyArgs[]` 可原样 Apply；exact action 必须保留 pack、durable lane、hash 与 stamp。
- façade smoke 必须证明默认 Go delegation、所有 public command/profile（包括 `onboard`）的 no-fallback 边界、显式 Go enable 和 Go disable 优先级；retained PowerShell façade 只能委托 Go，不得加载 legacy runtime module、执行 PowerShell 业务 fallback，或自行解析/重建 `onboard` mission intent、publication stamp/hash。未来删除公共 façade 前，`publicFacadeRemoval.removalImpact.migrationTargets[]` 中列出的全部 public façade references 必须完成 required / Go-native preferred 迁移目标处理，`smokeMigrationTargets[]` 中列出的 façade compatibility / dependent smoke 必须迁移到 Go-native preferred 或显式退休 façade assertions。
...
| `pack-smoke-matrix.ps1 -DiscoveryOnly` | 新增/删除 skeleton pack、提升/降级 production pack 或 pack smoke wrapper | inventory 中全部 schema-valid skeleton 加显式 production smoke pack 与 matrix/catalog/wrapper 一致性。 |
| `pack-smoke-matrix.ps1` | 需要全量 pack smoke 回归 | 串行运行全部 expected smoke pack（当前为 6 个 skeleton + mature `web-security` production smoke）。 |
| `pack-smoke-matrix.ps1 -Packs web-security,malware-analysis -Format json` | 需要快速子集或机器可读结果 | 子集 pack smoke + JSON envelope。 |
...
这些脚本是 thin wrapper，复用 `pack-smoke-lib.ps1`；其中 6 个 skeleton 自动进入 discovery，`web-security` 作为显式保留的 production smoke：
...
| 脚本 | 什么时候跑 | 覆盖重点 |
|---|---|---|
| `init-bootstrap-smoke.ps1` | 改 Go init/bootstrap、case scaffold 或 lifecycle façade 委托 | 兼容/维护 init/bootstrap 的 preview/apply、managed docs、template、managed block、state、doctor、默认 façade 委托、disabled no-fallback；不代表 public `onboard` intent publication 覆盖。 |
...
| `promote-candidates-apply-smoke.ps1` / `go test ./internal/rekit/promote` / focused `go test ./internal/rekit/releasecheck ./internal/rekit/cli -run "TestReleaseHandoffPackMemoryCandidatesDetectsOpenResidue|TestRunStatusKitShowsOpenPackMemoryCandidates"` | 改 Go candidate 写入、façade candidate 写入委托、promote package helper 或 release/status pack-memory proof handoff | 默认 façade candidate 写入、candidate/index/tooling candidate、deny、sanitization、reviewPlan merge/reject/cleanup/reconsume guidance、terminal reviewSummary proofSummary / proof progress / current stage / next-missing proof handoff 与 next-missing proof detail（when/action/format/evidence/boundary）、unique candidate path、restore helper、pack-root containment、pack-memory promote → reviewed recipe merge → fresh case reconsume、cleanup，以及 release/status expected proof paths、present/missing proof summary、proof progress、current stage、next-missing proof handoff 与同类 proof detail。 |
...
| 脚本 | 什么时候跑 | 覆盖重点 |
|---|---|---|
| `go test ./internal/rekit/onboarding ./internal/rekit/missionintent ./internal/rekit/cli -run 'Test(PreviewApplyReplayAndIdentityDrift|ApplyRecoversPartialExactPublication|RunOnboard)' -count=1` / `facade-smoke.ps1` | 改 public `onboard`、immutable mission intent、publication recovery/replay 或 façade passthrough | Go tests锁定显式 `Target` / `Pack` / `ProjectName` / opaque bounded `Goal` / `Actor` / `Executor` / `InitialLane`、WhatIf zero-write、exact stamp/hash、机器可读 `applyArgs[]`、intent-first / commit-last、live kit/pack 漂移后仍从 bounded durable envelope exact recovery、跨 case artifact transplant 拒绝、同一 Inspect 的 mission/intent/commit 共享持续 pinned `.rekit` / `onboarding` namespace且跨 artifact目录切换 fail-closed、committed replay、status onboarding route及 no board/lane/heavy/authority/session boundary；façade smoke只验证参数薄透传与 disabled no-fallback，不在 PowerShell 重建 intent/hash。 |
| `go test ./internal/rekit/commands ./internal/rekit/mission ./internal/rekit/workstream ./internal/rekit/cli ./internal/rekit/hostcmd ./internal/rekit/sessionhost -run 'Test(QualifyPublicNextAction|PromoteContinuePreviewToApply|MissionCommanderActionQueue|BindMissionCommanderNextActionAuthorityContinueCommands|RunDriverStepContinueProductPath|RunDailyMissionControlRouteSmokeProductPath|ResolveDailyRequest|RunDailyLaneSelectorDoesNotBecomeControl)' -count=1` | 改 public action typed owner、daily intent、status quickstart 或 continue preview/Apply bridge | 锁定 daily resume/goal/correction/control/adoption 单一 owner、lane 仅作 selector、bare public continue 只提升为 JSON preview、preview 保留 selector/owner/generation 的 exact Apply、command/invocation parity、Apply 后 fresh preview、selected lane/current-legacy entrypoint 与 owner guard。 |
| `go test -count=1 -p=2 -timeout=30m ./internal/rekit/cli ./cmd/rekit -run 'Test(ParsePublicOptions|PublicInvocationRecognizesOnlyUserCommands|RunPublicHelpDoesNotDispatchRuntime|PublicSummaryKeepsDiagnosticsOutOfDefaultOutput|PublicDiagnosticsPreservesTypedJSON|InvocationModeRoutesUnifiedExecutable|ProjectLocalExecutablePublicHelpStatusAndDiagnostics)'` | 改 project-local executable 的 no-mode `help/status/continue`、默认摘要或 diagnostics 分层 | 锁定 public parser allowlist、维护 flag 拒绝、project-local binding、默认“现在/原因/下一步”、`--diagnostics` typed JSON、current 不泄漏 executable `/rekit`，以及 `continue` preview-only/no mutation/no Claude launch。 |
| `go test ./internal/rekit/cli -run '^TestRunDailyMissionControlRouteSmokeProductPath$' -count=1` | 改 case onboarding、status quickstart、continue/reconcile、handoff 或 replacement executor takeover 日常路线 | 单个临时 case 的 Go-native route smoke：missing-board status → typed onboarding → status/quickstart continue WhatIf → preview result returned typed Apply → 读回 durable run receipt/artifacts → fresh status 重新发布 preview → open intervention → quickstart reconcile WhatIf+Apply → status refresh → typed project handoff WhatIf+Apply → explicit lane handoff WhatIf + returned typed Apply → fresh lane-scoped durable takeover artifact → 新会话 status；同时锁定 preview no-write、returned typed request、refresh route、run artifacts 与 no authority/confirmed。它不执行 heavy-tool、不 spawn session，也不新增 PowerShell runtime owner。 |
| `go test ./internal/rekit/cli -run '^TestRunDriverStep' -count=1` / `facade-smoke.ps1` | 改 `run-driver-step`、current driver request parser、plan hash/receipt/status refresh或 façade passthrough | Go tests覆盖 focused case `start → continue → reconcile → continue` product path、outer WhatIf no-write、typed Apply request、stale hash fail-closed、single matching Apply、三类 mutation 的 preview currentness guard、refreshed status/receipt identity、no authority/confirmed，以及 missing-board onboarding/gate/handoff/unknown flag/duplicate phase拒绝；façade smoke覆盖 public Go-only/no-fallback委托与参数透传。runner只接受 focused `start`、`continue`、`reconcile`；plan hash覆盖deterministic preview，Apply会在lane mutation lease内再次校验preview SHA；不调用shell、不管理session、不执行reviewer/adapter/heavy-tool。 |
| `plan-subagents-smoke.ps1` / `go test ./internal/rekit/subagents ./internal/rekit/reviewersession ./internal/rekit/externalsession` / `go test ./internal/rekit/cli -run 'Test(RunPlanSubagentsReviewer(Intake|Orchestration)|RunCurrentLoopStopsForExternalReviewerHandoffs)'` | 改 `plan-subagents`、review packet/result contract、Remote Reviewer transport 或 reviewer intake/writeback | 当前 PowerShell script 覆盖 planning route/sharding/artifacts/no-write/out-of-case/no-fallback；Go tests覆盖 strict reviewer result、packet/route/shard/items/evidence binding、route output contract与顶层 binding、packet-owned canonical prompt gate、transport source/transported SHA 与 bytes 双身份、bounded compaction、blocked fail-closed、WhatIf no-write、Apply verification-before-decision、deterministic duplicate retry、postValidation，以及 dispatch 后 status/handoff/continue、continue run `status.json` / `digest.md`、lane `RESUME.md` 与 checkpoint 的 open reviewer result path、intake commands、waiting / intake-ready state、packet-level progress（completed/open/nextOpen/remaining）与完成后清零 handoff；reviewer intake top-level `summary` 与 `postValidation.summary` 的 JSON/text 还会直接输出 compact `orchestrationProgress` 与 `reviewerWritebackSummary`，覆盖 multi-reviewer dispatch completed/open/nextOpen/remaining progress、latest reviewer result、packet/route/shard/session、owner binding、risk/conflict/route output/evidence refs 与 boundary 的 terminal handoff；blocked reviewer intake top-level `summary.repairGuidanceSummary` / text 直接输出 primary repair reason/action、evidence、boundary 与下一条 safe command。PowerShell script 尚未覆盖 intake WhatIf/Apply/duplicate/blocked/postValidation 或 downstream durable dispatch-intake handoff/progress；在补齐前不要把这些行为列为该 script 已覆盖。 |
| `agent-team-review-loop-smoke.ps1` / `go test ./internal/rekit/cli -run 'TestRunGo(ReviewerDecisionE2ENoteOverviewHandoff|GenericBinaryPackNeutralE2EStartPlanGateOverviewHandoff|WebSecurityPackNeutralE2EStartPlanGateOverviewHandoff)'` | 改 review loop、verification/decision 展示、note list 委托、note append façade、reviewer/decision 闭环或 pack-neutral ledger 闭环 | packet -> note what-if no-write -> verification append -> decision append -> note/list -> overview/handoff 最小闭环，含默认 façade note append/what-if、note list 文本/JSON 委托与 `REKIT_GO_DISABLE=1` no-fallback 边界；Go package E2E 额外覆盖 `_template` pack candidate → verification → decision → overview/handoff 可见性链路、`binary-re` non-feature lane 的 candidate/verification/decision → overview/handoff 可见性链路，以及 `web-security` endpoint candidate/verification/decision → overview/handoff 可见性链路。 |
...
| `binary-re-agent-team-dryrun-smoke.ps1` | 改 binary-re Agent Team dry-run、binary-analysis route、debug gate、真实临时 case flow 或 pack-neutral handoff | 公共 `/rekit` façade 下的临时 case：`init -Apply`、`start -Apply`、`plan-subagents` review packet、candidate/verification/decision `note` append、`gate -WhatIf/-Apply -Action debug`、`note -List`、`overview` text/JSON、lane `handoff -Apply` 与 `doctor`；不自动 spawn agent、不执行样本/debug/trace/dump/patch、不写 authority/confirmed。 |
...
| `continue-whatif-smoke.ps1` / `go test ./internal/rekit/cli -run 'Test(RunGoWorkstreamE2EStartNoteContinueHandoff|RunDriverStepContinueProductPath|RunDailyMissionControlRouteSmokeProductPath)'` | 改 Go continue preview/apply、public typed owner、continue JSON/text preview/apply façade 委托或 Go workstream 闭环 | non-write typed preview、`continuePlanSha256` 完整 snapshot binding、preview result returned exact `-ExpectedContinuePlanSha256` Apply、blocked preview 不发布/接受 Apply、case-local writes、Apply 后 fresh preview、command/invocation parity、selector/owner/generation binding、default façade delegation、blocked actions 与 authority guard；Go product path 额外覆盖 `_template` start → note → continue Apply → handoff。 |
...
| 改动类型 | 推荐验证 |
|---|---|
| 新增或提升 pack | 单 pack smoke -> `pack-smoke-matrix.ps1 -DiscoveryOnly` -> `pack-inventory-smoke.ps1` -> `go test -count=1 -p=2 -timeout=30m ./...` -> `doctor`；提升 mature pack 时还要确认 production smoke allowlist 与 maturity 一致。 |
| 改测试导航 catalog | `catalog-smoke.ps1` -> `pack-smoke-matrix.ps1 -DiscoveryOnly` -> `pack-inventory-smoke.ps1`。 |
| 改 pack smoke helper/matrix/discovery | `pack-smoke-matrix-selftest.ps1` -> `pack-smoke-matrix.ps1 -DiscoveryOnly` -> 1-2 个代表 pack smoke -> `pack-inventory-smoke.ps1`。 |
