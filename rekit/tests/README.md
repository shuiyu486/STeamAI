...
| `go test ./internal/rekit/cli -run '^TestRunDriverStep' -count=1` / `facade-smoke.ps1` | 改 `run-driver-step`、current driver request parser、plan hash/receipt/status refresh或 façade passthrough | Go tests覆盖 focused case `continue` 的 outer WhatIf no-write、typed Apply request、stale hash fail-closed、single matching Apply、refreshed status/receipt identity、no authority/confirmed，以及 onboarding/gate/unknown flag/duplicate phase拒绝；façade smoke覆盖 public Go-only/no-fallback委托与参数透传。MVP只接受 focused `continue`；plan hash覆盖deterministic preview，Apply会在lane mutation lease内再次校验preview SHA；不调用shell、不管理session、不执行reviewer/adapter/heavy-tool。 |
