...
try {
  $initPreview = Invoke-RekitSmoke -Arguments @('-Command','init','-Target',$caseRoot,'-Pack',$Pack,'-ProjectName',"ctf-dryrun-$suffix",'-WhatIf','-Format','json') | ConvertFrom-Json
  $initApplyArgs = @($initPreview.applyArgs | ForEach-Object { [string]$_ })
  if ($initApplyArgs.Count -eq 0) { throw 'ctf dry-run init preview omitted applyArgs' }
  ...
  if ([string]$candidate.command -ne 'note' -or -not [bool]$candidate.applied -or [string]$candidate.path -ne '.steamai/facts/candidates.jsonl' -or [string]$candidate.event.kind -ne 'candidate' -or [string]$candidate.event.lane -ne 'challenge-analysis-pwn') {
  ...
  $handoffPreview = Invoke-RekitSmoke -Arguments @('-Command','handoff','-Target',$caseRoot,'-Pack',$Pack,'-WhatIf','-Format','json','pwn') | ConvertFrom-Json
  $handoffApplyArgs = @($handoffPreview.applyArgs | ForEach-Object { [string]$_ })
  if ([string]::IsNullOrWhiteSpace([string]$handoffPreview.publicationPlanSha256) -or [string]::IsNullOrWhiteSpace([string]$handoffPreview.publicationStamp) -or $handoffApplyArgs.Count -eq 0) { throw 'ctf handoff preview omitted exact Apply action' }
  ...
  $handoffPath = Join-Path $caseRoot '.steamai\handovers\challenge-analysis-pwn-latest.md'
  ...
  foreach ($expected in @('challenge-analysis-pwn','/rekit continue -Lane challenge-analysis-pwn','workspace/challenges/challenge-analysis-pwn/packet.md','## verification','verifier=manual-review','## decision','decision=accept','## pending-gate','action=network','scope=single lab challenge connection only')) {
