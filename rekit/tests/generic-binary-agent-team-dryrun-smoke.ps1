...
$caseRoot = Join-Path $WorkRoot "generic-binary-agent-team-dryrun-$suffix"
$reviewRoot = Join-Path $WorkRoot "generic-binary-agent-team-review-$suffix"

try {
  Invoke-RekitSmoke -Arguments @('-Command','init','-Target',$caseRoot,'-Pack',$Pack,'-ProjectName',"generic-binary-dryrun-$suffix",'-Apply') | Out-Null

  $start = Invoke-RekitSmoke -Arguments @('-Command','start','-Target',$caseRoot,'-Pack',$Pack,'-Name','sample','-Apply','-Format','json') | ConvertFrom-Json
  if ([string]$start.command -ne 'start' -or -not [bool]$start.isMutation -or -not [bool]$start.applied -or [string]$start.lane.id -ne 'binary-analysis-sample' -or [string]$start.lane.type -ne 'binary-analysis' -or [string]$start.lane.workspace -ne 'workspace/binary/binary-analysis-sample') {
    throw "unexpected generic-binary start result: $($start | ConvertTo-Json -Depth 20)"
  }

  $packetRel = 'workspace\binary\binary-analysis-sample\packet.md'
  ...
  if ([string]$packet.route.id -ne 'generic-binary-re:binary-analysis' -or [string]$packet.observability.routeDebug.selectedBy -ne 'taskType') {
    throw "unexpected generic-binary route packet: $($packet | ConvertTo-Json -Depth 20)"
  }
  if ([string]$packet.observability.dispatchMode -ne 'manual-main-agent' -or @($packet.observability.shardStatuses).Count -ne 2 -or [string]@($packet.observability.shardStatuses)[0].status -ne 'planned' -or [string]$packet.reviewLoop.spawnOwner -ne 'main-agent') {
    throw "unexpected generic-binary dispatch observability: $($packet | ConvertTo-Json -Depth 20)"
  }
  foreach ($expected in @('runtime does not spawn subagents','canonical-write')) {
    Assert-ContainsText -Text (($packet | ConvertTo-Json -Depth 20)) -Expected $expected -Label 'generic-binary dispatch contract'
  }

  $candidate = Invoke-RekitSmoke -Arguments @('-Command','note','-Target',$caseRoot,'-Pack',$Pack,'-Kind','candidate','-Lane','binary-analysis-sample','-Subject','binary behavior candidate','-Summary','candidate awaiting bounded generic binary review','-Actor','binary-agent','-Confidence','high','-Status','open','-Risk','medium','-TargetRef','function-init','-BatchId','batch-generic-real-dryrun','-EvidenceRefs','workspace/binary/binary-analysis-sample/packet.md') | ConvertFrom-Json
  if ([string]$candidate.command -ne 'note' -or -not [bool]$candidate.applied -or [string]$candidate.path -ne '.rekit/facts/candidates.jsonl' -or [string]$candidate.event.kind -ne 'candidate' -or [string]$candidate.event.lane -ne 'binary-analysis-sample') {
    throw "unexpected generic-binary candidate append: $($candidate | ConvertTo-Json -Depth 20)"
  }
  ...
  if ([string]$gatePreview.command -ne 'gate' -or [bool]$gatePreview.isMutation -or -not [bool]$gatePreview.requiresConfirmation -or [string]$gatePreview.eventPreview.status -ne 'pending-gate' -or [string]$gatePreview.eventPreview.gate.action -ne 'debug') {
    throw "unexpected generic-binary gate preview: $($gatePreview | ConvertTo-Json -Depth 20)"
  }
  ...
  if ($beforeRequests -ne $afterPreviewRequests) { throw 'generic-binary gate what-if changed requests ledger' }
  ...
  if ([string]$gateApply.command -ne 'gate' -or -not [bool]$gateApply.isMutation -or -not [bool]$gateApply.applied -or [string]$gateApply.event.status -ne 'pending-gate' -or [string]$gateApply.event.gate.action -ne 'debug' -or [string]$gateApply.event.gate.scope -ne 'function behavior only') {
    throw "unexpected generic-binary gate apply: $($gateApply | ConvertTo-Json -Depth 20)"
  }

  $verification = Invoke-RekitSmoke -Arguments @('-Command','note','-Target',$caseRoot,'-Pack',$Pack,'-Kind','verification','-Lane','binary-analysis-sample','-Subject','bounded binary review','-Summary','bounded reviewer accepted generic binary evidence','-Actor','reviewer-smoke','-Verifier','tool-review','-Verdict','accepted','-TargetRef','function-init','-BatchId','batch-generic-real-dryrun','-EvidenceRefs','workspace/binary/binary-analysis-sample/packet.md') | ConvertFrom-Json
  if ([string]$verification.command -ne 'note' -or -not [bool]$verification.applied -or [string]$verification.event.verifier -ne 'tool-review' -or [string]$verification.event.verdict -ne 'accepted') {
    throw "unexpected generic-binary verification append: $($verification | ConvertTo-Json -Depth 20)"
  }
  ...
  if ([string]$decision.command -ne 'note' -or -not [bool]$decision.applied -or [string]$decision.event.decision -ne 'accept') {
    throw "unexpected generic-binary decision append: $($decision | ConvertTo-Json -Depth 20)"
  }
  ...
  foreach ($expected in @('generic binary debug gate','status=pending-gate','by=runtime-test','risk=medium','action=debug','scope=function behavior only','budget=30s','batch=batch-generic-real-dryrun')) {
    Assert-ContainsText -Text $requestList -Expected $expected -Label 'generic-binary request note list'
  }
  ...
  foreach ($expected in @('bounded binary review','verifier=tool-review','verdict=accepted','target=function-init','batch=batch-generic-real-dryrun')) {
    Assert-ContainsText -Text $verificationList -Expected $expected -Label 'generic-binary verification note list'
  }
  ...
  if ([string]$overviewJson.command -ne 'overview' -or [bool]$overviewJson.isMutation -or [string]$overviewJson.pack -ne $Pack -or [int]$overviewJson.counts.candidates -ne 1 -or [int]$overviewJson.counts.requests -ne 1 -or [int]$overviewJson.sections.pendingGates.total -ne 1 -or [int]$overviewJson.sections.verifications.total -ne 1 -or [int]$overviewJson.sections.decisions.total -ne 1 -or [int]$overviewJson.sections.batches.total -ne 1) {
    throw "unexpected generic-binary overview JSON: $($overviewJson | ConvertTo-Json -Depth 20)"
  }
  ...
  foreach ($expected in @('binary-analysis-sample','binary behavior candidate','generic binary debug gate','action=debug','scope=function behavior only','verifier=tool-review','verdict=accepted','decision=accept','reviewer accepted generic binary evidence','batch-generic-real-dryrun')) {
    Assert-ContainsText -Text $overview -Expected $expected -Label 'generic-binary overview text'
  }

  $handoff = Invoke-RekitSmoke -Arguments @('-Command','handoff','-Target',$caseRoot,'-Pack',$Pack,'-Apply','-Format','json','sample') | ConvertFrom-Json
  if ([string]$handoff.command -ne 'handoff' -or -not [bool]$handoff.isMutation -or -not [bool]$handoff.applied -or [bool]$handoff.project -or [string]$handoff.lane.id -ne 'binary-analysis-sample' -or [string]$handoff.lane.workspace -ne 'workspace/binary/binary-analysis-sample') {
    throw "unexpected generic-binary handoff result: $($handoff | ConvertTo-Json -Depth 20)"
  }
  $handoffPath = Join-Path $caseRoot '.rekit\handovers\binary-analysis-sample-latest.md'
  if (-not (Test-Path -LiteralPath $handoffPath)) { throw "missing generic-binary handoff: $handoffPath" }
  $handoffText = [System.IO.File]::ReadAllText($handoffPath, [System.Text.Encoding]::UTF8)
  foreach ($expected in @('binary-analysis-sample','/rekit continue binary-analysis-sample','workspace/binary/binary-analysis-sample/packet.md','## verification','verifier=tool-review','## decision','decision=accept','## pending-gate','action=debug','scope=function behavior only')) {
    Assert-ContainsText -Text $handoffText -Expected $expected -Label 'generic-binary handoff text'
  }
  foreach ($unexpected in @('web-security','workspace/features','feature-authz','endpoint-login','action=network','references/template','vmp-re')) {
    Assert-NotContainsText -Text $handoffText -Unexpected $unexpected -Label 'generic-binary handoff leakage guard'
  }

  Invoke-RekitSmoke -Arguments @('-Command','doctor','-Target',$caseRoot,'-Pack',$Pack) | Out-Null
  'generic-binary agent-team dry-run smoke ok'
} finally {
