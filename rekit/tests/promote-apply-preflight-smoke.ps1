...
try {
  ...
  $goReject = Invoke-GoRekitSmoke -Arguments @('-Command','promote','-Target',$caseRoot,'-Pack',$Pack,'-Apply') -AllowedExitCodes @(1)
  Assert-ContainsText -Text $goReject -Expected 'does not implement promote -Apply' -Label 'Go promote apply guard'
  ...
  Restore-TreeSnapshot -Root $packReferencesRoot -BeforeSnapshot $beforePackRefs -BeforeDirectories @{} -RootExisted $true
