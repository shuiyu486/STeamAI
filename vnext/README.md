...
它不 import 或调用旧 Go control plane、session host、current loop，也不发布 runtime executable。旧产品控制面已在 VNT-05 删除；legacy roots 只作为一次性 importer 的只读输入。
...
- `templates/research/`：值得跨成员共享和复核的研究产物。
...
- `legacy-import.md`：旧 `.steamai` / `.rekit` 到薄核心的一次性只读导入合同。
- `project-skill/SKILL.md`：分发到外部 case 的 canonical skill exact byte mirror。

外部 case 同时得到 `.steamai-vnext/contracts/` 中的模板/learning/import 合同，以及 selected pack 与 `common/**` policy closure 的同 revision 只读 snapshot；分发后日常不读取 mutable source clone。
