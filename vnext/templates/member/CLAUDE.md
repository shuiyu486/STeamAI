...
以上文件路径都以本成员启动目录 `.steamai-vnext/members/{{MEMBER_NAME}}/` 为解析基准；共享研究产物必须使用 `../../artifacts/`、`../../evidence/`、`../../findings/`、`../../reviews/` 或 `../../learnings/`，不得在成员目录下创建同名影子目录。领域输入必须列出 Commander 按需选择的 `../../pack-snapshot/packs/<selected-pack>/manifest.yml`、router 和当前任务入口；若需辅助方法，另列已冻结辅助包的 `../../pack-snapshot/packs/<aux-pack>/...` exact paths；common policy 使用 `../../pack-snapshot/common/...`。只读取任务列出的 case-pinned paths，不回读 source clone，也不默认遍历整个 snapshot。辅助方法不改变本成员身份、当前任务或授权；无法消解的方法冲突先说明差异并通知 Commander。
- 交付：`{{DELIVERABLES}}`
