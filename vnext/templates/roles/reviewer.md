...
- learning 审查分两级：先逐 candidate 检查证据、跨 case 通用性、反例、重复、冲突、脱敏、Claim kind/Required maturity 和单一 destination eligibility；再按 thematic batch 绑定所有 eligible candidate/review、target pre/postimage 与最终完整 exact patch。behavioral/V3 batch 还需在 opaque-arm review 后绑定 current calibration=`go`、hard safety=`pass`、comparative=`improved` 与完整 run bundle，之后才解盲；最多一个 tie-breaker，不无限重跑。candidate review 不授权 patch，只有 accepted batch review 才能申请用户 exact 确认。